...
| §110 | 2026-06-23 — **Proceso de MIGRACIÓN server-side (no re-subir)** (directiva arquitecto Daniel): `scripts/migrate-*.mjs` (Admin SDK + sharp, dry-run/idempotente) — `.110.1` backfill LQIP (PENDIENTE credencial bersaglio: ADC=altorra). `.110.2` hallazgo: `siteContent` (nosotros/hero) sigue vacío→foto (render LQIP revertido §106, F3 solo cubrió piezas+colecciones). `.110.3` auth gotcha quota_project per-proyecto. [OPUS-4.8] | 1646 |
...
| §115 | 2026-06-24 — **RBAC rol "catálogo" de Kary EN PROD** (TODO-19/31a): `catalogo:0` bajo editor → auto-denegado salvo Piezas+Colecciones; candado reglas + CFs + nav/login + UI usuarios crea cuentas. Verif. EN VIVO (Kary). **4 fixes cazados en vivo**: 3er mapa de rol (L-55) · createUser 403 invoker→delete+recreate (L-56) · menú falsy-0 `??` (L-55) · parpadeo blanco admin→shell inmediato (L-57). Pend.: panel tipo app (TODO-33, Decisión Fuerte). rules 196/196. [OPUS-4.8] | 1703 |
| §116 | 2026-06-25 — **Programa de Visibilidad (TODO-35) EN PROD** (trigger "ALTORRA CARS YA TERMINO CONTINUA"): SSG portado de Altorra → piezas/colecciones/journal indexables (`generate-pieces.mjs`+`urls.js`+`404.html`+robots-IA+cron); marca/Maps (`tenant_config.json`→JewelryStore+WebSite+seller); GA4 ENCENDIDO (estaba muerto) + Consent Mode v2; bugfixes (maps/iconos/NAP). **GA/GSC config (Chrome)**: retención 14m + GA4↔GSC vinculado + sitemap reenviado. Cuenta `bersagliojewelry@gmail.com` authuser=3. Sin cache bump. TAIL: A2b/eventos-clave/IP/HUB-prompt. META: ampliar skills GA4/GSC (config consola) → HUB. CRUDO→bóveda. [OPUS-4.8] | 1715 |
| §117 | 2026-06-25 — **Storage: rol catálogo sube imágenes** (URGENTE, Daniel): desacople Firestore↔Storage — al crear `catalogo` (§115) se abrió en `firestore.rules` pero NO en `storage.rules` (`isContentRole` sin catálogo) → Kary editaba la pieza pero Storage rechazaba la foto ("Error al subir imagen"=`storage/unauthorized`, disfrazado de "conexión"). Fix: mirror `isCatalogo()` en storage (pieces+collections); `assets`=editor+ (least-privilege). Validado+deploy storage. `e300ff2`. LECCIÓN: al cambiar un rol, reflejarlo en TODOS los artefactos del contrato. [OPUS-4.8] | 1727 |
| §118 | 2026-06-25 — **Ficha pieza "Carta Gemológica" + fixes demo-críticos (TODO-34) EN PROD**: `buildSpecs` agrupa (hero gema + Calidad/Metal/Origen, hide-when-empty, escala 2→11 sin huecos) · `descriptionFor` cero-demo · riel "También podría gustarte" con URLs limpias `pieceUrl` (`10a26bf`, no `?p=`) · refinamientos Daniel (esmeralda, columnas armonizadas, solo datos reales) · `og-image.jpg` 1200×630. Cache v32. Mergeado PR #356/#357. Piezas `zzz-prueba-*` ya removidas. [OPUS-4.8] | 1739 |
| §119 | 2026-06-25 — **Grilla inteligente (Flexbox) + recos por contenido + GA4 (TODO-36) EN PROD** (W-11 comité×4+Gemini): `balancedCols` (`grid-balance.js`) sin huérfana (5→3+2) vía `--cols`+`justify-center` (cambio Gemini) · `computeRelated` categoría=COMPUERTA + título HONESTO + fallback curado (cambio Gemini) · GA4 `view_item`(+`source_piece_slug`)/`view_item_list`(IO)/`select_item` arreglado (`.piece-card`→`[data-piece-slug]`, evento muerto cazado por Gemini). 25 tests. Cache v33. PR #357/#358. [OPUS-4.8] | 1751 |
| §120 | 2026-06-25 — **Plan Maestro Comercio B0+B0.5 — WhatsApp directo (frena la fuga)** (TODO-37 arranca): CTAs de la ficha (sin-precio/con-precio/404) abren WhatsApp con la pieza escrita (nombre+Ref+URL) vía helper puro `waLink` (fuente única del número, fallback real) · formulario conservado como vía secundaria (captura CRM hasta B2) · evento GA4 `whatsapp_click` con item de pieza + FB Lead. Cache v34. 266 tests + enlace E2E por Node. EN PROD ✅ (PR #359). SIGUIENTE: B1 mostrador. [OPUS-4.8] | 1763 |
| §121 | 2026-06-26 — **Catálogo de prueba EN PROD + decisiones del dueño**: 9 piezas de prueba (`seedDemo:true`, 5 c/precio + 4 bajo consulta) subidas a prod vía Firebase MCP (ADC altorra no escribe prod, §110.3), enlazadas a las 7 colecciones reales de Kary; verificado EN VIVO en `bersagliojewelry.co` (9 renderizan, 0 errores). DECISIONES: pruebas en web real (excepción temporal a no-demo, fase pre-lanzamiento) · Wompi = cuenta de Kary Persona Natural (NO PJ; topes $2.5M/$10M; aumento a las 20 tx, Daniel avisa) · ADDI CONGELADO (Kary vincula). [OPUS-4.8] | 1775 |
| §122 | 2026-06-26 — **B1 paso 1: extensión de inventario en `pieces` EN PROD**: campos nuevos que controla Kary — `stockType`(finito/encargo)·`cantidad`(int≥0)·`gender`(mujer/hombre/unisex) vía `pieceClassValid()` (type+enum, idiom de opcionales, sin hasOnly). `estado`/`reserva*` (stock transaccional) DIFERIDOS al CF (candado CF-only, seguridad). Form admin + seed. **Reglas DESPLEGADAS a prod** (read-back OK) · 201 tests · build verde. SIGUIENTE paso 2: CF `cotizacionRapida`. [OPUS-4.8] | 1787 |
