...
| §131 | 2026-06-27 — **Inventario v3 (TODO-40 F1) DESPLEGADO a prod**: B4 admin (enum3+visibilidad+cantidad CF-only) + B5 SSG/cliente + B6. Daniel mergeó (#378/#379); reglas v3 + functions (`crearPedido`/`ajustarStock`/`cambiarTipoPieza`) DESPLEGADAS. 213/213 reglas. L-59. [OPUS-4.8] | 1884 |
| §132 | 2026-06-27 — **Carga masiva: 32 piezas reales (certificados TrueLab)**: QR (`jsqr`+`sharp`) → scrape del SPA (Chrome MCP, pushState, sin tocar credenciales) → clasif. de tipo por foto (montage) → carga vía MCP. Código=Nº reporte, Oro 18k, imágenes del cert (temporales). 32/32 EN VIVO. L-60. [OPUS-4.8] | 1895 |
| §133 | 2026-06-27 — **Evaluación integral de marca** (12 págs+8 comps+32 piezas+comité 5 lentes): arreglos seguros (reseñas inventadas eliminadas · footer 3 enlaces rotos · "sin precio"→`priceLabel` · slug oculto = TODO-45 ✅) + backlog TODO-47-51 (riesgo SIC). v42. [OPUS-4.8] | 1906 |
| §134 | 2026-06-27 — **Voz de marca del catálogo** (TODO-44 ✅): 32 piezas (nombre + descripción 3-mov + badge, vía MCP) + 9 destacadas variadas + 7 colecciones reescritas + **skill `catalogo-voz-bersaglio`** (invocable). Badge en tarjeta, v43. [OPUS-4.8] | 1918 |
