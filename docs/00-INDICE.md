...
| §106 | 2026-06-23 — **REVERT del render LQIP (§104 F1)** (Daniel: "ahora PEOR — vacío→borroso→nítido = 3 estados"). En MPA estático el LQIP llega con el getDoc (no antes) → SUMA un 3er estado en vez de reemplazar el vacío (§103.3). Revertido SOLO el render (`editorial`/`hero`/`nosotros` → vuelve a neutro→foto §102); **conservada la fontanería** (makeLqip/safeLqip/lqip.js/companion/defaults/tests) para el F2, donde la caché-memoria hace que el borroso PRECEDA a la imagen. Tests 55/55, build ✓, cache v26. Lección: feedback usuario > teoría; un placeholder que suma estado empeora. L-50. [OPUS-4.8] | 1586 |
