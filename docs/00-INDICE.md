...
| §112 | 2026-06-23 — **Cache-Control en imágenes de Storage** (el blur salía en CADA visita, no solo la 1ª): RCA = objetos subidos SIN cacheControl → Firebase sirve `private, max-age=0` → revalida cada visita. Fix `_upload` (`public, max-age=31536000`; URL versionada por token) + backfill `migrate-cache-control.mjs` (18/18, verificado curl). F3 de §108.10. `2eb71fe`. → L-53. [OPUS-4.8] | 1668 |
