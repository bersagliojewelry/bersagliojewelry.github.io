...
>
> ⚠️ **Índice repartido (range-shard §140)**: aquí viven los ADR **recientes (§116+)**. Los **antiguos (§1–§115)** están en la hija **`docs/00a-INDICE-HIST.md`** (offsets congelados). El enrutamiento semántico de abajo aplica a TODO el cerebro.
...
| ¿Cómo/dónde se calcula la mora / aging / cartera vencida / a quién cobrar? | `js/crm-estado-cuenta.js` (helper PURO `estadoCuenta`, FIFO en vivo) + ADR §51 (`00a`) |
...
| El "por qué" de una decisión / detalle de un § | tabla "§ → línea" abajo (§116+) **o** `00a-INDICE-HIST` (§1–§115) → 📚 `99-HISTORIAL` |

---

## Mapa § → línea (§116+ · recientes)

> ⏪ **¿Buscas §1–§115?** Están en la hija **`docs/00a-INDICE-HIST.md`** (histórico congelado).
...
| §140 | 2026-06-27 — **Cerebro: range-shard del índice → `00a-INDICE-HIST`** (§1–§115): descongestiona `00` (30.8k→~10k); kernel intacto (TODO-55 para extender check #3). Completa el range-shard diferido por TODO-32. [OPUS-4.8] | 1991 |

> Mantener este índice sincronizado: cuando se agregue un ADR §141+ al historial,
> añadir su fila aquí con la línea de inicio (`Select-String` o `grep`). Cuando `00`
> se acerque de nuevo al tope, mover el tramo más viejo a `00a-INDICE-HIST` (range-shard, §140).
