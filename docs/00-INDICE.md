...
| §101 | 2026-06-23 — **Proceso de validación de Decisión Fuerte — veredicto** (consejo Gemini doble-ciego). El pipeline `proceso-decision-fuerte` ya era sólido (Gemini independiente —anti-anclaje, NO "ciego": SÍ lee código/cerebro— reinventó casi lo mismo = convergencia/validación). Cambio único: Paso 7 endurecido a **GATE EMPÍRICO con "Pruebas de Estado"** ("verificado" jamás sobre análisis estático; evidencia real antes del gate humano de lo irreversible) + doble-ciego paralelo preferible. Refuté de Gemini: cámara-de-eco (el comité con lentes diversos cazó la Ley 1581 que Gemini no vio) + latencia-desincroniza (refutado en vivo). Aclaración Daniel: Gemini puede editar (agente), le damos read-only por rol. NO 2º comité. Skill+bóveda. [OPUS-4.8] | 1523 |
| §102 | 2026-06-23 — **Carga fluida: placeholder verde→neutro (corrige UX del §100)**. Daniel: "pantalla verde antes de la foto, no fluido como colecciones". RCA: el §100 usó gradiente esmeralda SATURADO como placeholder (se ve como pantalla de espera). Patrón fluido ya existía (`featured.js`): neutro `oklch(94% 0.02 150)` invisible + imagen como `background:url` encima. Fix: 4 placeholders → neutro + render con background-image inline (hero/atelier Nosotros, editorial Home). Error de CRITERIO de diseño, no arquitectónico. Verif. PROD navegador real. Skill `arquitecto-software` invocada. Cache v23. `2a1936a`/`da9feae`. L-46. [OPUS-4.8] | 1536 |
| §103 | 2026-06-23 — **Arquitectura carga "app-like" (DECISIÓN; implementación PENDIENTE)**. Daniel: imágenes CMS lentas + nav no-app + "sin caché que tape cambios en vivo". Workflow completo (comité×3 + Gemini, bóveda; Gemini superó a mi comité). Veredicto: **router falso-SPA** (memoria→nav instantánea) + caché-memoria + prefetch-hover + **Blurhash en Firestore**; CERO SWR (flicker)/prerender (costo). Refutado: ejemplo precio (siteContent sin precios); matiz: 1ª carga fría espera getDoc (instantáneo-frío solo con SSG-bake). FORK→GO Daniel. Faseado (F1 Blurhash: rules+modelo+admin+render; F2 router: toda la nav) → sesión fresca + gate empírico. Decidir≠implementar (§101). [OPUS-4.8] | 1548 |
| §104 | 2026-06-23 — **Implementación §103 F1: LQIP "blur-up" en imágenes del CMS**. data-URI ~40px (`makeLqip`, reusa canvas) en campo compañero `<campo>Lqip` dentro de `siteContent`; render = doble fondo CSS (real arriba/LQIP detrás) en divs + LQIP como fondo del `<img>` en hero-Home/avatar → blur-up sin JS/CSP, degrada solo. **LQIP > Blurhash-lib** (cero dep, reusa canvas; `safeLqip` regex base64 = imposible breakout del url()). **Corrige §103.6**: `siteContentValid` whitelista solo SECCIONES (no claves internas) → campo interno aditivo = CERO cambio de `firestore.rules`/deploy. Cache v24. Tests 257/257 + reglas 187/187 (+lock) + build ✓. Colateral: reparado test-rot §102 en `no-demo-home` (6/10 rojo silencioso; node:tests sin CI → M-07). L-47/L-48. [OPUS-4.8] | 1560 |
| §105 | 2026-06-23 — **Fix "zoom/asentamiento" feo de imágenes en RECARGA** (Daniel, prod). Causa MEDIDA (no asumida): NO es resize de imagen (caja `.cat-tile-img` estable 180×242, transform none); es la entrada **`.reveal`** (translateY30+opacity, 0.9s) reproduciéndose en recarga sobre secciones ya visibles → el transform promueve a capa GPU y rasteriza/difumina las imágenes. Fix SISTÉMICO en `reveal.js`: lo visible en el 1er paint se asienta SIN animar (`.reveal-static`); solo lo que entra por scroll anima → cubre TODAS las secciones con imágenes (incl. piezas). Verif. navegador: above-fold instantáneo, below-fold anima al scroll, 0 errores. Cache v24. L-49. [OPUS-4.8] | 1574 |
| §106 | 2026-06-23 — **REVERT del render LQIP (§104 F1)** (Daniel: "ahora PEOR — vacío→borroso→nítido = 3 estados"). En MPA estático el LQIP llega con el getDoc (no antes) → SUMA un 3er estado en vez de reemplazar el vacío (§103.3). Revertido SOLO el render (`editorial`/`hero`/`nosotros` → vuelve a neutro→foto §102); **conservada la fontanería** (makeLqip/safeLqip/lqip.js/companion/defaults/tests) para el F2, donde la caché-memoria hace que el borroso PRECEDA a la imagen. Tests 55/55, build ✓, cache v26. Lección: feedback usuario > teoría; un placeholder que suma estado empeora. L-50. [OPUS-4.8] | 1588 |
