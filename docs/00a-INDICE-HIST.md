# 00a — ÍNDICE SINÁPTICO · HISTÓRICO (§1–§115 · mapa § → línea del Historial ADR)

> **Nodo neuronal: hija de `00-INDICE` (range-shard §140).** Mapa § → línea de
> `docs/99-HISTORIAL-ADR.md` para los ADR **antiguos (§1–§115)**, congelados
> (sus offsets no cambian: el Historial es append-only). Se consulta on-demand
> (Trigger de Error/Historia, ver `CLAUDE.md §G`).
>
> **¿Por qué existe?** `00-INDICE.md` se desbordaba crónicamente (~28k tope; 30.8k);
> el plan del manifest (`_caps_trinquete`) era partir en `00a/00b`. Aquí vive la
> mitad FROZEN; los ADR **recientes (§116+)** y el enrutamiento semántico siguen en
> la madre **`00-INDICE.md`** ← ve allí para lo vivo.
>
> **Cómo usarlo (regla anti-saturación)**: busca aquí el § + su línea → `Read docs/99-HISTORIAL-ADR.md offset=<línea> limit=~150`. NUNCA leas el historial completo (satura al instante). La línea es **pista, no verdad**: si el tramo no arranca en el header esperado, regenera con `grep -n "^## " docs/99-HISTORIAL-ADR.md` (o `Select-String`).

---

## Mapa § → línea (§1–§115 · histórico)
...
> **Frontera del shard (§140)**: §116+ y el enrutamiento semántico viven en la madre `00-INDICE.md`.
> Mantén este índice histórico sincronizado solo si se RE-SHARDEA `99` (sus offsets son congelados);
> los ADR nuevos NO entran aquí — entran en `00-INDICE.md`.
