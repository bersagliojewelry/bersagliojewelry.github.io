...
| **Cache version vigente** | `bersaglio-v78` (`public/sw.js`; SW↔05 OK; admin `APP_VERSION v41`). v78 = §170 pulido revisión adversarial UI caja/bóveda (revalidar turno · opId propio · guard overlays · retry listeners); v77 = B5c/§169 fixes (reponer turnoId · opId perezoso); v76 = UI caja/bóveda F2.0 B5b-1. |
...
- 🟢 **PLAN ÚNICO ERP v4 (TODO-68**, spec `2026-07-04-plan-unico-erp-v4`) = roadmap vigente. **F1 COMPLETO ✅ (§165-§168)**. **F2.0 caja + bóveda antirrobo COMPLETO ✅ EN PROD (§169)**: backend (8 CFs) + UI (POS turno + Bóveda owner + aprobaciones) desplegados + `config/caja` sembrado (`enforceTurno:false·fondoTrabajo:200000·limiteCajon:4000000`) + **gate Chrome owner VERDE** (2026-07-07) + 2 fixes post-gate + **§170 pulido revisión adversarial** (4 hallazgos bajos, SW v78). **Pend OPERATIVO (no dev)**: runbook §9.4 (Kary activa `enforceTurno`). **B5d rol `caja` DIFERIDO** (L-78; Kary crea la cajera después). Sigue F2: 2.3 térmica+cajón · 2.4 apartados. **GATE vivo: precios reales + 1ª APPROVED.**
