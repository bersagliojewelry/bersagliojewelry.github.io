...
| **Branch activa** | `Desarrollo` (solo docs de cerebro sin mergear — viajan con el próximo PR). **🔒 TREN M2a+M2b+M3+M4 EN PROD (§78.8: PR #232; v13 + sección Cartera + aviso SLA verificados VIVOS por fetch)**. Vigilar: 1er corte real = 1 de julio 03:50 Bogotá (`cortes/2026-06`, respaldo `generarCorte`). **13 functions** nodejs22 vivas (`corteMensual` + `generarCorte`); `.co` HTTP 200. `git fetch` antes de afirmar (L-26). |
