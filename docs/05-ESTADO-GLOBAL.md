...
| **Cache version vigente** | `bersaglio-v88` (`public/sw.js`; SW↔05 OK; admin `APP_VERSION v48`). v88 = rail v2 + KPI cartera + void-gate (§182). v86-v87 = anti-fugas (§181). Historial → ADRs. |
| **PROD / branch** | `Desarrollo`≈`main` (**merge a main = Claude**, autoriz. `[[feedback_claude_deploy_autorizado]]`; **commit exacto → `git fetch`**, §114). **EN PROD (main)** hasta §179 (Mostrador+caja OBLIGATORIA · F2.2 §176 · Grupo A UX §177 · index skeleton §178 · **POS-pro money-model §179: datáfono+pago-dividido+sin-pieza+vouchers, DESPLEGADO** · 32 piezas v3 · legal). **`main`≡`Desarrollo` alineados**. **TODO-73 ✅ OPERATIVO** (`firebase deploy --only functions` hecho + gate holístico verde; interinato respetó webhook/firma/reaper/snapshot). Deploy functions/rules = MANUAL (CI no los toca: deploy.yml=Pages, firebase-deploy.yml=Hosting; L-22). **Pend operativo Daniel**: precios reales (⛔ GATE) + 1ª APPROVED + imágenes IA. **MCP Firebase = escritura prod**. `arquitecto-software` SIEMPRE. |
...
- 🟢 **PLAN ÚNICO ERP v4 (TODO-68**, spec `2026-07-04`) = roadmap. **F1·F2.0·F2.1·caja-tiempo-real·TODO-70 ✅ EN PROD (§165-§173)** (`config/caja enforceTurno:true` verificado-vivo:2026-07-08 · `config/identidad.activo=true` LIVE Kary; detalle→ADRs). **F2.2 ✅ OPERATIVO (§176)** (falta cargar servicios reales, data). **Grupo A UX (§177)** + **index skeleton (§178, carga fría, TODO-74 cerrado)** ✅ EN PROD (`[[feedback_validacion_chrome_holistica]]`). **TODO-73 POS-pro money-model ✅ OPERATIVO EN PROD (§179)** (datáfono·`pagos[]` split-tender·venta-sin-pieza·vouchers al cierre; core+UI desplegados, f2-3 16/16 + gate Chrome holístico verde). SIGUE: F2.4 apartados (TODO-39) → 2.3 térmica. **GATE negocio: precios reales + 1ª APPROVED.**
- 🟣 **FABLE 5 ACTIVO hasta el 12-jul** (cuota %). §181 (anti-fugas) y **§182 (rail v2 + plan maestro v5)** DESPLEGADOS. Norte: spec `2026-07-10-plan-maestro-v5` (F-IA-2 → F-TESORERIA → F-COMPRAS → F-REPORTES → limpieza → rompimiento → lanzamiento). **Gate de Daniel: Kary NO usa el sistema hasta confiabilidad 100%.** 1ª APPROVED ✅; resta cargar precios reales.
