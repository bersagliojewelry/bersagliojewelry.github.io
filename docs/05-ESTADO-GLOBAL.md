...
| **Cache version vigente** | `bersaglio-v89` (`public/sw.js`; SW↔05 OK; admin `APP_VERSION v49`). v89 = F-IA-2 B1 "Negocio y equipo" con pestañas (fusiona Parámetros+Usuarios; rail Sistema=2 ítems). v88 = rail v2 (§182). Historial → ADRs. |
...
- 🟣 **INTERINATO #3 · OPUS 4.8** (2026-07-10; Fable al 93%, vuelve tras reset del jueves). Protocolo `opus-interino-protocolo` VINCULANTE (R1-R7). **SOLO F-IA-2** por spec `2026-07-10-f-ia-2-DISENO.md`: **B0 ✅ auditado por Fable (mismo día) + decisiones CERRADAS en §0.7 (D1-D7) → Opus ejecuta B1→B5 SIN preguntar** (CERO functions/dinero nuevo). Cola del TITULAR: auditar B1-B5 + F-TESORERIA + F-COMPRAS + F-REPORTES + apartados (spec §6). Norte: plan v5. **Gate de Daniel: Kary NO usa el sistema hasta confiabilidad 100%.** 1ª APPROVED ✅; precios reales = paso final.
