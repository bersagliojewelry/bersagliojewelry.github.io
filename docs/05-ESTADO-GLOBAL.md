...
| **Cache version vigente** | `bersaglio-v98` (`public/sw.js`; SW↔05 OK; admin `APP_VERSION v55`). v98 = F-TESORERÍA bundle B2-B5 (**en código, deploy MANUAL pend.**). Historial → ADRs. |
...
- 🟢 **Roadmap = plan maestro v5** (spec `2026-07-10`). F1→F2.2 + caja tiempo real + POS-pro + **F-IA-2 ✅** EN PROD (§165-§192; `enforceTurno:true`/`identidad.activo` LIVE). SIGUE: **F-TESORERÍA (TODO-78; **B0-B4 + B5(D6·V1·V18) ✅ en código** —CFs/reglas/índices/espejo/página+cuadre+Bandeja/Hoy/socias+editor de reglas+frontera bóveda↔banco con pata atómica; verdes offline (teso 31/31 · **caja 38/38 sin regresión**), NO en prod; SIGUE B5: **V17** + D9 → B6 → deploy bundle, ver `10`+spec §9)** → F-COMPRAS → F-REPORTES → F2.4 apartados (TODO-39) → 2.3 térmica → limpieza → rompimiento → **lanzamiento (ahí: precios reales)**. 1ª APPROVED ✅.
- 🟣 **INTERINATO** · #3 ✅ auditado (§192). **#4 EN CURSO — ahora `OPUS 5`** (salió 2026-07-24; Daniel: todo lo de Opus 4.8 pasa a Opus 5 → marca `[OPUS-5]` + footer Opus 5). B0 auditado · B1 del titular · **B2·B3·B4 ✅ (los ejecutó Opus 5; sus commits llevan `[OPUS-4.8]` heredado por ERROR — pusheado, NO se reescribe: auditar por AMBOS marcadores) · B5 D6·V1·V18 ✅ `[OPUS-5]`**. SIGUE B5(V17·D9)·B6. Spec = 3 capas de refutación (§0.6/§0.7/§0.8: Kary carga sus cuentas por UI, cero seed). **Zonas calientes R3: bóveda(V1)+caja(V17)** = test primero (B5). **Gate Daniel: Kary NO usa hasta confiabilidad 100%.**
