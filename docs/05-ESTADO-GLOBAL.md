...
| **Cache version vigente** | `bersaglio-v10` (`public/sw.js`). Bump por Fase M / M2a. Indicador `APP_VERSION` `v10` en el sidebar (criterio de deploy). |
| **Branch activa** | `Desarrollo` ≡ `main` (merge `3c8f028`). **M2a COMPLETO Y EN PROD** (corrección de Kary): reglas M1 §72 + M2a-1b §73 + **UI verificada por experto §75** (12 agentes → 2 bugs de dinero corregidos; ⚠️ el PR #223 de Daniel publicó la UI buggy primero → fixes mergeados y CONFIRMADOS en vivo en el bundle). Cache `v10`. Siguiente: M2b. Prod `main` (hasta #221) DESPLEGADO: rediseño + CRM + Fase R + Panel v2 + Morosos §51 + F5 §52 + F4 §53 + frente D §64. **11 functions** nodejs22 vivas (§64 + §65 DESPLEGADOS, verificado `functions:list`); `.co` HTTP 200. `git fetch` antes de afirmar (L-26). |
