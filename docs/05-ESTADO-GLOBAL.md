...
| **Branch activa** | `Desarrollo` ≈ `main` (deploys frecuentes; **antes de mergear: `git fetch` + `reset --hard origin/main`** — Daniel mergea en paralelo por GitHub Desktop, L-26). **🆕 CMS WYSIWYG EN PROD (2026-06-18, validado Daniel)**: F1 preview fiel + F2 clic-para-editar + fidelidad desktop (viewport 1440 escalado) + preview sticky (`.adm-layout{100dvh}`) + **P3.5 imagen de portada** editable; `storage.rules` desplegado (+avif). **P4 (Nosotros editable: listas + modelo plano 12 claves) ✅ EN PROD** (2026-06-19, §83; validado Daniel; `firestore.rules`+`storage.rules` desplegadas). **Imágenes de Nosotros (hero/atelier/equipo) en `Desarrollo` `b78c3aa`, pendiente merge.** **🔒 TREN M0→M6 EN PROD**; ACUERDOS R1-R5 gateados/inertes, falta R6 (§81). ⚙️ **OPUS 4.8 interino**. 1er corte = 1 jul. SW `v16`; `.co` 200. |
