...
| **Branch activa** | `Desarrollo` (**sin mergear: F6 App Check §54 + frente B §65 + fix seg §66** `e9ecc37`). Prod `main` (PR #214) DESPLEGADO: rediseño + CRM + Fase R + Panel v2 + Morosos §51 + F5 §52 + F4 §53 + frente D §64. **11 functions** nodejs22 vivas (§64 + §65 DESPLEGADOS, verificado `functions:list`); `.co` HTTP 200. `git fetch` antes de afirmar (L-26). |
...
- 🟠 **F6 en curso**: App Check REPARADO (§58, canje 200 en vivo) — TODO-14: monitor ~100% ×7d → **Enforce** guiado (PROHIBIDO antes, L-32). Forms endurecidos ✅ (§59) · CI+entero-COP ✅ (§62) · PRE-1 backup+restore ✅ (§63) · **reconciliación+Salud ✅ DESPLEGADA (§64)** (10 functions; §64 mergeado a `main` PR #214). **RBAC claims ✅ DESPLEGADO (§65, 2026-06-10)**: 11 functions vivas (`syncRoleClaim`) + claim owner sellado (backfill, 0 drift) + reglas publicadas (incluye §66 `users/` owner-only y "Desactivar" que SÍ bloquea). F6 técnico: solo queda **paginación cursor**. App Check: monitor ×7d → Enforce (TODO-14). Luego: política cartera APROBADA → config en Fase M.
