...
| **PROD / branch** | **Web "app-like" §103-§113 CERRADO EN PROD ✅** (View Transitions §107 + caché SWR §108 + LQIP §111 + Cache-Control Storage §112 + correo OWNER §113), validado Daniel+Chrome. `Desarrollo`≈`main` (merge por PR de Daniel). **Commit exacto → `git fetch` (NO fijarlo aquí, §3.3/§114).** **🎯 FOCO = usuarios&permisos**: TODO-19 (rol "catálogo" de Kary + candado en reglas) · TODO-31 (auto-creación de usuarios + fix login-parpadea); plan archivo-por-archivo en `50 §5`. `arquitecto-software` SIEMPRE. ⚙️ OPUS 4.8 interino. |
