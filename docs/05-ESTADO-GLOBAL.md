...
| **Branch activa** | `Desarrollo` == `main` (PR #239). **🔒 TREN M0→M6 EN PROD**. **ACUERDOS v2 (Consejo): R1-R5 ✅ en `main` pero GATEADO/INERTE (`acuerdosActivos` off) — nada visible cambió** (mutex `acuerdoVigenteId` · saldo · escudo · `acuerdoAlCorte`; rules 144/144). **Falta R6** (ADR §81): deploy + bandera + verif. POR HITO. ⚙️ **OPUS 4.8 interino** (revisar al volver Fable). 1er corte = 1 jul. **13 fns** vivas; `.co` 200; SW `v16`. `git fetch` (L-26). |
