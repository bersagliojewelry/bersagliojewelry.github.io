...
| **Cache version vigente** | `bersaglio-v96` (`public/sw.js`; SW↔05 OK; admin `APP_VERSION v53`). v96 = SEO A3 Journal indexable (`/journal/<slug>.html` horneadas, schema Article; enlaces → `entryHref`; sitemap 52 URLs). v95 = Journal CMS colocación (`cover`/`order`; reglas journalValid DEPLOY MANUAL). v94 = Journal pulido visual. v93 = F-IA-2 B5. Historial → ADRs. |
| **PROD / branch** | `Desarrollo`≡`main` alineados (**merge a main = Claude**, `[[feedback_claude_deploy_autorizado]]`; **commit exacto → `git fetch`**, §114). **EN PROD hasta §188** (verificado `git fetch` 17jul: `main` contiene el HEAD de `Desarrollo`) — rail v2 · POS-pro §179 · F2.2 · 32 piezas v3 · SEO A1-A4 ✅ (§184/§187/§188) · legal; detalle → ADRs. Deploy functions/rules = MANUAL (CI = Pages/Hosting; L-22). **Precios reales = PASO FINAL** (Daniel 2026-07-10: se cargan cuando TODO esté 100% confiable y terminado, al lanzar a Kary — NO es gate intermedio). **MCP Firebase = escritura prod**. `arquitecto-software` SIEMPRE. |
...
- 🟣 **INTERINATO #3 · OPUS 4.8** (2026-07-10). Protocolo `opus-interino-protocolo` VINCULANTE (R1-R7). **SOLO F-IA-2** (spec `2026-07-10-f-ia-2-DISENO.md`): **B0·B1·B2·B3·B4·B5 ✅ → F-IA-2 CODE-COMPLETE (SW v93).** B5 = Salud legible + "Cierre de mes" in situ (D4) + microcopy GLOBAL (SOLO texto; desvíos+encolados → spec notas B5). ⏳ Pend: validar B1-B5 funcional-con-sesión (roles/guardar/ciclo aprobar-rechazar/números vivos) en deploy con login. Cola TITULAR: **auditar B1-B5 `[OPUS-4.8]`** + F-TESORERIA/COMPRAS/REPORTES + apartados (§6). **Gate Daniel: Kary NO usa hasta confiabilidad 100%.** 1ª APPROVED ✅; precios = paso final.
