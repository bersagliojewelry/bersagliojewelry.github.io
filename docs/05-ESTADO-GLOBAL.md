...
|---|---|
| **Build** | 🟢 Vite VERDE (~5s). Panel v2 en prod (§50-§53); norte = spec maestra v3. Suite local verde: acuerdos 18/18 (A8 §87) · estado 24 · paridad byte 4 · saldo 12 · reconciliación 9 · **rules 185** (emulador Java; +films/social/journal-gate §88) · resource-admin 16 · no-demo 4 · backup 7. ⚠️ Preview headless no pinta dinámico (L-05) → verificar en `dev`/deploy. |
| **Cache version vigente** | `bersaglio-v18` (`public/sw.js`). Bump por CMS cero-ficción Fase B (§88: films/social leen umbrales del SSoT `home-sections.js`). Indicador `APP_VERSION` en el sidebar = criterio de deploy. |
| **Branch activa** | `Desarrollo` ≈ `main` (**antes de mergear: `git fetch` + `reset --hard origin/main`**; Daniel mergea en paralelo por GitHub Desktop, L-26). **CMS web pública COMPLETO en prod** (§83-§86; inc1-4, PR #265-#267): WYSIWYG · Home/Contacto/Nosotros · imágenes · `global` fuente única de contacto. **TODO-24 (index 100% gestionable) Fase A+B CONSTRUIDAS** (§88, SW v18; panel Videos/Redes + puerta reglas + UX Kary; rules 185✓): **pendiente deploy reglas + censo legacy** (journal/films/social published-incompleto → despublicar/completar). **🔄 RESET-A-CERO de cartera/clientes** (Kary recarga). **R6 bug A8 RESUELTO** (§87 `55bc8ef`, red-team 0 hallazgos; gated, encender=Daniel). 🔒 TREN M0→M6 EN PROD. ⚙️ OPUS 4.8 interino. 1er corte = 1 jul. SW `v16`; `.co` 200. |
