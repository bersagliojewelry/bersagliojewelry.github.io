...
| **Cache version vigente** | `bersaglio-v39` (`public/sw.js`; verificado SW↔05; admin `APP_VERSION v24`). Último bump: **B1 paso 4a — confirmar pago** (§128): las ventas "por verificar" (transferencia/Wompi) traen botón "Confirmar pago" → CF `confirmarPago` (`pago_por_verificar`→`pagado`, SoD "vi la plata", solo la CF flipea). (v38 = todo en COP §127; v37 = POS "Mostrador" §126.) `APP_VERSION` = criterio de deploy. |
| **PROD / branch** | `Desarrollo`≈`main` (merge por PR de Daniel; **commit exacto → `git fetch`**, NO fijarlo aquí §114). **EN PROD ✅**: web app-like §103-§113 · rol catálogo de Kary §115 (ve SOLO Piezas/Colecciones; L-55/56/57) · Visibilidad/SEO+GA4 §116 (Google bajo `bersagliojewelry@gmail.com` authuser=3; TAIL A2b/eventos/HUB) · ficha §118 + grilla/recos §119 (`pieces` limpia). **EN MAIN (v36)**: B1 hasta §124. **🎯 B1 mostrador (TODO-37)**: **paso 3 §125-§126 CERRADO (código)** — `crearPedido` CF (candado atómico=pieza, recompute server-side, idempotente; 206 rules + 6 integración) + **POS UI `admin-pos.html`** (UI espeja a la CF; `02ab6a7`). **POS §126 + COP §127 MERGEADOS a prod ✅** (Daniel: "TODO MERGEADO"). **Paso 4a §128 `confirmarPago`** (por_verificar→pagado, CF DESPLEGADA; código `ba6da22` en `Desarrollo`, PEND merge). **PEND**: verif. en vivo (Kary registra venta + confirma pago). **SIGUIENTE: decisión paso 4b apartados/abonos** (TODO-39: reusar cartera vs pagos-en-pedido) → VOID + arqueo (paso 5). `arquitecto-software` SIEMPRE. ⚙️ OPUS 4.8 interino. |
