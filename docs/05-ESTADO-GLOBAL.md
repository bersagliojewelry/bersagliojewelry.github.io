...
- 🟣 **INTERINATO #3 · OPUS 4.8** (2026-07-10). Protocolo `opus-interino-protocolo` VINCULANTE (R1-R7). **SOLO F-IA-2** (spec `2026-07-10-f-ia-2-DISENO.md`): **B0·B1·B2 ✅ (B1+B2 EN PROD, SW v90); §0.7 D1-D7 cerradas por Fable → sigue B3(Hoy $)·B4(Bandeja)·B5(Salud+microcopy) SIN preguntar** (CERO functions/dinero nuevo). ⏳ Pend: validar B1+B2 funcional-con-sesión (guardar/roles) en deploy. Cola TITULAR: auditar B1-B5 + F-TESORERIA/COMPRAS/REPORTES + apartados (§6). Norte: plan v5. **Gate Daniel: Kary NO usa hasta confiabilidad 100%.** 1ª APPROVED ✅; precios = paso final.
