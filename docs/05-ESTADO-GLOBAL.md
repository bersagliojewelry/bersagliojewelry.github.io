...
| **Cache version vigente** | `bersaglio-v23` (`public/sw.js`). Bump por §102 (carga fluida: placeholder verde→neutro casi-blanco patrón featured + imagen como background sin hueco/salto). Indicador `APP_VERSION` en el sidebar = criterio de deploy. |
| **Branch activa** | `git fetch` 2026-06-23: `origin/main` = `d91152c` (Daniel mergeó TODO; **§100·§101·§102 EN PROD** verif. en vivo: cero-demo + carga fluida + móvil 340×425 + QA público LIMPIO). `Desarrollo`=`c2058f7` sincronizado, árbol limpio. **§103 carga "app-like" DECIDIDA** (router falso-SPA + caché-memoria + prefetch + Blurhash; CERO SWR/prerender) — **implementación PENDIENTE faseada**. Daniel conserva banner-hero/gema; regla: `arquitecto-software` SIEMPRE al construir. **PRÓXIMO**: **implementar §103 (F1 Blurhash→F2 router)** · alinear cerebro cars · borrado-imagen · C1 · responsive · limpiar CSS muerto. **🔄 RESET-A-CERO** (Kary). ⚙️ OPUS 4.8 interino. 1er corte 1-jul. |
