...
| **PROD / branch** | **Web "app-like" §103-§113 CERRADO EN PROD ✅** (View Transitions §107 + caché SWR §108 + LQIP §111 + Cache-Control Storage §112 + correo OWNER §113), validado Daniel+Chrome. `Desarrollo`≈`main` (merge por PR de Daniel). **Commit exacto → `git fetch` (NO fijarlo aquí, §3.3/§114).** **🎯 Rol "catálogo" de Kary CERRADO EN PROD ✅** (§115, TODO-19/31a; Kary verificada EN VIVO — ve/maneja SOLO Piezas+Colecciones; 4 fixes L-55/56/57). **🎯 Visibilidad/SEO §116 EN PROD ✅** (SSG piezas/colecciones/journal indexables + JewelryStore/Maps schema + **GA4 ENCENDIDO** [estaba muerto] con Consent Mode v2; GA/GSC config por Chrome: retención 14m·GA4↔GSC·sitemap; **Google bajo `bersagliojewelry@gmail.com` authuser=3**; TAIL: A2b/eventos-clave/HUB-prompt). **🎯 TODO-34 ficha (§118) + TODO-36 grilla/recos/GA4 (§119) EN PROD ✅** (mergeados PR #356-358; piezas `zzz-prueba-*` ya limpias, `pieces` vacía). **🎯 PLAN MAESTRO DE COMERCIO (TODO-37) — B0+B0.5 EN PROD ✅** (§120, PR #359): WhatsApp directo en la ficha + GA4 `whatsapp_click` (verificado en vivo emulador+seed). **SIGUIENTE: B1 (corazón, MOSTRADOR)** — entidad `pedidos` por CF callable + stock atómico (candado=pieza) + caja/arqueo + bruto/neto (spec `2026-06-25-plan-maestro-comercio-v3.md §11`). Cola: TODO-33 panel app-like · A2b cuando haya contenido. `arquitecto-software` SIEMPRE. ⚙️ OPUS 4.8 interino. |
