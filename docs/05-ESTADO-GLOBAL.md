...
| **Branch activa** | `origin/main` (PROD) = `8d469a3` (**PR #318 mergeado**): §100-§109 + §108 F1 caché/F3 LQIP + **migración LQIP** (`scripts/migrate-lqip.mjs`) **EN PROD**. `Desarrollo` = `f6ff72e` (**pendiente PR de Daniel**): **§111 LQIP en `siteContent` vía SWR cache-first** (hero/editorial/nosotros; resuelve L-50 sin el 3er estado de §106). **§108: F1✅ F3✅ F2(siteContent)✅(§111); F2 piezas opcional.** Pendiente §110.1: **correr la migración** (bloqueada credencial bersaglio; ADC=altorra → Daniel autorizó guiarlo: `set-quota-project` o SA key; baja urgencia por reset). Relevo → `10`. Cola: cerebro cars · C1 · responsive · CSS muerto. `arquitecto-software` SIEMPRE. **🔄 RESET-A-CERO** (Kary). ⚙️ OPUS 4.8 interino. 1er corte 1-jul. |
