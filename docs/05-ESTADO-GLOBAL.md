...
| **Branch activa** | `git fetch` 2026-06-23: `origin/main` (PROD) = `288fe39` — §100-§105 EN PROD (cero-demo · carga fluida · F1 LQIP · fix recarga). `origin/main` (PROD) = `d44b22c` (**PR #311**): §106 revert + §107 (F2.0 View Transitions) + v27 **YA EN PROD**. `Desarrollo` = `f01df68` (pusheado): **+§108 F1 (caché inteligente público-only)** — listo para su PR (deploy seguro + baja costo lecturas). **§108: workflow ✅ (4 bloqueantes), F1 ✅ verificada local; F2/F3 GATED a CONTENIDO** (catálogo vacío por reset → no construir render a ciegas, L-45). Router F2.1 innecesario. Regla: `arquitecto-software` SIEMPRE. Cola: alinear cerebro cars · C1 · responsive · CSS muerto. **🔄 RESET-A-CERO** (Kary). ⚙️ OPUS 4.8 interino. 1er corte 1-jul. |
