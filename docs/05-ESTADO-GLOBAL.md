...
| **Cache version vigente** | `bersaglio-v16` (`public/sw.js`). Bump por acuerdos de pago (UI de cuotas gateada por `config/cartera.acuerdosActivos`). Indicador `APP_VERSION` `v16` en el sidebar (criterio de deploy). |
| **Branch activa** | `Desarrollo` ≈ `main` (deploys frecuentes; **antes de mergear: `git fetch` + `reset --hard origin/main`** — Daniel mergea en paralelo por GitHub Desktop, L-26). **🆕 CMS WEB PÚBLICA EN PROD** (§83+§84, validado Daniel): WYSIWYG (F1 preview fiel · F2 clic-editar · viewport 1440 escalado · sticky 100dvh) · P3.5 imagen · **P4 Nosotros** (listas + modelo PLANO 12 claves + imágenes) · F3 barandas ("Publicado ✓" + aviso al salir) · **`global` datos de contacto FUENTE ÚNICA** (footer+Contacto derivan; corrige WhatsApp falso). `firestore.rules`+`storage.rules` desplegadas. **`global` inc3+inc4 (`f757b25`§85 wa.me CTAs · `26bf8f8`§86 legales) → fuente única, en `Desarrollo` pendiente merge** (inc2 ya en main PR #265). **🔒 TREN M0→M6 EN PROD**; ACUERDOS R1-R5 gateados/inertes, falta R6 (§81). ⚙️ **OPUS 4.8 interino**. 1er corte = 1 jul. SW `v16`; `.co` 200. |
