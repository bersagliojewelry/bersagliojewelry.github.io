...
- 🟢 **PLAN ÚNICO ERP v4 (TODO-68**, spec `2026-07-04`) = roadmap. **F1·F2.0·F2.1·caja-tiempo-real·TODO-70 ✅ EN PROD (§165-§173)** (`config/caja enforceTurno:true` verificado-vivo:2026-07-08 · `config/identidad.activo=true` LIVE Kary; detalle→ADRs). **F2.2 multi-línea (TODO-41): CÓDIGO EN MAIN + BACKEND DESPLEGADO ✅** (PR#411 `153c48a`; deploy `functions,firestore:rules` 2026-07-08, 47 fn upd; f2-2 16/16·rules 242/242 verdes). **⚠️ NO OPERATIVO — falta**: (1) seed 1 servicio + (2) gate live (login owner Daniel: venta pieza+servicio→total→anular) · (3) ADR §176. Hasta el gate: **avisar Kary NO usar "+ Servicio"**. → 2.4 apartados. **GATE vivo: precios reales + 1ª APPROVED.**
