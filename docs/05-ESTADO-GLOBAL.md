...
| **Branch activa** | `Desarrollo` ≈ `main` (antes de mergear: `git fetch` + `reset --hard origin/main`; Daniel mergea por GitHub Desktop, L-26). **EN CURSO — TODO-28 correcciones web** (Daniel 2026-06-21): F1-F3 ✅ (FAQ · journal/entrada estado-cero · listener **live-sync robusto re-suscribe** · fotos Editorial+Atelier editables) + F4 `content-visibility` catálogo ✅, **resto en curso** (detalle → `10` TODO-28 + `45` PERF-04). Cerrados: TODO-25 CAZA-BUGS §90 · TODO-26 cars §G ✅. Daniel mergeando→main (`de20b0f`); **SW v21 sin bump** (JS/CSS hasheados + HTML network-first). **🔄 RESET-A-CERO** (Kary recarga de cero). Hist. cerrada (CMS §83-88 · acuerdos R6 §87 gated · tren M0→M6 prod) → `00`→`99`. ⚙️ OPUS 4.8 interino. 1er corte 1-jul. |
