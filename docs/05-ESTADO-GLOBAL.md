...
| **Cache version vigente** | `bersaglio-v97` (`public/sw.js`; SW↔05 OK; admin `APP_VERSION v54`). v97 = fix PII GA4 (§189) · v96 = Journal indexable (§187) · v95 = Journal CMS (reglas journalValid **DEPLOY MANUAL**). Historial → ADRs. |
...
- 🟢 **Roadmap = plan maestro v5** (spec `2026-07-10`). F1→F2.2 + caja tiempo real + POS-pro + **F-IA-2 ✅** EN PROD (§165-§192; `enforceTurno:true`/`identidad.activo` LIVE). SIGUE: **F-TESORERÍA (DISEÑADA — spec cerrada TODO-78, Opus ejecuta)** → F-COMPRAS → F-REPORTES → F2.4 apartados (TODO-39) → 2.3 térmica → limpieza → rompimiento → **lanzamiento (ahí: precios reales)**. 1ª APPROVED ✅.
- 🟣 **INTERINATO OPUS 4.8** · **#3 (F-IA-2) ✅ CERRADO Y AUDITADO** (§192: calidad de titular, 2 nits). **#4 ABIERTO 2026-07-18 → F-TESORERÍA**: spec cerrada por Fable con 3 capas de refutación (comité ×3 §0.6 + consejo externo §0.7 + directiva del dueño §0.8: **Kary carga sus cuentas por UI, cero seed**). Opus arranca **B0 sin gate externo**; protocolo R1-R7 · marcar `[OPUS-4.8]` · **zonas calientes R3: bóveda(V1)+caja(V17)** = test primero. **Gate Daniel: Kary NO usa hasta confiabilidad 100%.**
