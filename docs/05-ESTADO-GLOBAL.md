...
| **Cache version vigente** | `bersaglio-v75` (`public/sw.js`; SW↔05 OK; admin `APP_VERSION v38`). v75 = domicilio local cobrado (traza como flete nacional); v74 = UI F1-CORE Pedidos operable + POS requiere-envío; v73 = código público BJ-XXXX-XXXX (§166). |
...
- 🟢 **PLAN ÚNICO ERP v4 (TODO-68**, spec `2026-07-04-plan-unico-erp-v4`) = roadmap vigente: F1 Pedidos&Logística → F2 POS completo → F3 Inventario → carril D → F4-F6. **F1 COMPLETO ✅ (§165-§168)**. **F2.0 caja/bóveda: backend B0-B4 ✅ + B5a/B5b-0 ✅ + B5b-1 UI ✅** (build+`test:caja-format` 9/9 verdes; SIN deploy). **Sigue B5c**: SW bump v76 + DEPLOY MANUAL (functions/rules/indexes — caja aún NO desplegada) + sembrar `config/caja` + gate Chrome + ADR. 🚩 Brecha rol `caja` a medias (L-78, reportada). Pend no-gate: push A.6. **GATE vivo: precios reales + monitorear 1ª APPROVED.**
