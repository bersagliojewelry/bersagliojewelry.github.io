...
| **Branch activa** | `Desarrollo` (adelante de `main`: **M6 construido** + docs — falta PR). **🔒 TREN M0→M5 EN PROD (§79.8)** · **M6 acuerdo por deuda CONSTRUIDO** (UI v15, verif. experta 16 agentes — ADR §80; reglas SIN cambios). Vigilar: 1er corte real = 1 de julio 03:50 Bogotá (`cortes/2026-06`, respaldo `generarCorte`). **13 functions** nodejs22 vivas; `.co` HTTP 200. `git fetch` antes de afirmar (L-26). |
