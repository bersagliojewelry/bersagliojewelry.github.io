...
| **Cache version vigente** | `bersaglio-v33` (`public/sw.js`; verificado SW↔05). Último bump: **TODO-36 responsive inteligente** — grilla Flexbox que se acomoda a la cantidad (5→3+2, sin huérfana) en destacadas/catálogo/relacionados + recomendaciones por CONTENIDO (categoría=compuerta, fallback curado, título honesto, **sin relleno al azar**) + medición GA4 real (view_item/view_item_list/select_item + `source_piece_slug`). (v32 = ficha "Carta Gemológica".) Indicador `APP_VERSION` = criterio de deploy. |
| **PROD / branch** | **Web "app-like" §103-§113 CERRADO EN PROD ✅** (View Transitions §107 + caché SWR §108 + LQIP §111 + Cache-Control Storage §112 + correo OWNER §113), validado Daniel+Chrome. `Desarrollo`≈`main` (merge por PR de Daniel). **Commit exacto → `git fetch` (NO fijarlo aquí, §3.3/§114).** **🎯 Rol "catálogo" de Kary CERRADO EN PROD ✅** (§115, TODO-19/31a; Kary verificada EN VIVO — ve/maneja SOLO Piezas+Colecciones; 4 fixes L-55/56/57). **🎯 Visibilidad/SEO §116 EN PROD ✅** (SSG piezas/colecciones/journal indexables + JewelryStore/Maps schema + **GA4 ENCENDIDO** [estaba muerto] con Consent Mode v2; GA/GSC config por Chrome: retención 14m·GA4↔GSC·sitemap; **Google bajo `bersagliojewelry@gmail.com` authuser=3**; TAIL: A2b/eventos-clave/HUB-prompt). **SIGUIENTE**: panel admin "tipo app" (TODO-33, Decisión Fuerte, sesión nueva) · A2b cuando haya contenido. `arquitecto-software` SIEMPRE. ⚙️ OPUS 4.8 interino. |
