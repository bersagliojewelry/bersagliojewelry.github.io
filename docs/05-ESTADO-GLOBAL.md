...
| **Cache version vigente** | `bersaglio-v45` (`public/sw.js`; verificado SW↔05; admin `APP_VERSION v28`). Último bump: **§138 ajustes web** — index hasta **16 destacadas** (antes 9; ajuste 15→16 = 4 columnas exactas) + botón "Explorar catálogo" al final + isla "Atajos" SOLO en index + **etiqueta única "Precio a consultar"** (`priceDisplay` SSoT; elimina "Cotización"/"Consultar precio"). (v43 = significado/voz §134-§137.) `APP_VERSION` = criterio de deploy. |
