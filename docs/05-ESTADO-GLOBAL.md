...
| **Branch activa** | `Desarrollo` (al día con `main`, PR #237). **🔒 TREN M0→M6 EN PROD**. **ACUERDOS: v1 GATEADA/inerte (`acuerdosActivos` off) + SUPERSEDED por el Consejo → spec v2, REWORK R1-R6** (mutex `acuerdoVigenteId` · saldo-only · escudo · estadoAlCorte). ⚙️ **OPUS 4.8 interino** (Fable no disponible; marcar todo). 1er corte real = 1 jul. **13 functions** vivas; `.co` 200. `git fetch` (L-26). |
