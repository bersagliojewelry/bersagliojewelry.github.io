...
| **Cache version vigente** | `bersaglio-v24` (`public/sw.js`). Bump por §104 (§103 **F1 LQIP**: placeholder difuso "blur-up" en imágenes del CMS — editorial/hero/atelier/avatar). Indicador `APP_VERSION` en el sidebar = criterio de deploy. |
| **Branch activa** | `git fetch` 2026-06-23: `origin/main` = `d91152c` (Daniel mergeó TODO; **§100·§101·§102 EN PROD** verif. en vivo: cero-demo + carga fluida + móvil 340×425 + QA público LIMPIO). `Desarrollo` adelantado (§104 LQIP F1 + cierre §103), **pendiente merge de Daniel**. **§103 F1 (LQIP "blur-up") IMPLEMENTADA** (§104, cache v24; sin cambio de reglas — whitelist es por sección, L-48); **F2 router falso-SPA PENDIENTE**. Daniel conserva banner-hero/gema; regla: `arquitecto-software` SIEMPRE al construir. **PRÓXIMO**: **§103 F2 (router falso-SPA, comité #2 casos borde)** · alinear cerebro cars · borrado-imagen · C1 · responsive · limpiar CSS muerto. **🔄 RESET-A-CERO** (Kary). ⚙️ OPUS 4.8 interino. 1er corte 1-jul. |
