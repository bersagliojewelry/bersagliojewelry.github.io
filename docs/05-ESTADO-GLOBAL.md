...
| **Branch activa** | `Desarrollo` (**sin mergear: F6 App Check §54 + frente D §64** `c554aec`). Prod `main` (PR #199+) DESPLEGADO: rediseño + CRM + Fase R + Panel v2 + Morosos §51 + F5 §52 + F4 §53. **7 functions** nodejs22 vivas (PRE-1 cerrado §63; las 3 de §64 ENTRAN con el próximo deploy); `.co` HTTP 200. `git fetch` antes de afirmar (L-26). |
...
- 🟠 **F6 en curso**: App Check REPARADO (§58, canje 200 en vivo) — TODO-14: monitor ~100% ×7d → **Enforce** guiado (PROHIBIDO antes, L-32). Forms endurecidos ✅ (§59) · CI+entero-COP ✅ (§62) · PRE-1 backup+restore ✅ (§63) · **reconciliación+Salud ✅ construida (§64) — deploy rules+functions PENDIENTE de OK de Daniel** (bloqueado por clasificador; tests verdes). Resto F6: RBAC claims · paginación cursor. Luego: política cartera APROBADA → config en Fase M.
