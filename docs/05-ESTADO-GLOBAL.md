...
| **Cache version vigente** | `bersaglio-v12` (`public/sw.js`). Bump por Fase M / M3 forward-compat. Indicador `APP_VERSION` `v12` en el sidebar (criterio de deploy). |
| **Branch activa** | `Desarrollo` (≡ `main` #228 + cierre M3). **🔒 CANDADO M3 EN PROD Y CERRADO (§77.8)**: reglas desplegadas 2026-06-12 + smoke 4/4 en prod vía navegador + auditoría interna + limpieza verificada. **TREN M2a+M2b+M3 COMPLETO**. Vigilar: 1er día de Kary bajo v12 = smoke admin natural. Siguiente: **M4** (auditoría detectiva en Salud + corte mensual). **11 functions** nodejs22 vivas; `.co` HTTP 200. `git fetch` antes de afirmar (L-26). |
