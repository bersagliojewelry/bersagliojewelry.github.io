...
| **Cache version vigente** | `bersaglio-v11` (`public/sw.js`). Bump por Fase M / M2b. Indicador `APP_VERSION` `v11` en el sidebar (criterio de deploy). |
| **Branch activa** | `Desarrollo`. **M2b CONSTRUIDO Y VERIFICADO (§76)**: cola de aprobación de Daniel en Salud (re-validación §74, batches atómicos; 21/21 + 104/104 + verif. experta 16 agentes con 7 fixes). SoD owner = excepción consciente (decisión Daniel 2026-06-11). Falta: PR `Desarrollo→main` (mergea Daniel) + guion CON Daniel en prod. M2a EN PROD (§75, main hasta #224). Siguiente: M3 candado (plan ENMENDADO — regla COINCIDENCIA por tipo; Consejo Externo ANTES). **11 functions** nodejs22 vivas; `.co` HTTP 200. `git fetch` antes de afirmar (L-26). |
