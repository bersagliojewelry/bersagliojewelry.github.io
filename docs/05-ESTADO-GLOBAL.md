...
| **Branch activa** | `Desarrollo`. **M3 CONSTRUIDO Y RED-TEAMEADO (§77)**: candado en `firestore.rules` (LOCAL, AÚN SIN DESPLEGAR) + UI forward-compat v12; 133/133 reglas + 44/44 puros. ⚠️ **DEPLOY EN 2 ETAPAS PENDIENTE**: (1) PR `Desarrollo→main` (Daniel) → verificar v12 vivo; (2) SOLO entonces `firebase deploy --only firestore:rules` + smoke mismo día (§77.7). M2a/M2b EN PROD (§75/§76). **11 functions** nodejs22 vivas; `.co` HTTP 200. `git fetch` antes de afirmar (L-26). |
