...
| **Branch activa** | `origin/main` (PROD) = `dcb3787` (**PR #321**): §100-§111 + §108 F1/F3 + migración + **§111 SWR siteContent VALIDADO en prod** (2 opiniones: Daniel + extensión Chrome ✅; consola limpia, transiciones suaves, 2ª carga instantánea). `Desarrollo` = `2eb71fe` (**pendiente PR**): blur-up catálogo/wishlist/detalle (111.8) + **§112 Cache-Control Storage** (RCA: subían sin cacheControl → `private,max-age=0` → blur en CADA visita; fix `_upload` + backfill 18/18 EJECUTADO en prod, verificado curl). **§108: F1✅ F3✅ F2(siteContent)✅; §110.1 migración 8 LQIP + §112 cache-control EJECUTADAS** (SA key bersaglio). Relevo → `10`. Cola: cerebro cars · C1 · responsive · CSS muerto. `arquitecto-software` SIEMPRE. **🔄 RESET-A-CERO** (Kary). ⚙️ OPUS 4.8 interino. 1er corte 1-jul. |
