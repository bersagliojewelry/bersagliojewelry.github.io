...
| **Branch activa** | `origin/main` (PROD) = `dcb3787` (**PR #321**): §100-§111 + §108 F1/F3 + migración + **§111 SWR siteContent VALIDADO en prod** (2 opiniones: Daniel + extensión Chrome ✅; consola limpia, transiciones suaves, 2ª carga instantánea). `Desarrollo` = `f8f79ec` (**pendiente PR**): **blur-up en catálogo/wishlist/detalle** (111.8 — completa la cobertura que §108.14 dejó solo en el Home). **§108: F1✅ F3✅ F2(siteContent)✅; §110.1 migración EJECUTADA** (8 LQIP, SA key bersaglio). Relevo → `10`. Cola: cerebro cars · C1 · responsive · CSS muerto. `arquitecto-software` SIEMPRE. **🔄 RESET-A-CERO** (Kary). ⚙️ OPUS 4.8 interino. 1er corte 1-jul. |
