...
| **Cache version vigente** | `bersaglio-v98` (`public/sw.js`; SW↔05 OK; admin `APP_VERSION v55`). v98 = F-TESORERÍA bundle B2-B5 (**DESPLEGADO 2026-07-24**). Historial → ADRs. |
...
- 🟢 **Roadmap = plan maestro v5** (spec `2026-07-10`). F1→F2.2 + caja tiempo real + POS-pro + **F-IA-2 ✅** EN PROD (§165-§192; `enforceTurno:true`/`identidad.activo` LIVE). SIGUE: **F-TESORERÍA (TODO-78; **B0-B4 + B5(D6·V1·V18) ✅ EN PROD 2026-07-24** —9 CFs + reglas + índices + seed virtuales desplegados; verdes offline teso 31/31 · caja 38/38. ⚠️ **E2E de UI con login PENDIENTE** (se verificó el deploy por API, no la pantalla). SIGUE B5: **V17** + D9 → B6, ver `10`+spec §9)** → F-COMPRAS → F-REPORTES → F2.4 apartados (TODO-39) → 2.3 térmica → limpieza → rompimiento → **lanzamiento (ahí: precios reales)**. 1ª APPROVED ✅.
