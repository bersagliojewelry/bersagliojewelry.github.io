...
|---|---|
| **Build** | 🟢 Vite VERDE (✓ ~14s). Rediseño Fase 1 aplicado (shell + Home modular + Nosotros + Contacto; doctrina pulida §41). ⚠️ Preview headless local no pinta dinámico (L-05) — verificar en `npm run dev`/deploy. Panel v2 en prod (F-CHASIS-A §50 · Morosos §51 · F5 §52 · F4 §53); norte = spec maestra v3. **Frente D §64 construido en `Desarrollo`**: reconciliación+Salud (trigger blindado, `reconciliacionDiaria`, `admin-salud.html`). Tests: sidebar 7/7 · estado 15/15 · saldo 12/12 · reconciliación 9/9 · rules **64/64** · backup-codec 7/7. |
| **Cache version vigente** | `bersaglio-v9` (en `public/sw.js`). Bump por HOTFIX prod (restaurar fallback de llaves Firebase). |
| **Branch activa** | `Desarrollo` (**sin mergear: F6 App Check §54 + frente D §64** `c554aec`). Prod `main` (PR #199+) DESPLEGADO: rediseño + CRM + Fase R + Panel v2 + Morosos §51 + F5 §52 + F4 §53. **10 functions** nodejs22 vivas (PRE-1 §63 + frente D §64 DESPLEGADO 2026-06-10, verificado `functions:list`); `.co` HTTP 200. `git fetch` antes de afirmar (L-26). |
...
- 🟠 **F6 en curso**: App Check REPARADO (§58, canje 200 en vivo) — TODO-14: monitor ~100% ×7d → **Enforce** guiado (PROHIBIDO antes, L-32). Forms endurecidos ✅ (§59) · CI+entero-COP ✅ (§62) · PRE-1 backup+restore ✅ (§63) · **reconciliación+Salud ✅ DESPLEGADA (§64)** — reglas+10 functions vivas; vista Salud a prod con el merge del PR; 1ª corrida 3:30 AM. Resto F6: RBAC claims · paginación cursor. Luego: política cartera APROBADA → config en Fase M.
