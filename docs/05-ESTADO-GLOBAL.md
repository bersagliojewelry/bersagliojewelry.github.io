...
| **Cache version vigente** | `bersaglio-v26` (`public/sw.js`). Bump por §106 (REVERT del render LQIP del §104: generaba 3er estado neutro→borroso→nítido; vuelve a neutro→foto; datos LQIP conservados para F2). v25=§105 recarga · v24=§104 F1. Indicador `APP_VERSION` = criterio de deploy. |
| **Branch activa** | `git fetch` 2026-06-23: `origin/main` = `d91152c` (Daniel mergeó TODO; **§100·§101·§102 EN PROD** verif. en vivo: cero-demo + carga fluida + móvil 340×425 + QA público LIMPIO). `origin/main` (PROD) = **§104 F1 LQIP EN PROD** (PR #306, `git fetch` 2026-06-23). `Desarrollo` con **§105 (fix recarga reveal, cache v25)** desplegando. **§103 F2 (navegación instantánea / router falso-SPA) = PENDIENTE → sesión NUEVA dedicada (TODO-30)**: es lo que elimina el **parpadeo entre páginas** que reportó Daniel (no lo arregla F1). **⚠️ F1 LQIP solo cubre imágenes subidas DESPUÉS** (existentes: re-subir o backfill+CORS). Regla: `arquitecto-software` SIEMPRE. **PRÓXIMO**: F2 · backfill/re-subir imágenes · alinear cerebro cars · C1 · responsive · CSS muerto. **🔄 RESET-A-CERO** (Kary). ⚙️ OPUS 4.8 interino. 1er corte 1-jul. |
