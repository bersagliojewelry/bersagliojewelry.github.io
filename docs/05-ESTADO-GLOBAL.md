...
- 🟢 **PLAN ÚNICO ERP v4 (TODO-68**, spec `2026-07-04`) = roadmap. **F1·F2.0·F2.1·caja-tiempo-real·TODO-70 ✅ EN PROD (§165-§173)** (`config/caja enforceTurno:true` verificado-vivo:2026-07-08 · `config/identidad.activo=true` LIVE Kary; detalle→ADRs). **F2.2 multi-línea ✅ OPERATIVO EN PROD (§176, TODO-41 cerrado)** (PR#411 + deploy `functions,firestore:rules` 2026-07-08; gate vivo Chrome verde: venta pieza+servicio $150k server-side→anular→arqueo $0). Falta uso real: **cargar servicios reales con precios** (data Kary/Daniel, no dev). → **F2.4 apartados (TODO-39)**. **GATE vivo de negocio: precios reales + 1ª APPROVED.**
