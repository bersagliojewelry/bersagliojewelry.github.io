...
| **Cache version vigente** | `bersaglio-v36` (`public/sw.js`; verificado SW↔05; admin `APP_VERSION v21`). Último bump: **B1 paso 2 — calculadora de precio por peso** (§124): modal en la topbar de Piezas; Kary ingresa valor del gramo (VARÍA, no se guarda) + peso + mano de obra → total = peso×gramo+mano. (v35 = form flotante+9 destacadas+orden contacto §123; v34 = WhatsApp directo §120.) `APP_VERSION` = criterio de deploy. |
| **PROD / branch** | **Web "app-like" §103-§113 CERRADO EN PROD ✅** (View Transitions §107 + caché SWR §108 + LQIP §111 + Cache-Control Storage §112 + correo OWNER §113), validado Daniel+Chrome. `Desarrollo`≈`main` (merge por PR de Daniel). **Commit exacto → `git fetch` (NO fijarlo aquí, §3.3/§114).** **🎯 Rol "catálogo" de Kary CERRADO EN PROD ✅** (§115, TODO-19/31a; Kary verificada EN VIVO — ve/maneja SOLO Piezas+Colecciones; 4 fixes L-55/56/57). **🎯 Visibilidad/SEO §116 EN PROD ✅** (SSG piezas/colecciones/journal indexables + JewelryStore/Maps schema + **GA4 ENCENDIDO** [estaba muerto] con Consent Mode v2; GA/GSC config por Chrome: retención 14m·GA4↔GSC·sitemap; **Google bajo `bersagliojewelry@gmail.com` authuser=3**; TAIL: A2b/eventos-clave/HUB-prompt). **🎯 TODO-34 ficha (§118) + TODO-36 grilla/recos/GA4 (§119) EN PROD ✅** (mergeados PR #356-358; piezas `zzz-prueba-*` ya limpias, `pieces` vacía). **🎯 PLAN MAESTRO DE COMERCIO (TODO-37) — B1 mostrador EN CONSTRUCCIÓN**: EN MAIN (≤PR #363, v36) B0.5 WhatsApp §120 + inventario §122 + calculadora §124 + 3 correcciones §123. **Paso 3 backend §125** = `crearPedido` CF (candado atómico=pieza, recompute server-side, snapshot inmutable, idempotente; 206 rules + 6 integración; **reglas DESPLEGADAS**; CF en `Desarrollo` SIN desplegar/mergear). **SIGUIENTE: POS UI** (Kary registra venta→llama CF) + deploy CF + verif. en vivo (spec `2026-06-25-b1-mostrador-design.md`). Cola: TODO-33 · A2b cuando haya contenido. `arquitecto-software` SIEMPRE. ⚙️ OPUS 4.8 interino. |
