...
- 🟠 **F6 en curso**: App Check REPARADO (§58, canje 200 en vivo) — TODO-14: monitor ~100% ×7d → **Enforce** guiado (PROHIBIDO antes, L-32). Forms endurecidos ✅ (§59) · CI+entero-COP ✅ (§62) · PRE-1 backup+restore ✅ (§63) · **reconciliación+Salud ✅ DESPLEGADA (§64)** (10 functions; §64 mergeado a `main` PR #214). **RBAC claims ✅ construido (§65, `3e4d4e2`) — ⏳ deploy PENDIENTE de OK de Daniel** (orden: functions→backfill→rules→preflight, §65.7; el backfill toca cuentas → autorización explícita). **Fix seg §66 (`e9ecc37`)**: "Desactivar" ahora deshabilita Auth (CF) + `active` check + reglas `users/` owner-only — el fix de ACCESO viaja con el merge del PR (sitio) y funciona en prod ya (la CF está viva); las reglas se apilan al deploy pendiente del §65. Resto F6: paginación cursor. Luego: política cartera APROBADA → config en Fase M.
