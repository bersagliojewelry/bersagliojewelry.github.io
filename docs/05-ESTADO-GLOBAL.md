...
| **Cache version vigente** | `bersaglio-v10` (en `public/sw.js`). Bump por Fase M / M2a (UI de corrección de Kary). Indicador `APP_VERSION` en el sidebar = `v10 · 2026-06-10` (Kary confirma el deploy). |
| **Branch activa** | `Desarrollo` (UI M2a por delante de `main`). **Reglas M1 §72 + M2a-1b §73 DESPLEGADAS**; **UI de M2a (corrección de Kary) VERIFICADA por experto (§75, 12 agentes → 3 bugs de dinero corregidos, 0 bloqueantes)** + cache `v10` → go-live por merge a `main`. Prod `main` (hasta #221) DESPLEGADO: rediseño + CRM + Fase R + Panel v2 + Morosos §51 + F5 §52 + F4 §53 + frente D §64. **11 functions** nodejs22 vivas (§64 + §65 DESPLEGADOS, verificado `functions:list`); `.co` HTTP 200. `git fetch` antes de afirmar (L-26). |
