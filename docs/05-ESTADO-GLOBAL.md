...
| **Branch activa** | `origin/main` (PROD): §100-§107 + **§108 F1 (caché) + F3 (LQIP) desplegados** (PRs hasta #316). `Desarrollo` = `fbe5379` (**pendiente PR de Daniel**): umbral comercial (Destacadas ≥1) + fix pieza-sin-precio + fix VT §109 + **migración LQIP** (`scripts/migrate-lqip.mjs` §110 — falta credencial de bersaglio para CORRER; ADC=altorra). **§108: F1✅ F3✅; F2 opcional** (no parpadea en práctica). Pendientes §110: **correr la migración** + extender LQIP a `siteContent` (nosotros/hero). Relevo completo → `10`. Cola: cerebro cars · C1 · responsive · CSS muerto. `arquitecto-software` SIEMPRE. **🔄 RESET-A-CERO** (Kary). ⚙️ OPUS 4.8 interino. 1er corte 1-jul. |
