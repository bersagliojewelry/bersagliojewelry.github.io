...
| **Branch activa** | `Desarrollo` (al día con `main` — PR #234 mergeado). **🔒 TREN M0→M6 EN PROD (§80.8: v15 + acuerdo por deuda vivos por fetch)**. EN CURSO: diseño plan de cuotas/acuerdos (comité; directiva asesores `50 §5`). Vigilar: 1er corte real = 1 de julio 03:50 Bogotá (`cortes/2026-06`, respaldo `generarCorte`). **13 functions** nodejs22 vivas; `.co` HTTP 200. `git fetch` antes de afirmar (L-26). |
