...
| **PROD / branch** | `Desarrollo`≈`main` (**merge a main = Claude**, autoriz. `[[feedback_claude_deploy_autorizado]]`; **commit exacto → `git fetch`**, §114). **EN PROD (main)** hasta §177 (Mostrador+caja OBLIGATORIA · F2.2 servicios OPERATIVO §176 · **Grupo A UX §177** · 32 piezas v3 · legal e-commerce). **⚠️ `Desarrollo` ADELANTE de `main`**: TODO-73 core money-model (`functions` **SIN desplegar**, backward-compat) — **NO desplegar hasta UI+gate**. **Pend operativo Daniel**: precios (⛔ GATE) + imágenes IA. **MCP Firebase = escritura prod**. `arquitecto-software` SIEMPRE. |
...
- 🟢 **PLAN ÚNICO ERP v4 (TODO-68**, spec `2026-07-04`) = roadmap. **F1·F2.0·F2.1·caja-tiempo-real·TODO-70 ✅ EN PROD (§165-§173)** (`config/caja enforceTurno:true` verificado-vivo:2026-07-08 · `config/identidad.activo=true` LIVE Kary; detalle→ADRs). **F2.2 ✅ OPERATIVO (§176)** (falta cargar servicios reales, data). **Grupo A UX ✅ EN PROD (§177)** (zoom·catálogo·caja-cerrada; lección validación holística `[[feedback_validacion_chrome_holistica]]`). **🟡 TODO-73 POS-pro money-model (Decisión Fuerte): CORE ✅ EN TDD** (datáfono·`pagos[]` split-tender·venta-sin-pieza·vouchers al cierre; `d6a5728` en `Desarrollo`, SIN deploy; spec+comité§8+decisiones§9). **UI pend. — Daniel trae AJUSTES en sesión nueva** → luego deploy `functions`+gate holístico+ADR §178. SIGUE: F2.4 (TODO-39). **GATE negocio: precios reales + 1ª APPROVED.**
