...
| **Cache version vigente** | `bersaglio-v91` (`public/sw.js`; SW↔05 OK; admin `APP_VERSION v51`). v91 = F-IA-2 B3 ("Hoy" gana pulso $ read-only; fuera Consultas recientes). v90 = B2 (Clientes ⇄ Cartera). v89 = B1 pestañas. v88 = rail v2 (§182). Historial → ADRs. |
...
- 🟣 **INTERINATO #3 · OPUS 4.8** (2026-07-10). Protocolo `opus-interino-protocolo` VINCULANTE (R1-R7). **SOLO F-IA-2** (spec `2026-07-10-f-ia-2-DISENO.md`): **B0·B1·B2·B3 ✅ (SW v91); §0.7 D1-D7 cerradas por Fable → sigue B4(Bandeja)·B5(Salud+microcopy) SIN preguntar** (CERO functions/dinero nuevo). B3 = "Hoy" con pulso $ (5 señales read-only) gateado por rol; dedup money-safe `ventasEfectivoTurno`→`caja-format` (POS y Hoy comparten fórmula). ⏳ Pend: validar B1-B3 funcional-con-sesión (guardar/roles/números vivos) en deploy. Cola TITULAR: auditar B1-B5 + F-TESORERIA/COMPRAS/REPORTES + apartados (§6). Norte: plan v5. **Gate Daniel: Kary NO usa hasta confiabilidad 100%.** 1ª APPROVED ✅; precios = paso final.
