...
| **Cache version vigente** | `bersaglio-v14` (`public/sw.js`). Bump por Fase M / M5 (gestiones de cobro en la ficha). Indicador `APP_VERSION` `v14` en el sidebar (criterio de deploy). |
| **Branch activa** | `Desarrollo` (adelante de `main`: **M5 construido** + docs — falta PR). **🔒 TREN M0→M4 EN PROD (§78.8)** · **M5 gestiones de cobro CONSTRUIDO** (UI v14, verif. experta 13 agentes — ADR §79; reglas vivas desde M1, sin cambios). Vigilar: 1er corte real = 1 de julio 03:50 Bogotá (`cortes/2026-06`, respaldo `generarCorte`). **13 functions** nodejs22 vivas; `.co` HTTP 200. `git fetch` antes de afirmar (L-26). |
