...
| **Branch activa** | `Desarrollo` (F5/F4 mergeados a `main`; **F6 App Check §54 sin mergear**). Prod `main` (PR #199+) DESPLEGADO: rediseño + CRM + Fase R + Panel v2 + PRE-integridad + Morosos §51 + **F5 filtros §52 + F4 Bandeja §53**. **7 functions** nodejs22 vivas (+`backupDiario` §60; **PRE-1 CERRADO §63: 1ª corrida OK 702 docs + restore PROBADO en gemelo + copia fuera**); `.co` HTTP 200. `git fetch` antes de afirmar (L-26). |
