...
| **Branch activa** | `Desarrollo` local = `cbfea97`. **`origin/main` = `cee5a85`** (`git fetch` 2026-06-22; Daniel mergea durante la sesión, L-26 — `origin/main` AVANZA solo). **YA EN MAIN**: §93 perf, **§94 fuentes VF** (`f926eca`), **§95 F5 borrado-colección** (`2de1397`). **PENDIENTE push+merge de Daniel**: **§96 shard `30`→`31-LECCIONES-FIRESTORE`** (`cbfea97`, +1 sobre `origin/Desarrollo`=`81b7a4c`). **SW v21 sin bump** (JS/CSS hasheados + HTML network-first). Resta: arranque C1 · F2-CRM colas mudas. **🔄 RESET-A-CERO** (Kary recarga de cero). ⚙️ OPUS 4.8 interino. 1er corte 1-jul. |
