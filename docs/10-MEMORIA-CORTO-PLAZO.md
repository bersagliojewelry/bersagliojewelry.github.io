...
> 2026-07-07 (Opus · **F2.1 vínculo cliente EN CURSO** · SSoT spec `2026-07-07-f2-1-vinculo-cliente-DISENO.md`, 2 crudos comité en bóveda): Decisión Fuerte contrato de identidad. **BACKEND EN `Desarrollo`** (`fa8533d`/`4a5d223`): core+5 CFs+reglas `clientesPorDoc` deny-all, **272 tests** (22+14+236). **UI por FLUJO COMPLETO** (directiva Daniel: dinero/facturación/CRM → comité+consejo+skills también en la IMPLEMENTACIÓN → `[[feedback_flujo_completo_nunca_parcial]]`): mockup + comité ×3 UI + consejo + **spec §9**. Fundación probada: transporte 5 callables + `advisory-match.js` (6 tests) + `selloConsent` ampliado. 🔴 clave: `doRegister` reset PRIMERO + panel try/catch (o venta perdida por idempotencia); `resolverCliente` admin-only (nunca checkout público). **RESTAN: cablear `pos.js`+`pedidos.js` + deploy(dark→pepper `IDENTIDAD_PEPPER`→flag `config/identidad.activo`→gate Chrome) + ADR §171.** Fix: `test:rules` respeta `FIRESTORE_EMULATOR_HOST` (8080 ocupado por server ALTORRA). Tag `[OPUS-4.8]`.
