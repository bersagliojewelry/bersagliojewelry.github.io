...
>
> **▶️ RETOMAR AQUÍ — construir ACUERDOS DE PAGO (spec sellada + decisiones tomadas)**: spec `2026-06-12-acuerdos-de-pago-design.md` con **P1=A · P2=A · P3=B/24meses RESPONDIDAS** (§3). Orden: slice 1 (fórmula TRAMOS inline en `crm-estado-cuenta` + copia byte + matriz de tests — INERTE, sin riesgo) → slice 2 reglas (idealmente tras integrar el Consejo Externo: prompt listo en bóveda `…consejo-externo-acuerdos-prompt.md`, Daniel lo pega en Gemini cuando pueda) → 3 corte → 4 UI → 5 detectores. **Cadencia nueva (directiva Daniel 2026-06-12, memoria `feedback_verificacion_por_hitos`)**: tests+build por commit SIEMPRE; verificación experta multi-agente UNA VEZ al cerrar la feature (post-slice 4/5), no por slice; prueba completa de plataforma cuando haya bloque sólido. ADR §81 al cerrar la feature.
