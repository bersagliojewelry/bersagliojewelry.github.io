...
| TODO-62 | **Pinch-zoom móvil iOS — pantallazos + CRASH + (regresión §156.17) carga lenta.** §156.17 REVERTIDO `bf9db59`. **§156.18 DESPLEGADO** (`home.css` `@media ≤680px` + SW v57): RCA = MEMORIA acumulativa → `content-visibility` en secciones estáticas (editorial/services/atelier, NO las dinámicas) + quitar `filter:blur()` decorativos; NUNCA promover GPU (causó la carga lenta). Verdes; reglas verif. en dist. **PEND: confirmar iPhone Daniel** (sin crash + carga rápida). → L-62 / §156.18. | 🟡 | confirmar iPhone |
...
>
> **2026-06-30 (§156.17 · TODO-62 — pinch-zoom iOS, DESPLEGADO):** pantallazos blancos al regresar del zoom = re-raster de ~36 reglas `backdrop-filter` (Safari 18: bg+backdrop-filter→blanco); §156.13/.14 cubrieron sólo 3 (whack-a-mole). FIX en `css/liquid-glass.css` (`@media ≤680px`): muro `backdrop-filter:none !important` + `--glass-tint`→.88 + `.bj-world translateZ(0)`. Desktop intacto; build/tests/brain verdes; sin bump. **Falta: confirmar en iPhone de Daniel.** → L-62 / ADR §156.17.
> **2026-06-29 (§156.1–.16 "primera cara" — DESPLEGADO):** hero inmersivo + tops/buscador compactos + "Asesoría privada" + cookie PC + hero móvil iOS. Detalle → ADR §156.x + bóveda. **Pend Daniel**: confirmaciones iOS finales + TODO "Hablar con el Atelier" en ficha.
