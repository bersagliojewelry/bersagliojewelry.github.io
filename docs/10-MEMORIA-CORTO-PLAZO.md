...
| TODO-63 | **Rediseño UX checkout** — diseñado (comité×4+Antigravity; spec `2026-06-30-checkout-redesign-design`) + **gate ✅** (GD-1 Diana=socia · GD-2 link Wompi/transferencia al NEGOCIO · GD-3 a-medida sin retracto). **EN CONSTRUCCIÓN**: cimientos `countries`/`envio-config` (12/12 tests) + paso **Entrega** rediseñado (radiogroup tipo · país bandera+indicativo · legal_id · pickup-con-dirección · consentimiento al final · elegibilidad por config) ✅ verif. en Chrome — commit `8d9f429` **NO desplegado**. **PEND antes de deploy**: ocultar-otros-métodos+escape · Wompi **redirect**+sessionStorage · pantalla de éxito · toques legales Términos GD-1/2/3 · deploy + validación Chrome (`[[feedback_validacion_chrome_directa]]`). Spec §7. | 🟡 | terminar redise + deploy |
