...
>
> **▶️ RETOMAR AQUÍ — ACUERDOS DE PAGO: solo falta R6 (deploy + encender + verificar)**. El build R1-R5 (diseño v2 del Consejo) está **CONSOLIDADO en ADR §81 + spec v2 (checklist R1-R5 ✅)** y MERGEADO a `main` (PR #239), pero **GATEADO/INERTE** (`config/cartera.acuerdosActivos` OFF → nada visible cambió en prod). Mutex `acuerdoVigenteId` (cierra jineteo) · solo saldo · escudo de 2 estados · `acuerdoAlCorte` cristalizado. Verde: suites puras + emulador 144/144 + build. **R6 (próxima sesión)**: (1) `firebase deploy --only firestore:rules,firestore:indexes,functions` (deploy mío, L-22); (2) encender en `config/cartera`: `acuerdosActivos=true` + `horizonteAcuerdoDias=730` + `acuerdoIncumplidoCuotas=2`; (3) **verificación POR HITO** (comité de agentes + prueba en vivo de la feature completa vía navegador de Daniel) + CRUDOs; (4) marcar cierre §81.9. ⚙️ R1-R5 = OPUS 4.8 interino (revisar al volver Fable). Detalle completo → ADR §81 (`00-INDICE`→`99`).
