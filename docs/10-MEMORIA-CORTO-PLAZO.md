...
> - **Consejo Externo ✅** (spec v3, §16) · **F-CHASIS-A ✅ construido** (rama `feat/panel-v2-f-chasis-a`, 6 commits, sin merge). **▶️ AHORA**: (1) **merge `feat→Desarrollo`** (decisión de Daniel) → desplegar; (2) **F1 — dominio `estadoCuenta`** (siguiente fase del roadmap). Pendiente brain al merge/deploy: ADR del diseño+build en `99` + fila en `00-INDICE` + `50-ARQUITECTURA` + `20-ESPACIAL` (admin nav v2).
...
| TODO-12 | **F-CHASIS-A construido** (rama `feat/panel-v2-f-chasis-a`, 6 commits `91f9105`..`7c2ab91`): renderSidebar+datos+tests · rail en vivo en 8 HTML · CSS grupos/adm-money+fix desborde · saldo por tokens (sin hex) · #confirm-dialog (cuenta+config) · revisión final aplicada. 6/6 tests + build verde + render verificado por snapshot. | ✅ Hecho (sin merge) | Merge `feat→Desarrollo` = decisión de Daniel |
