...
> **(1) TODO-35 · VISIBILIDAD SITE-WIDE** — **🟢 Fases A1 (piezas indexables) + B (marca/Maps) HECHAS en `Desarrollo`, SIN desplegar** (detalle en la fila TODO-35). B usó datos reales de Daniel (NAP+IG/FB/TikTok) y corrigió los links rotos de IG/FB en vivo. Catálogo prod VACÍO (Kary recarga) → 0 piezas horneadas hoy (correcto). **Falta**: A2 colecciones+journal · C GA4(`analytics.js`)+GSC · desplegar (⚠️ el merge a main BUNDLEA el TODO-34 pendiente de Daniel) · prompt Altorra HUB. Spec/research → bóveda `2026-06-25-*`.
...
| TODO-35 | **Visibilidad SITE-WIDE** (SEO·AEO·GA4·GSC·Maps·IA → #1; vía SSG portado de Altorra=HUB; spec/research→bóveda `2026-06-25-*`). **🟢 A1 (piezas) HECHA en `Desarrollo`**: generador `scripts/generate-pieces.mjs` (`/pieza/<slug>.html` Product+Breadcrumb+OG+noscript+PRERENDERED; guards selftest+bake-integrity; `noindex→index`) + `js/core/urls.js` SSoT + 9 callsites→URL limpia + `404.html` fallback + `robots.txt` bots-IA + cron diario. **🟢 FASE B (marca/Maps) HECHA en `Desarrollo`** (2026-06-25, datos reales de Daniel): `tenant_config.json` (NAP/sameAs SSoT, vertical JewelryStore) → JewelryStore+WebSite JSON-LD horneado en `dist/index.html` (verificado: parsea; NAP+sameAs reales + horarios 8–19h ×7; geo OMITIDO cero-demo) + `seller` en cada pieza · **FIX bug EN VIVO**: IG `@bersaglio_jewelry` + FB `profile.php?id=61583793442626` reales en `global-defaults.js` (estaban ROTOS: `@bersagliojewelry`/vanity) + TikTok en redes/footer + dirección/horario. **Falta**: A2 (colecciones+journal) · C (`js/analytics.js` GA4 G-F0CEWY7SP1 Consent Mode v2 + GSC sitemap) · desplegar (⚠️ bundlea TODO-34) · prompt HUB. ⚠️ App Check Enforce (TODO-14) bloquearía `generate` anónimo → `FIREBASE_SA_KEY` (ya soportado). | 🟡 | A2 + C + deploy |
