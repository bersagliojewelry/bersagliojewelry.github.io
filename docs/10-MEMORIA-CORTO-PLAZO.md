...
| TODO-41 | **Facturación multi-línea = F2.2 del plan v4** (Daniel 2026-06-26): factura/POS cobra pieza + servicios/modificaciones. **Decisión Daniel 2026-07-07: "AMBOS"** = catálogo de servicios (código+precio fijo, Kary gestiona) + **línea libre** (concepto+precio a mano) para trabajos únicos. **Decisión Fuerte** (toca `crearPedido`=motor de cobro + modelo `servicios` + UI POS multi-línea) → flujo COMPLETO (spec+comité+consejo+CF TDD estricto+gate live). Spec DISEÑO ✅ + **comité ×3 ✅** (CRUDO en bóveda `2026-07-07-comite-f2-2-multilinea-CRUDO.md`; síntesis vinculante = spec §8). Costura `items[]` ya sembrada (`pedidos-core.js:244`). A/B/C resueltas (A=owner-write acotado+soft-delete+audit · B=triple cap+umbral+configurable · C=servicio-sin-pieza→F2.3). **D FISCAL ✅ RESUELTO** (Claude-contador sobre RUT real: titular **No responsable de IVA cód.49** → NO cobra IVA en joya NI servicio → sin lógica fiscal por línea; `naturaleza` como future-proof; export gana columna `naturaleza`; detalle `42-LEGAL §7`). Fix seguridad CSV ✅ (SW v82). **Sigue = BUILD**: `servicios`(colección+reglas+CMS owner) · `crearPedido lineasExtra`(TDD estricto) · POS multi-línea (colapsado+chips+línea libre blindada) · export `naturaleza` · gate. | 🟢 | build F2.2 |
