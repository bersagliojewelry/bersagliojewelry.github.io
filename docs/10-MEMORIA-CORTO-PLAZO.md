...
> · **BUILD 2a** (Desarrollo; main atrás): ✅ P1 candado (POS 19/19) · ✅ P2 cripto (11/11) · ✅ P3 `iniciarPagoWeb` (público, 10/10) · ✅ P4 webhook `confirmarPagoWompi` (firma+re-consulta+idempotencia `webhookEvents`, 8/8). **SIGUIENTE**: reaper `liberarReservasVencidas` (Scheduler, re-consulta antes de liberar) + índice `pedidos(estado,reservaExpira)` → reglas(`pago_pendiente`+`webhookEvents` CF-only) → front(flag)+widget → deploy sandbox+secretos (Eventos/privada/integridad/pública). Tests ROJO 1º · App Check=2c · legal=cobro REAL.
