...
| TODO-49 | **Legal e-commerce** (prereq Wompi). **§157 HECHO+DESPLEGADO**: retracto Art.47/reversión Art.51/garantía legal/SIC + privacidad (consultas10/reclamos15, retención, Encargado) — válido hoy. Identidad RESUELTA (bóveda `LEGALES-kary`; titular Wompi=Kary=la comerciante). **RESTA** (detalle→`42-LEGAL §4/§5`): decisión Daniel publicar identidad (Art.50) · **IVA §02** (LEGAL-09, es No responsable de IVA) · aviso "cargo a nombre de…" · §03 rewrite · prueba consentimiento · abogado · 2 prompts consejo (Antigravity+ChatGPT) listos. `LEGALES/` raíz blindada en `.gitignore`. | 🟡 | decisión Daniel + Wompi |
...
>
> **2026-06-30 (§157 — legal pre-Wompi, DESPLEGADO):** Decisión Fuerte (skill `legal-colombia` + fuentes `.gov.co` + comité ×3 acotado). `terminos.js`/`privacidad.js`: retracto Art.47, reversión Art.51, garantía legal, enlace SIC, consultas10/reclamos15, retención, Encargado. Verificado e2e; sin cache bump; crudo→bóveda. **Resta (LEGAL-08, Kary)**: NIT+titular cuenta · aviso vendedor≠cobrador · §03 rewrite · prueba consentimiento. → ADR §157 / `42-LEGAL`.
