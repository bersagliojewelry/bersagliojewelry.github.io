...
>
> **🔨 WOMPI WEB = OPERATIVO Y VERIFICADO EN PROD (§164, A.9 ejecutado, TODO-42 ✅)**. **Próximo foco = OPERATIVO: cargar precios reales (Daniel/Kary) — cada pieza con precio se vuelve comprable — y monitorear la 1ª venta APPROVED** (pedido→pagado + alerta venta-web-pagada). Luego **Bloque D restante** (7b-7d/D.5/D.6, prerrequisitos: settings/gems prod + PAT GitHub). SSoT → plan Fable + spec Wompi. `[[project_comercio_pagos]]`
...
> ✅ **Cerrados** (detalle → ADRs/`99` + lecciones): **TODO-42 (encendido A.9 EJECUTADO §164 — Wompi web verificado en prod con $0; falta solo cargar precios reales + monitorear 1ª APPROVED)** · **TODO-66 (shard de `30` → hija `34-LECCIONES-META`, con §161)** · **TODO-64 (login parpadeos §159 → L-66, DESPLEGADO+verif live; confirmación natural: próximo login de Kary)** · **TODO-21 (revisión [OPUS-4.8] ejecutada como auditoría §158)** · TODO-62 (pinch-zoom iOS §156.18 → L-62) · TODO-61 (§156 primera cara + "Asesoría privada" §156.1-16) · TODO-60/59/58/56/55/54/53/52/51/46/45/44 · TODO-40+32. Pend Daniel: precios + fotos IA.
