...
> - **M2a EN CURSO** (slice más pesado; diseño+chunks → bóveda `fase-m-m2a-design.md` + tareas #1-#7). HECHO: contrato puro del gate (`js/crm-correccion.js`, 16/16) + capa de datos solicitudes/gestiones + **par atómico `corregirMovimientoBatch`** (`crm-service.js`) + **M2a-1b DESPLEGADO** (ensanche aditivo de `anulacionValida` §73; red-team W-01 0 bloqueantes; reglas 100/100). **UI: M2a-4 COMPLETO** (`admin-cuenta.html`/`cuenta.js`/`crm-service.js`, build verde): modal de anular con `motivoCategoria` obligatoria (cierra forward-risk §73) + `medioPago` en abono + **rediseño "Corregir saldo"** (motivo de lista + anuncio en pesos en vivo vía `planearCorreccionSaldo` + ruteo ajuste-directo/solicitud, sin toast optimista, error humano). **Siguiente UI**: (a) **M2a-3 botón "Corregir" por movimiento** — BLOQUEADO por decisión de contrato M2b (qué guarda la solicitud de corrección: `monto`=delta vs valor nuevo; forma de `datosCorreccion`) → pinear con el diseño de M2b · (b) M2a-5 corrección pendiente en ficha + bloqueo duplicados + RECHAZADA + re-solicitar · (c) M2a-6 versión en footer + cache bump SW + **smoke con Kary en prod** (gate de verificación del slice — NECESITA a Kary). ⚠️ La UI de dinero NO se verifica de verdad hasta el smoke de Kary (L-05: preview headless no sirve) → todo lo construido está build-verde pero sin probar en vivo.
