...
>
> **▶️ RETOMAR AQUÍ — PANEL v2: F-CHASIS-A desplegado + PRE-integridad lista; SIGUIENTE = función de MOROSOS/VENCIDOS** (2026-06-07, **ADR §50**):
> Panel v2 = **mini-ERP** (CRM/leads + cartera + facturación + inventario + pagos + trazabilidad), IA "C", construido por fases. **Norte**: spec `docs/superpowers/specs/2026-06-07-bersaglio-arquitectura-maestra-design.md` (v3, incluye Consejo Externo §16: saldo síncrono, sin backfill, O(M), DIAN por Adapter). **F-CHASIS-A** (rail como dato + `adm-money` + `#confirm-dialog`) ✅ construido por subagentes y **desplegado a prod** (`main` PR #195 `2fef1fe`; `.co` HTTP 200; rail v2 vivo).
> - **▶️ SIGUIENTE (sesión nueva) = función de MOROSOS/VENCIDOS** (fusiona F1+F2 + slice de F5; la mitad "centralizar color/etiqueta" YA la hizo F-CHASIS-A en `saldo-format.js`). Construir: (1) `fecha` real en cada movimiento nuevo (Kary la elige al registrar); (2) dominio `estadoCuenta` (helper puro espejo de `functions/saldo.js`); (3) consumir `config/negocio.diasPlazo`; (4) marcar VENCIDO + rangos de antigüedad; (5) vista CxC: vencidos en rojo, orden por mora, KPI de cartera vencida. Flujo spec→plan→build (subagentes + doble revisión; **verificar el repo tras CADA subagente — L-27**).
>   **DECISIONES tomadas (2026-06-07, en cristiano):** plazo **30 días** (estándar net-30; configurable en `config`; sin por-cliente por ahora) · el **Excel NO trae fecha** (confirmado en `tools/extraer-kardex.py`: solo nombre + saldos) → movimientos NUEVOS llevan fecha (Kary), los viejos arrancan en `config.fechaCorteMigracion` y Kary ajusta · **VENCIDO desde el día 1** pasado el plazo · **rangos** 1-30 / 31-60 / +60 días + vencidos en ROJO (alertas para que Kary decida a quién cobrar).
>   **Antes de F7 (ventas/factura/inventario)**: Consejo Externo + cerrar PRE-infra (TODO-14: backup + App Check).
...
| TODO-13 | **PRE-integridad (código)** ✅ commit `ea1f93f` (Desarrollo): `movimientos` append-only en reglas (delete:false + update SOLO anular con `motivoAnulacion`, helper `anulacionValida`) + modal de anulación con motivo en la ficha + `onInquiryCreated` idempotente. Reglas **33/33** (emulador) + build verde. **DEPLOY COORDINADO pendiente**: merge Desarrollo→main (front por Pages) **+** `firebase deploy --only firestore:rules,functions` JUNTOS (reglas sin front = anular se rompe). | 🟡 Código listo, sin deploy | Deploy coordinado |
