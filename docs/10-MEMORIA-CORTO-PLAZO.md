...
> **Sesión §138 (2026-06-27 tarde) — 4 AJUSTES WEB (Daniel):** 15 destacadas (+6 marcadas) + botón al final de la franja · isla "Atajos" SOLO en index (verif. live) · **etiqueta única "Precio a consultar"** (`priceDisplay` SSoT en `core/format.js`; elimina "Cotización"/"Consultar precio"; 32 piezas con `priceLabel=""`). Build+test:sidebar verdes. SW v44 · APP_VERSION v27. **CÓDIGO → requiere DEPLOY** (la data ya está live; el código viejo no la aprovecha). Detalle → ADR §138.
