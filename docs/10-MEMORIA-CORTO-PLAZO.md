...
> 2026-07-10 · **[OPUS-4.8] INTERINATO #3 — B0 de F-IA-2 ✅** (spec: inventario 17 págs + hallazgos H1-H8). H1 `plazoDefault` huérfano→dedup cobranza SEGURA [grep, 0 lectores]. **Decisiones Daniel**: H4=aprobaciones SOLO Bandeja (Salud deja de mostrarlas); **H2 (Caja/Fiscal) RESERVADA a Fable** ("activaré Fable para que analices esto") → análisis+rec de Opus listo en spec §6.7. Gates antes de su bloque: H6 (ubicar corte/acta), H7 (`aprobaciones.js`+`aviso-solicitudes.js` ya existen → leer antes de B4). Sin tocar prod. Sigue: B1 (4 pestañas) o handoff Fable. `[[feedback_opus_interino]]`
