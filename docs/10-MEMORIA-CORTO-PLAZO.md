...
>
> **🔨 WOMPI F2 ENCENDIDO en prod** (TODO-42; falta gate live real de Daniel) → **foco = rediseño UX checkout (TODO-63, DISEÑADO, listo para construir)**. Specs `2026-06-28-wompi-checkout-web-design` + `2026-06-30-checkout-redesign-design`. `[[project_comercio_pagos]]`
...
| TODO-42 | **Wompi F2 ENCENDIDO en prod 2026-06-30** (flag ON; v62; CFs prod tras fix [[L-65]]). **PEND gate live (Daniel)**: pegar URL webhook en panel (URL de Eventos) + re-test real (Widget=PRODUCCIÓN) pieza ≥$1.500. Precios = cuando Kary valide. SSoT → spec §12-§14. | 🟢 | URL Eventos + re-test |
...
| TODO-63 | **Rediseño UX checkout — DESPLEGADO + VERIFICADO LIVE (v63/v64)** — diseño comité×4+Antigravity + gate ✅. Entrega (tipo/país/legal_id/pickup-dirección/consentimiento) · enfoque de pago (ocultar+escape) · Wompi **REDIRECT** · §3 GD-2 · pantalla éxito · cimientos+tests 12/12. Verif. en Chrome en prod. **PEND**: gate-live del **redirect Wompi real** (Daniel: pieza ≥$1.500 + URL Eventos en panel → yo valido en Chrome) + refinamiento reserva-reintento (backend). Spec `2026-06-30-checkout-redesign-design`. **Fixes v64 (2026-07-03, desplegado)**: selector país "CO +57" · borrado fotos robusto (`deletePieceImage` salta no-Storage; piezas+colecciones; rol owner sí borra, la falla era en imágenes demo no-Storage) · caché SÍNCRONO del CMS (`data.getSiteContent`) anti-flash hero/toda sección. | 🟡 | gate-live redirect |
| TODO-64 | **Bug LOGIN admin — MÚLTIPLES parpadeos** al ingresar (Daniel 2026-07-03): el login parpadea muchas veces antes de autenticar. **Revisar en OTRO chat** (este llegó a ~800k/1M). Ver `admin-login.html` + flujo de auth (posible loop de `onAuthStateChanged`/redirect). | 🔲 | otro chat |
