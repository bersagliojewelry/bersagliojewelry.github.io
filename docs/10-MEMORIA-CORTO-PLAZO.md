...
>
> **🧭 FOCO = PLAN ÚNICO ERP v4 (TODO-68)** — SSoT spec `2026-07-04-plan-unico-erp-v4`. **F1 COMPLETO** (§165-§168). **F2.0 caja/bóveda: backend + B5b-1 UI ✅** (build + `test:caja-format` 9/9). **B5c PARCIAL (2026-07-07)**: functions+rules+indexes **DESPLEGADOS a prod** (8 CFs caja vivas + `crearPedido` actualizado, verif. `functions:list`) + `config/caja` sembrado (enforceTurno:false·fondo 200k·límite 4M). **FALTA para cerrar B5c**: (1) publicar la UI = **merge `Desarrollo→main`** (site/Pages — Daniel, "sin push"; SW v76 en archivo, prod aún v75); (2) **gate Chrome** (sesión owner) + ADR. Runbook §9.4 (activar enforceTurno) queda para cuando Kary opere. 🚩 Brecha rol `caja` (L-78) = candidato B5d con gate de dinero. Precios NO bloquean; 1ª APPROVED viva. `[[project_comercio_pagos]]`
