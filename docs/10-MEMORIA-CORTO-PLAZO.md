...
> 2026-07-09 · **F2.4 apartados SPEC** (W-11: evidencia+arquitecto+comité×3/17-ag+legal-CO). Detalle → spec `2026-07-09-f2-4-apartados-DISENO.md` + bóveda (CRUDO/DIGEST/EVIDENCIA-LEGAL). Comité corrigió P0s (anticipo=PASIVO no fiado; revenue sin hogar; stock=hold; idempotencia) verificados en código; moderé sobre-ingeniería (libro/outbox→F4). Cancelación resuelta (art.43.5 prohíbe decomiso→reembolso 100%+saldo-a-favor sin vencimiento). RBAC=cajera·20%/60d. Ver TODO-39.
> 2026-07-09 · **Auditoría Antigravity (`e825839`)** (comité copy + asesor + preview + Firestore MCP). **Hidratación P0 ARREGLADA+VERIFICADA** (`729c708`, cache v85): prerender horneaba real pero `home.js renderAll()` sin datos pintaba SKELETON → fix = `window.__BJ_CATALOG` inline (build) + `data.hydrateFromBake()` síncrono → preview 16 cards reales/0 skeletons/0 errores; SSG_SELFTEST cubre `bakeCatalogSeed`. Copy: quité superlativo falso "más grande de la región" de defaults + reescritura comité. **CLAVE**: copy en vivo = Firestore `siteContent/{home,nosotros}` (sobrescribe defaults) → cambios de copy de Antigravity NUNCA se vieron en vivo. CRUDO en bóveda. Copy nuevo **subido al CMS** (`siteContent/home`+`nosotros` vía MCP, `_version` bump) + **DESPLEGADO a prod** (merge `Desarrollo→main` `f0e9fe7` → Pages re-hornea CMS). **TODO-47** (Kary): CMS nosotros con claims sin verificar/inconsistentes (43 años vs "12+"/"40+" home · +12.000 vs "5000+" · 12-40 países · certificaciones).
