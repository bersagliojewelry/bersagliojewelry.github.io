...
| TODO-09 | **Fase 3 — RECONSTRUCCIÓN del CRM** (reemplazar el admin actual) con **mentalidad de arquitecto**: CRM/leads + facturación + inventario, modular/escalable/seguro/cost-aware, cero monolitos. Charter → `docs/50-ARQUITECTURA §3`. `crm-architect`+`ecommerce`. | 🔲 Abierto | Confirmar dominio (Bersaglio?) + arrancar tras Fase 2 |
