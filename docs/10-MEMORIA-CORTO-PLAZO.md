...
> 2026-07-10 · **[OPUS-4.8] F-IA-2 B4 ✅ (SW v92)** — Bandeja ÚNICA de aprobaciones: `admin-aprobaciones.html`+`bandeja.js` (monta `initAprobaciones()` cartera M2b TAL CUAL + réplica de `boveda.js:117` para bóveda con `aprobarEventoCaja` existente) + badge vivo `aprob-badge` (owner, en shared.js). Salud pierde 4ª tarjeta-semáforo + `#sec-aprobaciones` + modal rechazo (D2); Avisos(Hoy)/banner→Bandeja. **Bandeja=owner-only** (bóveda `read isOwner`; desvío justificado de §0.7 "role admin" → spec §B4). CERO callables nuevas. Verif: build + test:sidebar 13/13 + traza adversarial (callables existentes · bóveda idempotente por opId · sin refs colgantes en Salud · contrato DOM 16/16). ⏳ ciclo aprobar/rechazar vivo → deploy. `[[feedback_opus_interino]]`
> 2026-07-10 · **[OPUS-4.8] B0·B1·B2·B3 ✅** (detalle→spec §0.7/checklist + §B3): B1 4 pestañas · B2 directorio⇄Cartera · B3 "Hoy" pulso $ read-only (dedup `ventasEfectivoTurno`→caja-format). Desplegados; ⏳ funcional-con-sesión→deploy (L-05).
> 2026-07-10 · **[FABLE-5] D1-D7 CERRADAS + previo (→spec §0.7 · ADRs §181-183)**: §181+§182 en prod · 1ª APPROVED real ✅ · rail v2 → `[[feedback_reorganizar_no_es_depurar]]`. ⚠️ **Pend Daniel**: cartera migrada (spec v5 §8) · consejo+abogado apartados (TODO-39). Precios = paso FINAL.
