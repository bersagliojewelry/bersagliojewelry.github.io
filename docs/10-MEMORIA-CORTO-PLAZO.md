...
>
> **▶️ FOCO ACTIVO — CMS WEB PÚBLICA (2026-06-14, urgente · Kary)**: todo el contenido público administrable desde el panel + sync + SEO/UX. **Plan + comité (6 lentes) + modelo de datos + decisión SEO + orden de build + estado por ítem → spec `docs/superpowers/specs/2026-06-14-cms-web-publica-design.md` (§5 + Checklist)** — leerlo al reanudar. Autónomo nocturno. **HECHO**: P0 sync/safeUrl/comité (`47b07b8`/`39983f2`/`wrpym7h3p`) · P1.1 categorías (`d4caaf6`) · **motor CRUD** servicio `createTypedDoc…` (`8b507cb`, pieces/cols = wrappers verdes) + UI `createResourceAdmin` (núcleo puro testeable) + **panel `Contenido web`** de pestañas (`67fc21e`) · **journal E2E**: admin+reglas (emulador 151/151, 144 sin regresión) + público con **fallback baked** + fix bug "eager" (`66dfd04`). Verif: build verde · 197/197 no-emulador · 151/151 reglas. Prod 0 piezas/0 journal → todo baked (sin cambio visible). **▶️ GRAN PLAN definido** (comité ×3 + red-team, `wf_d99a59a5-746`) → spec **`2026-06-14-cms-gran-plan-dinamismo-design.md`** (modelo de datos, builder recortado 10× por FORMULARIO, no-demo, SPA/SSR, roadmap P0→P5, 5 bloqueantes pre-deploy). **✅ CONSEJO EXTERNO INTEGRADO** (Gemini, peer review en spec): adoptado **SSG build-bake + auto-rebuild** (CF onWrite→repository_dispatch) = sustrato SEO + **0 lecturas runtime** (mejora v1) · CSP estricta vía `<meta>` (tarea: auditar inline-scripts) · storage rule con claim ROL (no `token.admin`) · flujo invitación P5. REFUTADO "DOMPurify siempre" (esc() no inyecta HTML; solo si rich-text). **P0 HECHO** (`c257dca`): B1 `match /system` + B2 guard feat null + B4 listener journal LAZY (`data.loadJournal()`, fuera del load global) + B5 `storage.rules` endurecido/validado (rol+MIME png/jpeg/webp+cap) · **155/155 reglas**. + **B3-categorías no-demo** (`9f8c86f`: `cardsFrom`→[] sin colecciones, sección oculta). **▶️ REANUDAR EN P1** singleton piloto `siteContent/home` (scaffold NUEVO form→`setDoc(merge)`+_version, `getDoc` cacheado NO onSnapshot; hero+editorial; pestaña en `contenido-tabs.js`) + UNDO ("restaurar versión" del audit log) + `hideWhenEmpty` en el modelo. (**P0.5 SEED** = coordinado con Daniel/deploy, NO autónomo: escribe contenido a prod vía Admin SDK · **B3 films/social** = P3 con su colección.) Detalle por fase → spec. **Lecciones→`30`**: (a) `.innerHTML` crudo→`mount()`; (b) falso positivo hook con exec de RegExp→`String.match()`; (c) núcleo PURO `-core.js`; (d) `singleton=getDoc cacheado / lista=colección lazy` (Spark). **DEPLOY journal pendiente**: merge Daniel (L-26) + reglas (incl. B1+B5) + cache bump v16→v17.
>
