...
>
> **🧭 FOCO = PLAN ÚNICO ERP v4 (TODO-68)** — SSoT spec `2026-07-04-plan-unico-erp-v4`. **F1 · F2.0 · F2.1 · caja-tiempo-real ✅ EN PROD (§165-§172)** · **TODO-70 POS profesional ✅ §173**. **EN CURSO = F2.2 factura multi-línea (TODO-41)**: **H1-H4 ✅ construidos+probados en `Desarrollo`** (motor+reglas TDD · export · UI mostrador · CMS). **CÓDIGO EN MAIN ✅** (PR#411 `153c48a`). **Backend DESPLEGADO ✅ 2026-07-08** (`functions,firestore:rules` a prod; 47 fn upd + reglas; pre-deploy reglas 242/242·f2-2 16/16 verdes). **H5 restante = seed 1 servicio + gate live (login owner Daniel) → ADR §176** (ver TODO-41). **SIGUE tras F2.2**: 2.4 apartados (TODO-39) · 2.3 térmica (D-5). Precios NO bloquean; 1ª APPROVED viva. `[[project_comercio_pagos]]`
...
| TODO-41 | **Facturación multi-línea = F2.2** (catálogo servicios + línea libre; decisión "AMBOS" · spec §8 · gate fiscal: No responsable IVA · `42-LEGAL §7`). **H1-H4 + merge main (PR#411 `153c48a`) + backend DESPLEGADO a prod ✅ 2026-07-08** (`functions,firestore:rules`; f2-2 16/16·rules 242/242 verdes pre-deploy; SIN SW bump — admin network-first). Detalle de diseño/commits → spec §8 + git. **H5 restante**: (1) seed 1 servicio (CMS `admin-servicios.html`, owner Daniel); (2) **gate live Chrome (login owner Daniel)**: venta pieza+servicio→total→anular (prueba reglas `servicios` en vivo); (3) ADR **§176** + `00b`/`00` + `30`. Topes `config/caja` (2M/10M/500k). ⚠️ Hasta el gate: **avisar Kary NO usar "+ Servicio"**. | 🟡 | H5: seed+gate (owner Daniel) |
...
> 2026-07-08 · **Sesión "F2.2 H1-H4"** (Opus): construidos H1-H4 (motor+reglas TDD·export·UI·CMS) + **reconciliada divergencia de `main`** (§174/§175 cerebro-only, código NO conflictúa) + **merge a main** (PR#411 `153c48a`, lo hizo Daniel → **merge/deploy son MÍOS**, me quedé corto). **Backend desplegado a prod ✅** (`functions,firestore:rules`; 47 fn upd; reglas 242/242·f2-2 16/16 verdes pre-deploy). H5 restante = seed 1 servicio + gate live (owner Daniel) → ADR §176. → TODO-41. **Gotcha**: `test:rules` falla si el `http-server` de altorra ocupa `:8080` (matar PID) — entorno.
