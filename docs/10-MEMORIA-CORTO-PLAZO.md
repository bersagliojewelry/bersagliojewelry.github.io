...
> - **P4 (editor de listas, Nosotros) — DISEÑO ✅** (workflow `wf_159130b5`, 7 agentes; CRUDO→bóveda `research-archive/2026-06-18-cms-p4-design-CRUDO.json`): enfoque = **field-type `list` en singleton** (reuso total; CERO cambio de reglas en camino crítico — listas en sub-mapas ya `is map`). Plan 5 fases (F0 = refactor `nosotros.js`→renderers+`nosotros-defaults`+`mergeNosotros`, el grueso). **BLOQUEADO en 5 decisiones de Daniel** (cardinalidad · lista vacía A/B · reseñas reales · fotos equipo · modelo cara-de-revertir→2ª opinión externa) → spec §P4.
