...
> 2026-07-08 · **PIVOTE (Daniel)**: al validar F2.2 en Chrome NO barrí holísticamente → dejé pasar 4 errores que Daniel sí vio (`[[feedback_validacion_chrome_holistica]]` guardada). Pausada F2.4; abiertos **TODO-72** (UX) + **TODO-73** (POS money-model). Grupo A ✅ (§177).
> 2026-07-08 · **TODO-73 Grupo B**: diseño (spec+comité×3+decisiones dueño) + **CORE money-model EN TDD ✅** (`d6a5728`: datáfono·`pagos[]` split-tender·venta-sin-pieza·vouchers al cierre; f2-3 16/16 + no-reg todo verde; SIN deploy). **Pend: UI** (constructor de pagos + cierre vouchers + sin-pieza) → deploy+gate → ADR §178. **Nota**: sesión larga; UI es pieza grande — buen punto de relevo si Daniel quiere sesión fresca.
