...
>
> **🔴🚦 FRENTE VIVO (relevo): TODO-40 MODELO INVENTARIO — Fase 1 IMPLEMENTÁNDOSE.** Daniel OK. **B1 ✅** (`a45d44a`, migración legacy→v3, 17 tests verde). **Siguen B2-B6** (reglas→CFs→admin→SSG→tests; orden+detalle en la fila TODO-40 ↓ + plan §12.3 de la spec). Precede la carga masiva.
...
| TODO-40 | **MODELO INVENTARIO MULTI-TIPO** (Daniel 2026-06-26, Decisión Fuerte, PRECEDE la carga masiva): diseño+comité ×4+Daniel → Gemini ✅ → visión "web cobra AUTO, Mercado Libre" → v2 §11 → comité v2 ×5 ✅ → **modelo v3 §12** → **Daniel OK → IMPLEMENTANDO F1.** **Bloque 1 ✅** (`a45d44a`: `js/admin/inventario-model.js` modelo puro [enum3 stockType `finito`/`finito_refabricable`/`encargo`, `derivarEstado`, `migrarPiezaV3`, invariantes] + `scripts/migrate-inventario-v3.mjs` runner + `tests/inventario-migracion.test.mjs` **17/17 verde**). **Siguen (orden §12.3): B2 reglas** (enum3 + `cantidad`/`visibilidad` CF-only por IGUALDAD update+create + read blindado `!= privada`) · **B3 CFs** (`crearPedidoCore` decrementa+gate stockType+estado derivado; `ajustarStock`/`reabastecerStock`/`cambiarTipoPieza`; ledger `pieces/{id}/movimientos`) · **B4 admin** (piezas.js/.html: select enum3+visibilidad, QUITAR cantidad del save, botones) · **B5 SSG** (filtra privadas + PreOrder refabricable) · **B6 tests integración**. | 🟡 | F1 B2-B6 (relevo) |
...
>
> **▶️ Vivo**: TODO-40 Fase 1 (inventario v3). **B1 ✅** migración (`a45d44a`, 17 tests). Siguen B2 reglas → B3 CFs+ledger → B4 admin → B5 SSG → B6 tests integración (§12.3). Crudo+síntesis Gemini+comité v2 → bóveda `2026-06-26-*inventario*`.
