...
> 2. **🛡️ Semana 1 del plan §57** (bóveda `plan-operacion-robustecimiento-2026-06.md`): ✅ HECHO: forms endurecidos (§59) · 2FA · backup diario (§60) · **GEMELO VIVO** (§61: `bersaglio-gemelo.web.app`, sembrado, login aula verificado E2E; credenciales aula → seed-gemelo.mjs). **✅ PRE-1 CERRADO (§63): restauración PROBADA** (702/702 docs al gemelo, vista por Daniel, gemelo limpiado y re-sembrado; 1ª copia fuera → `C:\Users\romad\Documents\BersaglioBackups\`, rutina viernes). PENDIENTE: alertas de presupuesto GCP (Daniel) · recorrido del kill-switch (Daniel, bóveda `runbook-interruptor-emergencia.md`) · firmar contrato (bóveda) · talonarios/arqueo (Kary). **🏁 F6 COMPLETO** (§64-§68 · CI verde). **▶️ FASE M (plan §69 bóveda · runbook bóveda)**: **M0-H+M0 HECHOS y DESPLEGADOS (§70)** — config/cartera sembrada (owner-only), calibración (cero histórico → tope $50k provisional, recalibrar a 90d), Consejo Externo disponible (M3). **⏳ Preguntas 1-5 con Daniel/Kary (gatean el tren M2a/M2b/M3); siguiente slice: M1 reglas aditivas (`solicitudes`+`gestiones`+índice collectionGroup) — NO depende de respuestas.** Expand-contract vinculante; compuerta de adopción (smoke Kary) se integra al guion de M2a. **⏸️ DIAN/factura electrónica PAUSADA por Daniel** (solo si Kary la pide; ventas = "tirilla" comprobante interno rotulado "no es factura electrónica"; supuesto planeación ~$120-150M/año < umbral $183M; detalle → bóveda LEGALES). Decisiones 1-9 respondidas ✅ · **política de cartera v1 APROBADA** (→ config en Fase M) · contrato dueños = entregable pendiente.
