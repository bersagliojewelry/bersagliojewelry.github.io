...
> 2026-07-17 · **[OPUS-4.8] Auditoría GSC + GBP** (TODO-35 · B2 · skill `maps-gbp-local`). **Detalle y doctrina → el spec de abajo.** **GSC**: 9 indexadas / **27 "rastreada: actualmente sin indexar"** = juicio de VALOR (no bloqueo técnico; las rastrea a diario) · 2 noindex · 0 404. Sitemap REENVIADO (leído 10jul=37; hoy 52). Producto: 17 no válidas (falta `price`) → no bloquea indexar. CWV sin datos. **GBP**: verificado 100%, **85 reseñas ★5,0**, **74 respondidas (87%) / 11 sin responder** (CONTADO 9 págs; las 11 = recientes). Perfil sin tocar 6 sem; reseñas mixtas es/en → ❓turista anglo; **las 85 NO están en la web** → **TODO-48 SUBE** (espacio YA existe: `nosotros.js` §10 `resenas.items`, hide-when-empty; y la colección `reviews` + reglas están hechas pero SIN consumir). ⚠️ NO `aggregateRating` con reseñas de Google. **A4 geo ✅** (10.4251642,-75.5492068) deploy `d1015f2`. Palancas: responder 11 · reseñas en web · GBP fresco · precios · enlaces.
