...
>
> **🧭 FOCO = PLAN ÚNICO ERP v4 (TODO-68)** — SSoT spec `2026-07-04-plan-unico-erp-v4`. **F1 COMPLETO** (§165-§168). **F2.0 caja/bóveda: backend B0-B4 + B5a/B5b-0 + B5b-1 UI ✅** (build + `test:caja-format` 9/9 verdes, SIN deploy; L-72 resuelto d244d77). **PRÓXIMO B5c**: SW bump v76 + DEPLOY MANUAL (functions+rules+indexes, aún SIN desplegar) + sembrar `config/caja` (owner) + gate Chrome (owner) + ADR. 🚩 **Reportado a Daniel**: brecha del rol `caja` (L-78: turno/bóveda lo distinguen, pero `crearPedido`/`pedidos`-read/`auth.js` NO → la cajera aún no vende; Kary=owner opera todo) → habilitarla = paso con gate de dinero (candidato B5d). Precios NO bloquean; 1ª APPROVED sigue viva. `[[project_comercio_pagos]]`
