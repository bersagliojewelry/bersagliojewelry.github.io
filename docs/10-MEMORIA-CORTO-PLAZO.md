...
| TODO-58 | **Buscador por código** (spec `2026-06-29-buscador-codigo`). HECHO ✅: catálogo (filtro vivo, TODO-60) · Ref. ficha · **link `/p/<código>` (stubs OG)** · **acceso en INICIO** (banda discreta; `.bc*`→`components.css` = componente compartido). **Pend (NEXT=slice 5: pulir 404)**. Detalle → spec §7. | 🟡 | slice 5 |
...
> ✅ **Cerrados**: **TODO-59 (§153 color del oro)** · TODO-56/55/51/52/53/54/44/45/46 · TODO-40+32 → ADRs/`99`. Pend Daniel: precios + fotos IA.
...
>
> **2026-06-29 (cont.):** **TODO-58 slices 2+3 ✅** — links compartibles `/p/<código>` (32 stubs SSG: OG+canonical+noindex+redirect; `404.html` rutea `/p/`) **+ buscador en el INICIO** (banda discreta; `.bc*` movidos a `components.css` = componente compartido catálogo+home). Verif: build · test:codigo 11/11 · SSG_SELFTEST · redirect+submit probados en navegador. Lecciones [[L-61]] (artefactos SSG) · [[L-05]] (rAF no dispara en preview oculto). Sin cache bump. NEXT = slice 5 (404 polish).
