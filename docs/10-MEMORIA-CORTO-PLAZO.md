...
>
> **🧭 FOCO = PLAN ÚNICO ERP v4 (TODO-68)** — SSoT spec `2026-07-04-plan-unico-erp-v4`. **F1 · F2.0 · F2.1 · caja-tiempo-real ✅ EN PROD (§165-§172)** · **TODO-70 POS profesional ✅ §173**. **EN CURSO = F2.2 factura multi-línea (TODO-41)**: **H1-H4 ✅ construidos+probados en `Desarrollo`** (motor+reglas TDD · export · UI mostrador · CMS). **CÓDIGO EN MAIN ✅** (PR#411 `153c48a`; web deployando). **H5 restante → 🔴 PRIMERO deploy `functions,firestore:rules`** (sin él la UI de servicios MISCOBRA — avisar Kary) → gate live (login Daniel) → ADR §176 (ver TODO-41). **SIGUE tras F2.2**: 2.4 apartados (TODO-39) · 2.3 térmica (D-5). Precios NO bloquean; 1ª APPROVED viva. `[[project_comercio_pagos]]`
...
| TODO-41 | **Facturación multi-línea = F2.2** (decisión "AMBOS" = catálogo servicios + línea libre). Spec §8 ✅ (comité ×3 · gate fiscal RESUELTO: No responsable IVA → sin lógica por línea, `naturaleza` future-proof) · `42-LEGAL §7`. **H1-H4 ✅ (Opus · `Desarrollo`)**: motor `lineasExtra`+reglas `servicios/{id}` · export "Servicios (bruto)" · UI mostrador (`onServiciosChange`) · CMS `admin-servicios.html`+`servicios.js`. Claves de diseño: precio servicio server-side · línea libre blindada+fingerprint · `items[]`=SSoT · `desglose`=subtotal pieza · owner-write acotada+soft-delete+historial · total en vivo=servidor. Commits `eee8e65`/`9a6232a`/`1e09689`/`79ce696` · **f2-2 16/16·rules 242/242·no-reg 24+31·build verde** · SIN SW bump (admin=network-first; NO tocar 05 cache). **MERGE A MAIN ✅** (PR#411 `153c48a`; lo hizo Daniel manual — **el merge/deploy son MÍOS**, me quedé corto; `Desarrollo` ff→`153c48a`). Web deployando. **H5 restante (fresh, EN ORDEN)**: (1) **🔴 deploy `firebase deploy --only functions,firestore:rules`** — sin él la UI de servicios MISCOBRA + CMS denegado → **avisar Kary: NO usar "+ Servicio"**; (2) sembrar 1 servicio; (3) **gate live Chrome (login owner Daniel)**: venta pieza+servicio→verificar total→anular; (4) ADR **§176** + `00b`/`00` + `30`. Topes `config/caja` (2M/10M/500k). | 🟡 | H5: deploy backend 1º → gate (Daniel) |
...
> 2026-07-08 · **Sesión "F2.2 H1-H4"** (Opus): construidos H1-H4 (motor+reglas TDD·export·UI·CMS) + **reconciliada divergencia de `main`** (§174/§175 cerebro-only, código NO conflictúa) + **merge a main** (PR#411 `153c48a`, lo hizo Daniel → **merge/deploy son MÍOS**, me quedé corto). ADR=§176. Detalle + H5 restante (🔴 deploy backend 1º → gate) → TODO-41. **Gotcha**: `test:rules` falla si el `http-server` de altorra ocupa `:8080` (matar PID) — entorno.
