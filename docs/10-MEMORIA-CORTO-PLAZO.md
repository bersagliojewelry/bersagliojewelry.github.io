...
> - **P4 (Nosotros editable) ✅ COMPLETO en `Desarrollo`, sin mergear** (2026-06-19, [OPUS-4.8]): field-type `list` (add/quitar/reordenar) + MODELO PLANO 12 claves (tras 2ª opinión Gemini: eliminado grab-bag `cartagena`, migración cero pre-prod) + headers valores/timeline editables (decisión B) + guards anti poison-pill + cap reglas 24. Verif: build · 23 CMS · **169 reglas** · revisión adversarial 9 ag. (0 crít/regresión) · página idéntica. Detalle → spec §P4 + bóveda (`cms-p4-review`, `consejo-gemini-p4-modelo-SINTESIS`). **▶️ GATE**: (1) opinión externa del modelo ✅ HECHA · (2) **FALTA validación de Daniel** (editor en vivo, login) · (3) deploy reglas+sitio. **NO mergear antes de (2).**
