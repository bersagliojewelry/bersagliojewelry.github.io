...
> **Avance B1 (detalle → ADRs §122-§128)**: pasos 1-3 (inventario · calculadora · POS Mostrador `crearPedido`) + COP §127 **MERGEADOS a prod**; **paso 4a confirmar pago §128** (CF `confirmarPago` desplegada; botón en Ventas recientes; 9/9 tests) — código `ba6da22` PEND merge. CFs `crearPedido`+`confirmarPago` DESPLEGADAS. **➡️ SIGUIENTE: decisión paso 4b apartados** (TODO-39) → VOID+arqueo.
> _**PEND merge Daniel**: §128 (`ba6da22`). **PEND verif. EN VIVO**: Kary registra venta + confirma pago (pieza `seedDemo`). Gotcha dev L-58._ Cola: TODO-39 · TODO-33 · TODO-35.
...
> ✅ **Cerrados recientes**: histórico completo → ADRs §88-§128 + `00`/`99` (no re-listar aquí; últimos: §126 POS Mostrador + deploy CF · §127 todo en COP / cierra TODO-38 · §128 confirmar pago "vi la plata").
...
>
> **▶️ Hitos**: detalle en ADRs §88-§128. Vivo: **TODO-37 B1 mostrador** — paso 3 (POS §125-§126) + COP §127 MERGEADOS a prod; **paso 4a confirmar pago §128** (CF desplegada, pend. merge `ba6da22`).
>
> **🚦 Próximo**: **decisión paso 4b apartados/abonos** (TODO-39, §128.4): ¿el mostrador necesita apartar piezas (anticipo+saldo)? Si sí → reusar la CARTERA existente, no un sistema paralelo. Preguntar a Daniel. Antes: verif. en vivo (Kary registra venta + confirma pago, pieza `seedDemo`). Luego VOID + arqueo (paso 5). Cola: TODO-33 (parpadeo) · TODO-35 (A2b/eventos/HUB). **Regla**: `arquitecto-software` SIEMPRE · **Bersaglio = 100% COP, cero dólares (§127)** · flujo COMPLETO **W-11** cuando toque diseño/decisión cara · `[[feedback_workflows_acotados]]`.
