...
| TODO-47 | **Verdad de marca (riesgo SIC)** — Daniel confirmó (17jul): ✅ garantía de por vida (→ Términos §191) · ✅ certificados autenticidad · ✅ reseñas familiares (clientes reales) · ✅ **platino** (secundario; principal oro 18K) · ✅ **oro por peso** · ✅ **cifras home** (40+/5000+ reales; el default 12+/800+ era DEMO → sincronizado). Demo purgado de defaults (cifras + 4 reseñas ficticias en `nosotros-defaults`). **Resta menor**: equipo/horario. → §191.7 · `[[feedback_no_demo_en_index]]`. | 🟢 | casi cerrado |
| TODO-48 | **Reseñas reales en la web** — el espacio EXISTE (`nosotros.js §10` `resenas.items`, hide-when-empty; Firestore `siteContent/nosotros.resenas.items:[]` → hoy oculto; default ya sin fakes). Hay **85 reseñas ★5,0** reales en el GBP. Falta: curar cuáles + poblar (Firestore o colección `reviews`+gestión admin, ya con reglas §190). ⚠️ **arista legal**: republicar reseñas de Google en el sitio (atribución/política) → validar. Palanca alta (prueba social + contenido). | 🔲 | curaduría + legal |
...
> 2026-07-17 · **[OPUS-4.8] Jornada SEO + verdad-de-marca → ADRs §188-§191** (todo en PROD, SW v97): A4 `og:type=product` + **FAQPage retirado** (rich result muerto 07-may) §188 · 🔴 **fuga PII a GA4 cortada** (guard `stripPII` en `track()`, `test:analytics` 10/10) §189 → **TODO-17 SUBE** · **GBP 11 reseñas → 85/85** §190 · **garantía DE POR VIDA en Términos** (Kary confirmó; Ley 1480, alcance vía AskUserQuestion) §191 · **verdad de marca**: platino/oro-por-peso/cifras confirmados reales + demo purgado de defaults (`no-demo-home` 10/10) → **TODO-47 casi cerrado**. Palancas SEO restantes: **TODO-48 reseñas en web** · **FAQ visible verificable** (ya desbloqueada) · GBP fresco · precios · enlaces. Hallazgo: guard no-demo no cubre `js/pages/` → chip.
...
> **Pend Daniel/no-gate** (→ ADRs §165-171): cartera migrada · consejo+abogado apartados (39) · instructivo Kary · push A.6 · fotos (67) · abogado CO. ADC gcloud caducado (firebase CLI OK). **Precios = paso FINAL.**
