...
> · **BUILD 2a — BACKEND COMPLETO ✅** (Desarrollo; main atrás): P1 candado · P2 cripto · P3 `iniciarPagoWeb` · P4 webhook `confirmarPagoWompi` · P5 reaper `liberarReservasVencidas`+índice `pedidos(estado,reservaExpira)`. **54 tests verdes** (POS 19 regresión + wompi 11 + iniciarPago 10 + webhook 8 + reaper 6). **SIGUIENTE**: reglas (`webhookEvents` CF-only) → front (4ª opción "Pagar ahora"+Widget+checkbox habeas-data+aviso vendedor, tras flag) → deploy sandbox + secretos (Eventos/privada/integridad/pública) → gate live (tarjetas de prueba). App Check=2c · legal(TODO-49)=cobro REAL.
