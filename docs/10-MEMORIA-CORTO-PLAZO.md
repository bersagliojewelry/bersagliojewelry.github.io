...
>
> **▶️ RETOMA = sesión NUEVA dedicada al §103 F2** (navegación instantánea / router falso-SPA, **TODO-30**). **Prioridad** (Daniel 2026-06-23, con capturas): el **parpadeo al cambiar de menú** (Inicio↔Nosotros↔…) + la imagen que recarga = **cruce de shells = recarga completa** (§103.1b); **F1 NO lo arregla, F2 SÍ**. Toca TODA la nav (forms/drawers/scroll/atrás/carrito/SEO) → **comité #2 casos borde + gate**; ground-truth §103.2/.5. **Evaluar primero View Transitions nativas (cross-document, Chrome) vs el router** dentro de F2 (posible camino más liviano), y **reactivar el render LQIP** (revertido en §106) con la caché-memoria. Boot §G.1 + leer §103/§104/§105/§106.
>
> **▶️ 2026-06-23** ([OPUS-4.8]): §100-§103 (cero-demo · gate empírico · carga fluida · "app-like" DECIDIDA) → 99. **§104 F1 LQIP + §105 fix recarga `reveal.js` = EN PROD** (PR #306/#307). **§106 REVERT del render LQIP**: hacía vacío→borroso→nítido (3 estados, peor); el LQIP llega con el getDoc, no antes → SUMA estado. Vuelto a neutro→foto; fontanería conservada para F2. cache v26, tests 55/55. **Molestia REAL de Daniel = parpadeo entre PÁGINAS = F2 (pendiente).** → 99 §104/105/106 · L-47..L-50/M-07. [[feedback_no_demo_en_index]].
>
