...
>
> **SIGUE B5 (costuras finales, §6)**: D9 (abono del CRM→cuenta, flag off hasta test verde) + D6 (editor "Reglas del sistema" en Negocio y equipo, owner-only) + microcopy global + extensión del cuadre diario 3:30 en Salud. **⚠️ Zonas calientes R3 = test PRIMERO**: V1 (bóveda↔banco `consignacion_in`/`retiro_efectivo_out`, misma tx) · V17 (abono efectivo→`movsCaja`, exige turno) · V18 ("Retiro de banco" espejo de la Consignación). Luego B6 (rompimiento adversarial read-only + Chrome holístico) → **DEPLOY MANUAL del bundle** (`firebase deploy --only firestore:rules,firestore:indexes,functions` + seed virtuales prod; SW ya v98) + E2E holístico Chrome.
>
