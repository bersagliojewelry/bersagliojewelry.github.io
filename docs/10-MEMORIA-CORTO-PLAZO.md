...
> 2026-09-04 · **[OPUS-5] «Deploy to GitHub Pages» en ROJO desde la corrida #735** (2-sep, merge `9ec3ebf`) — el `npm test` de B-01 no instalaba las deps de `functions/` en el runner (gitignored: en el PC del dueño existen y sale verde), y dos pruebas del corte piden `functions/corte.js` → «Cannot find module firebase-functions/v2/scheduler». Arreglado en `Desarrollo` **`5c51a2a`**; **pendiente el merge a main que autoriza Daniel**. Candidata a lección «gates que mienten» (INMO §38): 3 días de deploy en rojo sin que nadie mirara.
