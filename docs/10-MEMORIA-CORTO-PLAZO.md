...
| TODO-41 | **Facturación multi-línea = F2.2** (Daniel "AMBOS" = catálogo servicios + línea libre). Spec ✅ + comité ×3 ✅ + gate fiscal RESUELTO (No responsable de IVA → sin lógica por línea; `naturaleza` future-proof). Spec `2026-07-07-f2-2 §8` · `42-LEGAL §7` · CRUDO bóveda. Costura `items[]` en `pedidos-core.js:244`. BUILD: `servicios`(colección+reglas+CMS)·`crearPedido lineasExtra`(TDD)·POS multi-línea·export `naturaleza`·gate. Prereq TODO-70 ✅ §173. | 🟢 | listo (spec §8 ✅) |
...
> 2026-07-08 · **Mantenimiento de cerebro** (out-of-scope §173): (1) **2º range-shard §174** — `00-INDICE` 30.8k→15.6k, §116–§157 → nueva hija `00b-INDICE` (kernel ya shard-aware §140, sin cambio). (2) **Auditoría Nivel-2 §175** (vencida 59 ADRs) ACOTADA (2 subagentes read-only + sondas directas): retrieval 5/5, cero sinapsis rota; 7 hallazgos → 4 resueltos (sello 05 · memoria `opus_interino` reactivada 07-06 · +2 filas de ruteo · marcador `verificado-vivo` en enforceTurno) + **TODO-71** (gates `[[...]]`/ssotFacts = cars-operador). GC: podada la fila TODO-70 malformada (cerrada §173) + esta bitácora. brain:check SANO · deepAudit re-sellado 07-08. §173 detalle → ADR.
> **Consolidado (→ ADRs)**: F2.1 §171 (flag LIVE · [a verificar] abogado CO) · F2.0 §169-170 (runbook §9.4 + B5d rol `caja` → Kary) · F1 §165-168. Pend no-gate: push A.6 · badge sidebar · TODO-67 fotos · botón "Atelier". ⚠️ ADC gcloud caducado. Nivel-2 cerebro HECHA (§175).
