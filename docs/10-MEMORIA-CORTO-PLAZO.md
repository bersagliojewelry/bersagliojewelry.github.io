...
>
> **▶️ RETOMAR AQUÍ — PANEL v2: MOROSOS/VENCIDOS desplegado a prod (ADR §51); SIGUIENTE = F5 completo** (2026-06-07):
...
> - **F5 (filtros) ✅ construido** (ADR §52, `Desarrollo` sin mergear): chips de estado (Todos/Vencidos/Al día/A favor) + rango de mora (1-30/31-60/+60) + select de vendedora + contador sobre la lista CxC; front puro, build verde. **Control de crédito DESCARTADO** (decisión Daniel 2026-06-07: la tienda no usa cupo por cliente; la validación va con F7). Front-only → live al mergear a `main` (Pages).
> - **▶️ SIGUIENTE**: F4-leads (Bandeja: `leads` + ingestión por CF + UI estados/SLA/convertir) **o** seguir con dinero (F7 ventas/factura — requiere Consejo Externo + PRE-infra TODO-14 antes).
