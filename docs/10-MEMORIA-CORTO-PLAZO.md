...
| TODO-30 | **§103 F2 navegación/carga app-like** — F2.0 View Transitions ✅ (§107). **AHORA: §108 "caché inteligente" (SWR nativo Firestore) con WORKFLOW COMPLETO** (mandato Daniel: comité+Gemini+skills+agentes BOUNDED; mitigar bugs antes de codear). Diseño/research/casos borde → §108. Router falso-SPA F2.1 = quizá innecesario. | 🔲 | sesión nueva (workflow) |
...
>
> **▶️ RETOMA = §108 F2.x "caché inteligente" (SWR nativo Firestore) — CON WORKFLOW COMPLETO (mandato explícito de Daniel)**. Daniel cerró pidiendo: la PRÓXIMA sesión hace el **workflow completo — `proceso-decision-fuerte` + `comité-expertos` ×3 + consejo externo Gemini (read-only §15) + skills (`arquitecto-software`, `caza-bugs`) + agentes — BOUNDED (`[[feedback_workflows_acotados]]`: razonar sobre el research/diseño ya verificado, agentes sin re-explorar libre + tope)** para **revisar/mitigar bugs y anticipar casos borde ANTES de codear**. **Todo el diseño + research + casos borde + fuentes → ADR §108 (LÉELO)** (resumen: `persistentLocalCache` + render cache-first + diff-gate; arregla la imagen blanco→foto, quizá hace innecesario el router). Boot §G.1 + leer **§108**. **PENDIENTE: merge de Daniel `Desarrollo`→`main`** (despliega F2.0 §107 + revert §106 + v27).
>
...
>
> **🚦 COLA REAL — gated/pendiente**: (1) **§103 F2 = PRIORIDAD: F2.0 View Transitions ✅ §107; AHORA §108 "caché inteligente" (SWR nativo Firestore) CON WORKFLOW COMPLETO (TODO-30, mandato Daniel)** — arregla la imagen blanco→foto + reactiva LQIP §106; el router F2.1 probablemente queda innecesario; F3 SSG opcional; (2) **al reactivar LQIP**: backfill/re-subir imágenes existentes (las pre-F1 no tienen LQIP); (3) **alinear cerebro de cars** (sesión dedicada); (4) borrado auto imagen Storage (irreversible); (5) C1 (PERF-07); (6) responsive; (7) limpiar CSS muerto + `/img/*` demo; (8) re-verificar catálogo/pieza CON piezas cuando Kary cargue. **Regla**: `arquitecto-software` SIEMPRE. `[[feedback_workflows_acotados]]`. [OPUS-4.8].
