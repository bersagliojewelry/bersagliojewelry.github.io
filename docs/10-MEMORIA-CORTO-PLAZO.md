...
> **(1) TODO-35 · VISIBILIDAD SITE-WIDE** (publicar web + SEO·AEO·GA4·Search Console·Google Maps): **Altorra Cars (HUB) CONSTRUYÓ + PROPAGÓ** el paquete (✅ 2026-06-25: las 7 skills + agente `seo-auditor` YA están en `skills/` + catalogadas en `skills-inventory.md §Paquete de Visibilidad`; falta IMPLEMENTAR aquí). Próxima sesión = implementar en Bersaglio: portar el **SSG** a piezas/colecciones/journal + GA4 + GSC + Maps + `product-feeds` + quitar `noindex` gated por flag `status` + publicar. **ARRANCA con el trigger de Daniel: "ALTORRA CARS YA TERMINO CONTINUA".** Pediré datos reales (redes/NAP/cuentas GA4-GSC-GBP). Spec/prompts/research → bóveda `2026-06-25-*`.
