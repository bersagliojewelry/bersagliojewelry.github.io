...
> - **P3.5 imagen de portada (Home) ✅** (`c6b7ff7`; `storage.rules` avif **desplegado 2026-06-19**). Daniel valida la subida real en vivo.
> - **P4 (Nosotros editable) ✅ EN PROD** (§83, 2026-06-19; validado Daniel): listas (add/quitar/reordenar) + modelo PLANO 12 claves (2ª opinión Gemini adoptada) + headers editables + guards poison-pill + cap reglas 24. **Imágenes** (hero/atelier/fotos equipo) en `Desarrollo` `b78c3aa`, **pendiente merge de Daniel**. Detalle → §83 + spec §P4.
> - **PENDIENTE DE 1 MERGE FINAL → main** (Daniel, GitHub Desktop): imágenes Nosotros (`b78c3aa`) + F3 barandas (`3c91ecf`) + `global` increment 1 (`7174348`). NO requieren deploy de reglas (admin/UI + claves global ya whitelisted). Tras el merge: smoke de Daniel (subir foto, ver "Publicado ✓", editar redes).
> - **`global` increment 1 ✅ construido**: singleton `siteContent/global` (redes + tagline) editable (pestaña "Datos globales"); footer (every-page) lo consume. **Corrige bug**: WhatsApp placeholder `573001234567`→real `573013752592` (footer vía global; wishlist fix directo).
> - **SIGUE (continuar el plan)**: `global` increment 2 (canales de Contacto + resto de consumidores WhatsApp a `global`) · `hideWhenEmpty` general · usuarios/SPA P5. **Pendiente GC**: ADR WYSIWYG (F1+F2) + P3.5 + F3 + global a `99` (aún sin consolidar).
>
