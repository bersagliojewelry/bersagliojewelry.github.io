...
>
> **TODO-24 (index 100% gestionable + CERO ficción) — Fase A+B CONSTRUIDAS** (§88; `feedback_no_demo_en_index`). Fase A `4ae6c0f`; **Fase B EN `Desarrollo`, SIN commitear, SW v18** (panel Videos/Redes + puerta reglas + defensa en profundidad + UX Kary; rules 185✓ puros 36✓ build✓ + review 4 ag.). **▶️ RESTA**: commitear → merge Daniel → `firebase deploy --only firestore:rules` → censo legacy. Detalle ↓ ledger TODO-24 / §88.
>
...
| TODO-24 | **index 100% gestionable + CERO ficción** (`feedback_no_demo_en_index`; spec `2026-06-20-cms-cero-ficcion-design.md`; decisiones Daniel: umbral 3/4/3/1 · enlace+miniatura · curado manual). Fase A `4ae6c0f` + **Fase B CONSTRUIDA (§88)**: panel Videos/Redes (`select`) · puerta reglas films/social + journalValid endurecido · defensa en profundidad (render re-filtra) · UX Kary · gate barrera #5. Verif rules 185+puros 36+build+review 4 ag. Detalle → §88. SW v18. Menor: ¿`href` obligatorio al publicar Redes? (flag Daniel). | 🟡 deploy | merge Daniel + `firebase deploy --only firestore:rules` + censo legacy |
...
> **2026-06-20** (cont. "continua"): **TODO-24 Fase A `4ae6c0f` + Fase B CONSTRUIDA (§88)** — SSoT umbrales/completitud · panel Videos/Redes (`select`) · puerta reglas films/social + journalValid endurecido (`.trim()`) · defensa en profundidad (render re-filtra) · UX Kary · gate barrera #5 · SW v18. Review 4 ag. → HIGH(render-legacy)+MED(whitespace) corregidos. rules 185✓ puros 36✓ build✓. SIN commitear aún. [OPUS-4.8].
