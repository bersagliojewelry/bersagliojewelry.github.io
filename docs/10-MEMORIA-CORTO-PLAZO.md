...
> · **SIGUIENTE**: Daniel pega esa URL en panel "URL de Eventos" + Guardar · luego Claude: encender flag `WOMPI_WEB_ENABLED=true` + CSS `.ck-pay-legal` + build + deploy site · precio TEMP <$2.5M en 1 pieza de prueba · **gate live** con tarjetas de prueba Wompi (validación Chrome) + confirmar endpoint by-reference del reaper. **2c**=legal(TODO-49)+llaves prod(Kary)+App Check+quitar precios temp.
