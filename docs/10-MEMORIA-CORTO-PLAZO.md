...
| TODO-76 | **F-IA-2 + DEPURACIÓN de plataforma (INTERINATO Opus 4.8)** — spec: `docs/superpowers/specs/2026-07-10-f-ia-2-DISENO.md`. **B0 ✅** (inventario 17 págs por-código + 8 hallazgos H1-H8 en la spec; H1=`plazoDefault` huérfano→dedup segura; H2=Caja/Fiscal/Identidad sin UI hoy; H6/H7=localizar corte/acta + leer aprobaciones.js antes de B4). SIGUE: **B1** Parámetros (6 pestañas +Usuarios; Kardex+Pendientes ELIMINAR) → B2 Split Clientes/Cartera → B3 "Hoy" $ → B4 Bandeja (+badge) → B5 Salud legible + microcopy. CERO functions/dinero nuevo; gates Chrome holísticos. `[[feedback_reorganizar_no_es_depurar]]` | 🟡 | Opus: B1 |
...
> 2026-07-10 · **[OPUS-4.8] INTERINATO #3 arranca — B0 de F-IA-2 ✅**. Recorrido por-código de las 17 páginas del rail; 8 hallazgos (H1-H8) en la spec. Verificado: H1 `plazoDefault` es campo huérfano (grep: 0 lectores) → dedup de cobranza SEGURA. En duda para su bloque (declarado, R7): H2 (Caja/Fiscal/Identidad sin editor hoy → pestañas solo-lectura, no inventar), H6 (ubicar corte/acta antes de moverlos), H7 (`aprobaciones.js`+`aviso-solicitudes.js` ya existen → leer antes de B4). NO se tocó código de prod (solo spec). Siguiente: B1 Parámetros unificados. `[[feedback_opus_interino]]`
> 2026-07-10 · **[FABLE-5] §181+§182 CERRADOS Y DESPLEGADOS** (→ADRs). **1ª APPROVED real ✅**. ⚠️ Decisión Daniel+Kary: conservar la cartera migrada (spec v5 §8).
> 2026-07-10 · **[FABLE-5] TODO-75 ✅ (§183)** + **prompt único a ALTORRA entregado** (copia: bóveda `2026-07-10-PROMPT-ALTORRA-sinapsis.md`).
> 2026-07-10 · **[FABLE-5] RELEVO → INTERINATO #3**: Fable al 93%; spec F-IA-2 con decisiones cerradas (TODO-76) + flag `05` + cola del titular (auditoría → F-TESORERIA → F-COMPRAS → F-REPORTES → apartados → ficha-pestañas → n8n). Pendientes DANIEL: decisión cartera migrada (spec v5 §8) · consejo+abogado apartados (TODO-39) · pegar prompt ALTORRA. **Precios reales = paso FINAL del lanzamiento (corrección Daniel), NO gate intermedio.**
> 2026-07-10 · **[FABLE-5] Feedback duro de Daniel al rail v2**: "cambios de nombre sin sentido... funciones regadas... ni yo lo entiendo" → nueva memoria `[[feedback_reorganizar_no_es_depurar]]` + F-IA-2 re-alcanzada como DEPURACIÓN de contenido (B0 inventario + veredictos: eliminar Migración-Kardex y tablero Pendientes, Sistema→2 items con Usuarios como pestaña owner, acta/corte de cartera→Salud "Cierre de mes", estados vacíos honestos, microcopy global es-CO). La meta-lección queda para consolidar como M-NN en la auditoría del titular.
