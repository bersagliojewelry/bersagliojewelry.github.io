...
> _**§126 MERGEADO a prod** (Daniel mergeó el POS). **PEND. MERGE Daniel**: §127 todo-en-COP (`9f36c2e`, en `Desarrollo`). CF `crearPedido` desplegada + POS ya en prod → **PEND: Kary registra una venta de prueba EN VIVO** (pieza `seedDemo`). Gotcha dev L-58._ Cola: TODO-33 · TODO-35.
...
> ✅ **Cerrados recientes**: histórico completo → ADRs §88-§127 + `00`/`99` (no re-listar aquí; últimos: §125 backend pedido · §126 POS Mostrador + deploy CF · §127 todo en COP / cierra TODO-38).
...
>
> **🚦 Próximo**: **B1 paso 3 (POS Mostrador) CERRADO en código + CF desplegada §126** → falta **merge de Daniel a `main`** (la página a prod) + **verif. en vivo** (Kary registra venta de pieza `seedDemo`). Luego **paso 4 `registrarPago`** 1..N + comprobante por-verificar (spec §7). Cola: TODO-33 (parpadeo) · TODO-35 (A2b/eventos/HUB). **Regla**: `arquitecto-software` SIEMPRE · **Bersaglio = 100% COP, cero dólares (§127)** · flujo COMPLETO **W-11** cuando toque diseño/decisión cara · `[[feedback_workflows_acotados]]` · `[[feedback-reintentar-agentes-no-saltar-flujo]]`.
