...
>
> **🔴🚦 FRENTE VIVO (relevo): TODO-40 MODELO INVENTARIO — Fase 1 EN CURSO.** **Backend COMPLETO+verde: B1 migración + B2 reglas + B3 CFs** (commits+tests en la fila TODO-40 ↓). **Siguen B4 admin UI · B5 SSG+cliente · B6 verif E2E** (capa de presentación; ⚠️ gate de deploy en la fila). Precede la carga masiva.
...
| TODO-40 | **MODELO INVENTARIO MULTI-TIPO** (Decisión Fuerte, PRECEDE la carga masiva). Diseño v3 §12 (comité ×4+Daniel+Gemini+comité v2 ×5). **F1 BACKEND COMPLETO+VERDE**: **B1** migración (`a45d44a`, `js/admin/inventario-model.js` modelo puro enum3 `finito`/`finito_refabricable`/`encargo`+`derivarEstado`+invariantes; runner `migrate-inventario-v3.mjs`; 17t) · **B2** reglas (`f255141`, enum3+`visibilidad`+`cantidad` CF-only por IGUALDAD update / permitida create + read blindado fail-closed; 212t) · **B3** CFs (`e45ef00`+`78c6ffe`: `crearPedido` decrementa+estado derivado+ledger `pieces/{id}/movimientos`; `ajustarStock`/`cambiarTipoPieza`; 19+9 int). **Siguen (UI/SSG): B4 admin** (`piezas.js`/`.html`: select enum3+visibilidad, QUITAR cantidad del save, botón reabastecer/ajustar→CF `ajustarStock`) · **B5 SSG+cliente** (`generate-pieces` filtra privadas+PreOrder refab; query pública `where visibilidad=='publica'`) · **B6 verif E2E**. ⚠️ **GATE DEPLOY**: migración `--apply` + B5 (query filtra) ANTES de reglas (si no, catálogo público se oculta). | 🟡 | F1 B4-B6 |
...
>
> **▶️ Vivo**: TODO-40 Fase 1 (inventario v3). **Backend ✅** B1 migración+B2 reglas+B3 CFs (`a45d44a`/`f255141`/`e45ef00`/`78c6ffe`; 17+212+19+9 tests). Siguen B4 admin UI → B5 SSG+cliente → B6 verif E2E (§12.3). Crudo+síntesis → bóveda `2026-06-26-*inventario*`.
