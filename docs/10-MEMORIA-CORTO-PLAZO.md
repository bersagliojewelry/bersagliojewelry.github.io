...
>
> **🐛 BUG (Daniel, URGENTE) → FIX §99** (`0850847`): flash imagen VIEJA→nueva en **Nosotros** al cambiar la portada = doble-paint (provisional→Firestore, línea 462 vs 466); gate `_siteReady` (no pinta `<img>` provisional, fondo reservado hasta confirmar). Verif. navegador (sin img provisional); **flash real → confirmar Daniel en prod**. **PENDIENTE: extender el gate a Contacto (hero) y Home (atelier/editorial)** —mismo patrón— TRAS confirmar que resuelve. → L-44.
>
> **▶️ HECHO ESTE TURNO** (orden de Daniel: fuentes → F5 → shard → auditoría → F2): **§94 fuentes VF** + **§95 F5 borrado-colección YA EN MAIN** (`origin/main`=`cee5a85`, Daniel mergeó, L-26). **PENDIENTES push+merge**: **§96 shard `30`→`31-LECCIONES-FIRESTORE`** (`cbfea97`; descubrimiento kernel→M-06) · **§97 auditoría semántica** (`51d3bcc`; HA-01 estado-git corregido) · **§98 F2 colas mudas CRM** (`856f913`: 18 listeners → `subscribeWithRetry` robusto en `core/live-query.js`). **PRÓXIMO**: (1) **arranque C1 DIAGNOSTICADO** (PERF-07: `body opacity:0` hasta `bj-ready` + hero JS-rendered + `await loadShell` bloquea el hero con 6 drawers no-críticos; opciones A=reorden boot / B=SSG hero / C=aligerar fade) → **implementación = TURNO APARTE** (Decisión Fuerte §G.2: comité blindado + 2ª opinión Gemini [ya recomendó B] + medición en DISPOSITIVO + aprobación Daniel; NO medible headless, L-05); (2) responsive fino device-driven. `[[feedback_workflows_acotados]]`. [OPUS-4.8]. **PRÓXIMO** (orden pedido por Daniel: mantenimiento cerebro → F2): (1) **auditoría semántica del cerebro** (skill `auditoria-cerebro`, vencida: 13 ADRs nuevos > 12); (2) **F2-CRM colas mudas** (`crm-service.js` ~11 listeners sin error-cb, dinero → `subscribeWithRetry`; verif. experta); (3) **arranque C1** (estructural, Decisión de Daniel); (4) responsive fino. `[[feedback_workflows_acotados]]`. [OPUS-4.8].
