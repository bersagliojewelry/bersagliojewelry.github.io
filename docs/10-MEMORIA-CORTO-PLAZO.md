...
>
> **▶️ RETOMA — TODO-28 F4 PERF (fluidez móvil)**: comité ×3 blindado + consejo Gemini (convergen) → **IMPLEMENTADO + push `Desarrollo`** (código `d433b25` + cerebro; Daniel mergea). Diagnóstico: header `backdrop-filter blur(46px)` = costo POR CUADRO al scroll (H1) + aurora animada debajo lo re-blurea aun en reposo (H2) + arranque por JS (H3); `header.js:196` onScroll ya passive. Solución `.bj-lite` por CAPACIDAD (boot.js §C3: deviceMemory≤4/saveData/coarse/≤920/RM, defensivo): header blur 46→16, aurora/blobs/dock `animation:none` en equipos modestos; **capaces IDÉNTICOS** + `content-visibility` wishlist/journal. **FALTA**: Daniel valida en celular real (`192.168.1.2:5173`/gemelo) → merge; fuentes (invisible); arranque C1 (estructural, aparte). **Lección**: workflows pesados sin tope se desbocan (1ª auditoría 2h30/4.7M → DETENIDA) → blindar (`[[feedback_workflows_acotados]]`). Detalle → `45` PERF-04 + bóveda `2026-06-21-perf-fluidez-movil-comite-v4.md`. [OPUS-4.8].
