...
>
> **🧭 FOCO = PLAN ÚNICO ERP v4 (TODO-68, Daniel 2026-07-04: "un solo sistema, tú defines el orden")** — SSoT: spec `2026-07-04-plan-unico-erp-v4`. **F1 COMPLETO: PUENTE ✅ (§165) + CORE ✅ (§167, backend `994776a` + UI `d12be6f` + gate E2E 3 flujos EN PROD 2026-07-06)**. Restan F1-extras que NO gatean: push FCM A.6 · badge sidebar. **EN CURSO: F2.0 caja/bóveda — diseño CERRADO+gateado; implementación TDD B0-B5. B0 reglas ✅ · B1 turnos ✅ · B2 enlace venta↔turno ✅ · B3 bóveda ✅ → PRÓXIMO B4 (Dual-Approval + alertas FCM).** Precios NO bloquean; monitorear 1ª APPROVED sigue vivo. `[[project_comercio_pagos]]`
...
> 2026-07-06 (Opus · **F2.0 DISEÑO CERRADO → implementación**): diseño CERRADO (comité ×3 + Gemini + reencuadre roles: `owner`=Kary · rol NUEVO `caja`=cajera · Daniel=dev; decisiones §8.6 = bóveda $0 · fondo $200k · limiteCajon $4M · Dual-Approval Kary) → **spec ejecutable SSoT `docs/superpowers/specs/2026-07-06-f2-0-caja-boveda-DISENO.md §8-§10`** (6 bloques TDD B0-B5; invariantes duros + CRUDOs comité/Gemini viven allí). **✅ B0-B3 HECHO** (sin deploy; detalle+evidencia por bloque → spec §10 checklist): **B0** reglas (test:rules 233·`movsCaja`→**L-73**) · **B1** turnos por puntero singleton (`caja-core.js`→**L-74**) · **B2** `crearPedido` enlaza turnoId (atomicidad #5·`enforceTurno` OFF-default·cota `docsDelTurno`) · **B3** bóveda `registrarTraslado/reverso/recalcBovedaCore` (recompute-no-incrementa · reverso inmutable §9.3 · checkpoint acota · guards monto/saldo). Verde: caja.integration **23/23** + pedidos **24/24** + wompi **16/16** (flake concurrente→**L-76**; contador→**L-75**). **PRÓXIMO B4** (Dual-Approval ajustes/reversos + alertas FCM owner §9.1/§9.8). ⚑ `enforceTurno` OFF → cero impacto prod hasta runbook §9.4. L-72 residual `d244d77`. Pend F1: push A.6. ⚠️ Runbook siembra root.
