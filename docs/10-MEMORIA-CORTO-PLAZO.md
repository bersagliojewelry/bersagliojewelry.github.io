...
>
> **▶️ RETOMA = §103 F2.1 (router falso-SPA), sesión dedicada — TODO-30**. **F2.0 (View Transitions cross-document) HECHO** (§107: cross-fade entre páginas, sin flash blanco; ~80% de la molestia; cache v27, `liquid-glass.css`). **F2.1 = nav INSTANTÁNEA real** (intercepta cruce de shells, `fetch`+swap `<main>` en memoria + prefetch) → exige **`destroy()`/teardown en CADA page-handler** (listeners/onSnapshot/observers, §3.5 anti-zombi; hoy NO lo tienen) + CSS/scroll/atrás-adelante/drawers/forms/SEO → **comité de casos borde + gate**. Hacerlo **SOLO si Daniel siente que VT no basta**. Reactivar render LQIP (§106) con la caché-memoria del router. Ground-truth §103.2/.5 + §107. Boot §G.1 + leer §103/§107.
>
> **▶️ 2026-06-23** ([OPUS-4.8]): §100-§103 (cero-demo · gate empírico · carga fluida · "app-like" DECIDIDA) → 99. **§104 F1 LQIP + §105 fix recarga `reveal.js` = EN PROD** (PR #306/#307). **§106 REVERT del render LQIP**: hacía vacío→borroso→nítido (3 estados, peor); el LQIP llega con el getDoc, no antes → SUMA estado. Vuelto a neutro→foto; fontanería conservada para F2. **§107 F2.0 = View Transitions cross-document** (cross-fade entre páginas, sin flash; cache v27; router intacto). → 99 §104..§107 · L-47..L-51/M-07. [[feedback_no_demo_en_index]].
>
> **🚦 COLA REAL — gated/pendiente**: (1) **§103 F2: F2.0 View Transitions ✅ §107; F2.1 router falso-SPA = TODO-30** (nav instantánea real + reactiva render LQIP §106 con caché-memoria; exige teardown en page-handlers; **solo si VT no basta**; F3 SSG opcional); (2) **al reactivar LQIP**: backfill/re-subir imágenes existentes (las pre-F1 no tienen LQIP); (3) **alinear cerebro de cars** (sesión dedicada); (4) borrado auto imagen Storage (irreversible); (5) C1 (PERF-07); (6) responsive; (7) limpiar CSS muerto + `/img/*` demo; (8) re-verificar catálogo/pieza CON piezas cuando Kary cargue. **Regla**: `arquitecto-software` SIEMPRE. `[[feedback_workflows_acotados]]`. [OPUS-4.8].
