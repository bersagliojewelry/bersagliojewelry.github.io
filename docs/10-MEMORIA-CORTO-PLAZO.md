...
>
> **▶️ RETOMAR AQUÍ — ACUERDOS R6 = NO-GO: falta cerrar 1 bug de dinero (2026-06-13)**. R1-R5 en `main` (PR #239) pero GATEADO/INERTE (`acuerdosActivos` OFF). Comité POR HITO ×2 (1ª degradada por rate-limit→GO FALSO; re-corrida=NO-GO). **Censo prod LIMPIO (0 acuerdos)** · emulador 144/144.
> - **5/6 fixes hechos+commiteados** (`Desarrollo`, [OPUS-4.8], c/u con test+verde): BLOCKER gate corte `6d127ba` · MEDIUM knob panel `bff9972` · MEDIUM `acuerdosLargos` por movId cubierto `8fd7ca3` · LOW UI cancelar+onLine `c7dd7ca` · LOW test paridad `3cb5970` · script encendido tipado+censo `e40e004`. Gap menor diferido: `acuerdoAlCorte` para plan honrado (deudaCubierta≤0).
> - **🔴 HIGH ABIERTO (bloquea R6)**: el clamp `pagado` aún oculta mora con **abono parcial real + cuotas infladas** (Σcuotas>deudaCubierta). Bug A8 REPRODUCIDO (factura 200k+abono 100k pre-pacto+cuotas Σ200k → CON acuerdo vencido 0 vs SIN 100k). `be342aa` solo cerró el caso 100% fabricado — NO revertir (mejora conservadora, nunca esconde MÁS). **Causa raíz** (🛰️ Decisión Fuerte, NO apurar): el modelo `pagado=Σcuotas−deudaCubierta` asume Σcuotas==deuda-al-pactar(D0), no validado; A8 ≡ test honesto line78 salvo el TIMING del abono. **Fix** = `pagado=clamp(D0−deudaCubierta,0,Σcuotas)` con D0=deuda cubierta AL PACTO (FIFO pact-time con créditos pre-pacto = verdad de servidor) + corregir fixtures (line78 usa timestamp pre-pacto por descuido) · complemento: rechazar Σcuotas>saldoAlPactar en validador+reglas+UI. → **candidato CONSEJO EXTERNO** (`docs/15`).
> - **Tras cerrar el HIGH**: re-verificar → ADR §81.9 + archivar CRUDOs a bóveda → **R6 con Daniel** (merge a main [SIN bump SW: HTML network-first + JS rehasheado] + `firebase deploy --only firestore:rules,firestore:indexes,functions` + `node functions/encender-acuerdos.mjs --aplicar` + prueba en vivo navegador owner · `git fetch` L-26). **Lecciones→`30`**: comité degradado=GO falso (re-correr; partial-fail≠"pasó"); re-verificar fixes atrapa fixes incompletos. **CRUDOs**: tasks/w71voltb9 (GO falso)·wh5hsk9is (NO-GO 15)·wno2hr1mj (re-verif A8).
