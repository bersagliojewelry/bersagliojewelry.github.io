...
| TODO-49 | **Legal e-commerce** (prereq Wompi). **§157 HECHO+DESPLEGADO**: retracto Art.47/reversión Art.51/garantía legal/SIC + privacidad (consultas10/reclamos15, retención, Encargado) — válido hoy. Identidad RESUELTA (Kary=la comerciante). **Consejo ×2 (ChatGPT+Antigravity) RECIBIDO+integrado §157.8** (claims de código verificados ✓; puente §03 ya corregido; aviso del cargo ya existe `carrito.js:311`). **IVA §02 (LEGAL-09) + identidad real publicada (LEGAL-08) ✅ DESPLEGADO §157.9** (Daniel autorizó). **RESTA solo al ENCENDER Wompi** (→`42-LEGAL §5`): persistir consentimiento (LEGAL-10, code+deploy functions) + reescribir §03 "web cobra online" + RUCOM (origen mineral) + abogado. `LEGALES/` blindada. | 🟡 | encender Wompi |
