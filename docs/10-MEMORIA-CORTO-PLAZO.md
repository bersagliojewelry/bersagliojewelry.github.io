...
>
> **▶️ RETOMA (chat nuevo)**: arrancar **§103 Fase 1 Blurhash** (ground-truth + plan + archivos en **ADR §103**; toca `firestore.rules`+modelo+`image-optimizer`+`singleton-admin`+render → test emulador + gate empírico) → luego F2 router falso-SPA. Boot normal §G.1 + leer §103. Resto de cola ↓.
>
> **▶️ 2026-06-23 — sprint web EN PROD** ([OPUS-4.8]): **§100** cero-demo + móvil (L-45); **§101** proceso Decisión Fuerte → **GATE EMPÍRICO**; **§102** carga fluida (placeholder verde saturado → neutro invisible, patrón `featured`; L-46). **Barrido QA público** (caza-bugs 7 páginas × estados-cero × móvil) = **LIMPIO**. **§103 carga "app-like" DECIDIDA** (workflow completo: comité×3 + Gemini, bóveda; Gemini superó a mi comité): **router falso-SPA + caché-memoria + prefetch-hover + Blurhash en Firestore**; CERO SWR-localStorage (flicker) ni prerender (costo); GO de Daniel. **Implementación PENDIENTE** (decidir≠implementar §101). [[feedback_no_demo_en_index]].
>
> **🚦 COLA REAL — gated/pendiente**: (1) **IMPLEMENTAR §103 carga app-like, FASEADO + gate empírico** (F1 Blurhash: toca `firestore.rules`+modelo+`image-optimizer`+`singleton-admin`+render → test emulador + deploy manual; F2 router falso-SPA: toca TODA la nav → comité #2 casos borde; F3 opcional SSG-bake) → **sesión fresca**; (2) **alinear cerebro de cars** (sesión dedicada); (3) borrado auto imagen Storage (irreversible); (4) C1 (PERF-07); (5) responsive; (6) limpiar CSS muerto + `/img/*` demo; (7) re-verificar catálogo/pieza CON piezas cuando Kary cargue. **Regla**: skill `arquitecto-software` SIEMPRE al construir. `[[feedback_workflows_acotados]]`. [OPUS-4.8].
