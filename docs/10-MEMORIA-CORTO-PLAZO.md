...
>
> **▶️ 2026-06-23 — reconciliación de relevo**: el relevo previo listó como "PRÓXIMO" 2 tareas que ese día ya cerró (auditoría=§97, F2-CRM=§98). **Meta-lección**: cruzar el relevo vs `deepAudit`+ADRs del turno antes de cerrar.
>
> **▶️ 2026-06-23 — §100 cero-demo + §101 proceso** ([OPUS-4.8]): **§100 EN PROD verificado en vivo** (`36da485`+`38d015a`): el flash de Nosotros era un **fondo CSS demo** (no el doble-paint del §99, que se revirtió); 3 demos CSS + `FALLBACK_IMG` → superficie de marca; móvil 1×2→340×425; cache v22; Daniel conserva banner-hero/gema. **L-45** (corrige L-44). **§101 CERRADO**: veredicto del proceso de Decisión Fuerte (Gemini doble-ciego ≈ reinventó el pipeline = validado); único cambio: Paso 7 → **GATE EMPÍRICO con Pruebas de Estado** (skill `proceso-decision-fuerte`). [[feedback_no_demo_en_index]] · [[feedback_consejo_externo_readonly]].
>
> **🚦 COLA REAL — gated en Daniel**: (1) **borrado auto de imagen vieja de Storage** (feature aparte: diff URLs + `deleteObject` solo tras `setDoc` OK; IRREVERSIBLE → gate empírico); (2) **arranque C1** (PERF-07, Decisión Fuerte); (3) responsive fino. Housekeeping: borrar archivos `/img/*` demo (sin refs en prod). `[[feedback_workflows_acotados]]`. [OPUS-4.8].
