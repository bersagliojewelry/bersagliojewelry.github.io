...
>
> **▶️ 2026-06-23 (cont.) — §100 cero-demo en multimedia** (`36da485`+merge `38d015a`, **EN PROD verificado en vivo**): el §99 (flash Nosotros) tenía **RCA ERRADA** y su fix se REVIRTIÓ. RCA real (verif. navegador real): el "foto vieja" era un **fondo CSS demo** (`.abt-hero-image-bg`→earrings); el `_siteReady`/`withoutImages` del §99 lo empeoraba. Auditadas todas las secciones: 3 fondos CSS demo (earrings/ring/model) + `FALLBACK_IMG` categorías → superficie de marca (cero-ficción). Móvil 1×2→340×425 (`+width:100%`). Cache v22. Daniel **conserva** banner-hero/gema (marca). Lección **L-45** (corrige L-44). [[feedback_no_demo_en_index]].
>
> **🚦 COLA REAL — gated en Daniel**: (1) **§101** — Daniel corre el prompt Gemini (`2026-06-22-consejo-externo-proceso-prompt.md`) → veredicto + cierra `proceso-decision-fuerte`; (2) **borrado auto de imagen vieja de Storage** (feature aparte: diff URLs + `deleteObject` solo tras `setDoc` OK; IRREVERSIBLE → gate empírico, NO con contexto saturado); (3) **arranque C1** (PERF-07); (4) responsive fino. Housekeeping: borrar archivos `/img/*` demo (sin refs en prod). `[[feedback_workflows_acotados]]`. [OPUS-4.8].
