...
> 2026-07-17 · **[OPUS-4.8] Auditoría GSC (TODO-35)** — **9 indexadas / 27 "rastreada: actualmente sin indexar"** (= juicio de VALOR de Google, NO bloqueo técnico: las rastrea a diario; casi todas `/pieza/*`) · 2 noindex (esperado) · 0 404. Sitemap **REENVIADO 17jul** (leído 10jul=37 URLs; hoy 52). Producto: 17 no válidas — falta `price`: **NO bloquea indexación, solo el rich snippet** (verificado docs Google); review/rating = opcionales → **NO inventar** (TODO-48). CWV sin datos (poco tráfico). **L-nueva**: "solicitar indexación" pieza-por-pieza NO sirve contra "rastreada sin indexar" (ya la rastreó; no es descubrimiento). Palancas: GBP(B2)·precios·reseñas·enlaces externos·Journal.
> 2026-07-17 · **[OPUS-4.8] Journal ✅ §186** (visual+CMS colocación, v95) **· §187** (indexable `/journal/<slug>.html` + Article, v96) — prod verificado.
