...
| TODO-28 | **Correcciones web** (Daniel 2026-06-21 navegando prod; plan 5 fases + 4 decisiones → recon 7 ag. en bóveda). **F1 quick-wins ✅** (FAQ Nosotros path muerto · `/journal.html` vacío no crashea · will-change permanente fuera — `efeb13c`, verif. navegador); pend. entrada falso-404. **F2** live-sync: re-suscribir listener público que muere sin reconectar (`firestore-service.js:272`, transversal a TODO lo live) + colas mudas CRM (L-40). **F3** CMS: foto Editorial home + Atelier + FAQ Contacto editables. **F4** perf: `.glass` blur fuera de listas N (catálogo/wishlist/admin). **F5** comité+consejo: borrado-colección = **bloquear si tiene piezas** (decidido) + carga/orden/motion copiando cars+insema **mejorado**. | 🟡 F1 hecho | sigue F2 |
...
> **2026-06-21 · CORRECCIONES WEB (TODO-28)**: recon 7 ag. (bóveda) → 5 fases; F1 quick-wins commiteado+verificado (`efeb13c`). Sigue F2 live-sync. [OPUS-4.8].
