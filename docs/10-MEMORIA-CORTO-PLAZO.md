...
| TODO-35 | **Visibilidad SITE-WIDE** (SEO·AEO·GA4·GSC·Maps·SSG). Spec `2026-07-10-visibilidad-seo-aeo-geo-DISENO.md` (47 findings). **A1 ✅** titles/meta · **A2 ✅ [§184]** 9 landing de faceta (/coleccion×5 + /gema×4; deploy `922fabc`; prod verificado curl; hidratación visual = ambiental L-05/Chrome-automatización). **A3 ✅ [§187]** 6 guías horneadas `/journal/<slug>.html` (Article; sitemap 52). SIGUE: **B2 GBP** (mayor palanca local) · **A4** geo/FAQ (pide lat/lng del pin) · A5 feed ⏳precios. **Pend GSC**: pedir indexación de las 6 URLs del journal. Pend previo: eventos `generate_lead`/`contact` + 2º flujo GA. Revisar GSC ~3-7d (¿7 URLs indexadas? cascada). 🔑 Google authuser=3. | 🟢 | B2/A4/A3-cont |
...
> 2026-07-11 · **[OPUS-4.8] Journal ✅ pulido visual + CMS colocación (§186 · SW v95 · merge `0b6ef72`)** — capital 54px/1-col + `.hj-trio` centrado; campos `cover`(portada)/`order`+`featured`=destacada-franja; panel miniatura/orden/casillas; reglas `journalValid` DESPLEGADAS (manual); 6 docs migrados (portada Cómo-elegir + 3 destacadas Muzo/Oro/Anillo + orden 1-6). Tests 6/6 + reglas 243/243 + composición determinista real ✅; CSS live prod.
> 2026-07-11 · **[OPUS-4.8] SEO A3 ✅ Journal indexable (§187 · SW v96 · merge `84f2139`)** — `/journal/<slug>.html` horneadas (schema Article + cuerpo en noscript + `PRERENDERED_ENTRY_SLUG`); `entrada.html?e=` sigue vivo (fallback) pero noindex; hub y sitemap → horneadas (**52 URLs**, +6 con lastmod real); `entryHref()` = SSoT del enlace; bug og:image absoluto corregido. SSG_SELFTEST + horneado real + **prod verificado** (200/index/canonical/Article). **PEND**: pedir indexación de las 6 URLs en GSC (authuser=3) + revisar cobertura ~3-7d.
