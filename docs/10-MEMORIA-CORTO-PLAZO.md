...
>
> **▶️ RETOMAR AQUÍ — ACUERDOS DE PAGO **v2** (Consejo Externo integrado; REWORK pendiente)**. El Consejo (Gemini) DEMOLIÓ la v1: hueco de fraude (jineteo) + sobre-ingeniería. **Adoptados 4 hallazgos** (síntesis en bóveda `…consejo-externo-acuerdos-respuesta.md`; spec reescrita a **v2**): (1) **mutex `acuerdoVigenteId` en el doc del cliente** (cierra el acuerdo SUELTO); (2) **solo `alcance:'saldo'`** (el FIFO global no atribuye por factura); (3) **matar TRAMOS → escudo de 2 estados** (al-día/perforado revive vencimientos originales); (4) **cristalizar `estadoAlCorte`** en el corte (DIAN). **La implementación v1 (slices 1-5) está SUPERSEDED** — en el árbol pero GATEADA/inerte (`acuerdosActivos` off). **REWORK (spec §1.10)**: **R1 fórmula escudo ✅ HECHO** (commit `6a7ba3f`; 15/15+insumos 3/3+paridad+suite vieja intacta). **Sigue: R2 reglas mutex `acuerdoVigenteId`** (matar la rama 'factura' de `acuerdoValido`; el puntero en el doc del cliente con transición validada por getAfter; quitar `acuerdoMaxSinAprobacion`/detector `solapados`) → R3 corte (cristalizar `acuerdoAlCorte`) → R4 UI saldo-only (el toggle cuotas + renegociar usan el batch con el puntero) → R5 detectores (sin `solapados`) → R6 deploy+bandera+verificación POR HITO+ADR §81. PR del tren cuando Daniel quiera (la bandera apaga todo).
