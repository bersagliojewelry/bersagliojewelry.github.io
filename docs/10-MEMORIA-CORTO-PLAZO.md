...
| TODO-73 | **POS-pro money-model (Daniel 2026-07-08, Decisión Fuerte, TDD)** — SSoT spec `docs/superpowers/specs/2026-07-08-pos-pro-datafono-servicios-DISENO.md` (**comité ×3 ✅ §8** · decisiones dueño ✅ §9). Alcance (detalle → spec): datáfono · vouchers al cierre (no bloquea) · venta sin pieza · **pago dividido `pagos[]`**. **CORE ✅ EN TDD** (`d6a5728`: f2-3 16/16 + no-reg f2-2/caja/saldo/avanzar/codigo/wompi verdes; SIN deploy, backward-compat; wrapper reenvía por spread). **Pend: UI** (`pos.js`/`admin-pos.html`: constructor de pagos + datáfono + cierre con conteo de vouchers [monto≠cantidad; exigir si hay datáfono] + botón venta-sin-pieza [flag `_sinPieza`]) → deploy `functions` + gate holístico → ADR §178. Interinato: NO se tocó webhook/firma/reaper/snapshot. | 🟡 | UI pend. → deploy+gate |
...
> 2026-07-08 · **F2.2 cerrada (§176)** (deploy+gate prod verde). **Gotcha**: `test:rules` falla si el `http-server` de altorra ocupa `:8080` (matar PID).
> 2026-07-08 · **Correcciones (Daniel)**: validación Chrome no-holística → 4 errores visibles (lección `[[feedback_validacion_chrome_holistica]]`). **TODO-72 UX ✅ (§177)** + **TODO-73 money-model: diseño+comité×3+CORE TDD ✅** (`d6a5728`, f2-3 16/16+no-reg verde, SIN deploy) · UI pend. → Daniel trae ajustes en sesión nueva.
