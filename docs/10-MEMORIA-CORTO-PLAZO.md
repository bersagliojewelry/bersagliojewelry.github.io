...
> - **Fase 3 (CRM)**: **Bloque 1 (Fundamentos) CONSTRUIDO Y VERIFICADO** ✅ (ADR §42, 2026-06-06). Backend del núcleo de cuentas por cobrar: rol `vendedora` + RBAC (clientes/movimientos append-only/solicitudes/config) + 54 tests verde en emulador local. Endurecido con revisión adversarial (7 huecos corregidos). Diseño: `crm-cuentas-design.md`. Charter: `50-ARQUITECTURA §3`+§5.
>
> **▶️ RETOMAR AQUÍ — Bloque 2 (Saldo)**: Cloud Function `onWrite` sobre `clientes/{id}/movimientos/{movId}` que recalcula `saldoActual = Σ(cargos no anulados) − Σ(abonos)` en transacción idempotente (spec §4). **Decidir ahí** el signo de `ajuste`/saldo a favor (hoy las reglas exigen `monto>=0`; el signo lo da el `tipo`). Luego bloques 3-6 (Panel Kary → App vendedora responsive → Migración → Reportes). **Prerrequisito YA resuelto**: `npm run test:rules` corre local (JDK 25 Temurin, ver `30 §L-12` para el comando). NO toca producción hasta merge con OK de Daniel. Decisión vigente: rol en `users` doc (custom claims S4 = optimización futura).
>
...
| TODO-09 | **Fase 3 — CRM**: **Bloque 1 (Fundamentos) ✅ HECHO** (ADR §42: rol vendedora + reglas RBAC + 54 tests). Siguiente: **Bloque 2** (CF de saldo). Charter `50-ARQUITECTURA §3`+§5. `crm-architect`+`ecommerce`. | 🟡 En curso (Bloque 1 ✅) | Bloque 2 = Cloud Function |
