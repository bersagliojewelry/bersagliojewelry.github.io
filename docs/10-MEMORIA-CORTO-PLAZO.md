...
> Podada (GC) 2026-06-22. Histórico completo → **ADR §37-§99** (mapa en `00`→detalle `99`; lecciones L-/M- en `30`/`31`).
>
> **🐛 BUG (Daniel, URGENTE) → FIX §99** (`0850847`): flash imagen VIEJA→nueva en **Nosotros** al cambiar la portada = doble-paint (provisional→Firestore, línea 462 vs 466); gate `_siteReady` (no pinta `<img>` provisional, fondo reservado). Verif. navegador; **flash real → confirmar Daniel en prod**. → L-44. **Alcance extensión (verificado)**: **Home `editorial.js`** (`c.image`) = mismo patrón → PENDIENTE gate (home es modular; el hero usa banner estático = NO gatear, LCP). **Contacto** hero = solo texto, sin imagen → N/A. **PENDIENTE BORRADO auto de imagen vieja** (Daniel SÍ lo quiere): el `save` del singleton NO borra huérfanas de Storage → implementar diff de URLs + `deleteObject` SOLO tras `setDoc` OK (best-effort). **IRREVERSIBLE → gate empírico (prueba con Storage real), NO improvisar con contexto saturado** (proceso §100).
>
> **🧭 PROCESO DE VALIDACIÓN (Daniel 2026-06-22) → skill `proceso-decision-fuerte`**: pipeline para Decisión Fuerte (triaje por riesgo → reviso → comité acotado → Gemini anti-anclaje → análisis crítico verificado → **GATE EMPÍRICO** → revalidación proporcional → veredicto + captura). Comité ×4 HECHO (CRUDO bóveda `2026-06-22-proceso-validacion-comite-CRUDO.md`); **prompt Gemini listo** (`...consejo-externo-proceso-prompt.md`) → **Daniel lo corre** → análisis crítico + veredicto → **ADR §100**. Hallazgo clave del comité: a la propuesta le faltaba VERIFICACIÓN EMPÍRICA (5 capas de opinar, 0 de probar). [[feedback_workflows_acotados]].
>
> **▶️ ESTE TURNO** → ADRs §94-99 + PERF-07, **TODO EN MAIN** (`origin/main`=`8032d1b`, Daniel mergeó; árbol limpio): fuentes·F5·shard·auditoría·F2·C1-diag·flash-Nosotros. **RETOMA (sesión fresca)**: (1) **Daniel trae respuesta de Gemini** (`...consejo-externo-proceso-prompt.md`) → análisis crítico verificado + veredicto → **ADR §100** + cerrar el proceso `proceso-decision-fuerte`; (2) **borrado-imagen + flash-Home** con el GATE EMPÍRICO (Storage real, no headless); (3) **arranque C1** (PERF-07, Decisión Fuerte). `[[feedback_workflows_acotados]]`. [OPUS-4.8]. **PRÓXIMO** (orden pedido por Daniel: mantenimiento cerebro → F2): (1) **auditoría semántica del cerebro** (skill `auditoria-cerebro`, vencida: 13 ADRs nuevos > 12); (2) **F2-CRM colas mudas** (`crm-service.js` ~11 listeners sin error-cb, dinero → `subscribeWithRetry`; verif. experta); (3) **arranque C1** (estructural, Decisión de Daniel); (4) responsive fino. `[[feedback_workflows_acotados]]`. [OPUS-4.8].
