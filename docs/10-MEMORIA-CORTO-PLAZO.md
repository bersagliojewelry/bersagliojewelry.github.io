...
| TODO-25 | **Capacidad CAZA-BUGS** (Daniel 2026-06-21, tras §89): (1) reflejo barato siempre-on — al tocar/rozar un subsistema verificar su comportamiento END-TO-END, no solo mi cambio; (2) escalar bugs no-triviales/caros a workflow adversarial + comité + consejo externo + skills (§3.6/§3.7 calibran: NO comité para lo trivial). Cross-repo: **skill portable** (global ×4 repos) + reflejo §G vía cars-operador (L-31) + receta `60-WORKFLOWS`. **Autocrítica §89**: estuve EN `categories.js` y lo dí por OK con lente cero-ficción sin probar el camino vivo (formalizar como meta-lección al hacer el shard de `30`). **Documentar en los 4 repos.** | 🔲 | scope Daniel (ahora/sesión nueva) |
...
> **2026-06-20** (sesión "continua"): **ÍNDICE 100% CERO-FICCIÓN CERRADO** (§88) — Fase B (Videos/Redes + reglas + UX Kary; review 4 ag.; PR #272 v18 prod) + href Redes (`8abaab4`) + Destacadas hide-when-empty (`a566dc5`). Reglas desplegadas+verificadas (censo prod vacío). SW v20. [OPUS-4.8].
