...
>
> **▶️ FOCO ACTIVO — CMS WEB PÚBLICA (Kary, urgente)**: contenido público administrable desde el panel. **Specs (leer al reanudar)**: `2026-06-14-cms-web-publica-design.md` (P0-P1.3) · `2026-06-14-cms-gran-plan-dinamismo-design.md` (gran plan + consejo Gemini integrado: SSG build-bake, CSP `<meta>`, storage rule claim ROL, roadmap P0→P5) · **`2026-06-15-cms-edicion-visual-wysiwyg-design.md`** (NUEVA dirección ↓). Deliberaciones → bóveda. **HECHO Y MERGEADO a main** (PRs #243-#246; build verde · 202/202 · **163/163 reglas**): journal E2E · P0 bloqueantes (`c257dca`) · B3-categorías no-demo · **P1+P2 textos**: motor singleton + **Home 100% editable** (hero/editorial/atelier/cta) + **Contacto** (hero/proceso/faq) — `c98f77d`/`905330c`/`95c5b1f`/`ba46db7`. Reglas siteContent **por-página**. Prod 0 → todo baked. **Preview WYSIWYG** (comité 12 ag.): split **preview vivo + clic-para-editar** reusando `heroInner`/`data-sf`; NO lienzo/`contentEditable`/drag (MVP=F1 Home). **✅ Consejo Gemini INTEGRADO**: **iframe-`srcdoc` en F1** (revierte el comité; `admin-contenido.html` no carga el CSS público→`<div>`=bleed inverso; bóveda `…-consejo-gemini-wysiwyg-SINTESIS.md`) + límites de caracteres + presets schema-idéntico + **XSS `colecciones.js:53` ARREGLADO** (commit `1fcd8c2`, pusheado). **▶️ REANUDAR**: (1) **F1 preview vivo iframe-`srcdoc` CONSTRUIDO** (split form|preview de las 4 secciones de texto del Home, debounce 180ms, botón Descartar; `live-preview.js` [iframe sandbox, lee el CSS público de `/` en dev+prod] + `home-preview.js` [reusa heroInner/editorialInner/atelierInner/ctaInner exportados] + `singleton-admin.js` split; build verde). **FALTA: smoke con LOGIN (Kary/Daniel — el panel es `requireAuth`) + límites de caracteres + extender a Contacto**; (2) P2 texto: **nosotros** (~11 secc.) + **global** (datos contacto) + **UNDO** (snapshot en audit log) + `hideWhenEmpty`. (P0.5 SEED coord. Daniel · films/social P3 · imágenes P3.5 · sections[] P4 · usuarios/SPA P5 → spec.) **Lecciones→`30`**: `.innerHTML`→`mount()` · falso positivo hook exec de RegExp→`String.match()` · núcleo PURO `-core.js` · `singleton=getDoc cacheado / lista=colección lazy`. **Pendiente real**: `firebase deploy --only firestore:rules,storage` + cache v16→v17 (¿ya corrido? verificar).
>
