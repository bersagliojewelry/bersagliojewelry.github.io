...
>
> **🧭 FOCO = PLAN ÚNICO ERP v4 (TODO-68, Daniel 2026-07-04: "un solo sistema, tú defines el orden")** — SSoT: spec `2026-07-04-plan-unico-erp-v4`. **F1 COMPLETO: PUENTE ✅ (§165) + CORE ✅ (§167, backend `994776a` + UI `d12be6f` + gate E2E 3 flujos EN PROD 2026-07-06)**. Restan F1-extras que NO gatean: push FCM A.6 · badge sidebar. **EN CURSO: F2.0 caja/bóveda — backend TDD B0-B4 ✅ COMPLETO → PRÓXIMO B5 (UI: POS caja + vista Bóveda + panel aprobaciones + wrappers onCall + fix L-72 + SW bump + deploy + gate Chrome). B5 = bloque frontend/deploy → buen límite para sesión fresca.** Precios NO bloquean; monitorear 1ª APPROVED sigue vivo. `[[project_comercio_pagos]]`
