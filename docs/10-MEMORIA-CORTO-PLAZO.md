...
| TODO-34 | **Auditoría pág. PIEZA** (Daniel 2026-06-25). Fixes demo-críticos ✅. · ✅ **"También podría gustarte"** (trap `?p=` viejo → `10a26bf` URLs limpias; verif. vivo). · ✅ **REDISEÑO ficha "Carta Gemológica"** implementado+verificado LOCAL (Chrome 11 y 2 specs): `buildSpecs` agrupado + `.pz-ficha` + gema **octágono talla-esmeralda c/ lustre** (`emeraldGemSVG`, antes hexágono plano) + 2-col sin líneas + sello pastilla + **galería alineada a info** (quitado `sticky` roto por `body{overflow-x:hidden}`, delta 0); tokens reales Bersaglio, cero-demo, **cache v32**; guía Claude Design handoff (`~/Downloads/design_handoff_carta_gemologica/`, NO mirror, adaptado). Commits en Desarrollo `[OPUS-4.8]`. ⚠️ **5 piezas PRUEBA LIVE en prod** (`zzz-prueba-*`, validación Daniel — **BORRAR EN EL MERGE**, override no-demo OK). Bug aparte: `/img/og-image.jpg` 404 (OG roto site-wide) → task `task_cbec3f7b`. **PEND**: merge (Daniel) + BORRAR pruebas + verif prod → **ADR**. · Tail: reglas `sizes`·slug·collection enum·precio·caché stale·lightbox·cart sin-precio. (SEO→TODO-35.) | 🟡 | merge + borrar pruebas + ADR |
