...
| TODO-62 | **Pinch-zoom móvil iOS — pantallazos BLANCOS + CRASH "No se puede abrir" + (regresión) carga lenta.** §156.17 (quitar backdrop-filter + `translateZ(0)`) **REVERTIDO** `bf9db59` (empeoró: carga lenta + crash persiste). **Nuevo RCA**: crash = MEMORIA al re-rasterizar el home en zoom (sin `content-visibility` + `filter:blur()` decorativos no reducidos en móvil). Flujo en curso; prod baseline §156.16. → L-62. | 🔴 | flujo→fix sin regresión |
