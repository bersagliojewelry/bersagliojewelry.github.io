...
>
> **▶️ Validación en vivo (Chrome directo, prod .co) 2026-06-24 — CAZADO el parpadeo**: el admin heredaba `@view-transition{navigation:auto}` de liquid-glass (admin.css `@import`) → sobre el `body display:none` hasta requireAuth (§115/L-57) ABORTABA (`InvalidStateError`) en cada nav = parpadeo. **FIX**: `admin.css` `@view-transition{navigation:none}` + SW v28. Confirma comité/Gemini ("VT empeora sobre latencia alta") EN VIVO. Red: 11 canales Firestore Listen + 0 reCAPTCHA (App Check reusa token IndexedDB → NO re-atesta cada nav, refuta a Gemini). Pendiente: re-verificar en vivo tras deploy + sonda (deploy del merge aún no vivo).
>
> **🚦 Próximo**: **SONDA construida** (`3e90733`, `js/core/perf-probe.js` gateada `?perf=1`; marks en firebase-config/auth + 3 páginas top: cuentas/dashboard/piezas → `console.table` del desglose por tramo). **Captura del baseline = necesita login admin en PROD** (dev salta App Check, no es representativo — comité): Daniel entra al panel con `?perf=1` y leo los números (o lo conduzco por Chrome conectado tras su login). Con el baseline → **Fase 0** (A3+esqueletos→A2→prefetch JS→fonts→VT al final). Lección a canonizar al medir/construir (prefetch-de-datos moot en MPA · `memoryLocalCache`≠persistencia Auth/AppCheck · VT cross-doc empeora sobre latencia alta) — pendiente nº por GC de `30` (hoy 98% cap). Detalle en bóveda. Después: contenido real web (TODO-07) · App Check (TODO-14). **Regla**: `arquitecto-software` SIEMPRE · `[[feedback_workflows_acotados]]`.
