...
| TODO-61 | **Rediseño "primera cara"** (Diseño/UI · W-11 §156: arquitecto+comité×4+Gemini+mockup+gate navegador REAL). **Hero ✅ DESPLEGADO** (foto-héroe real `images/hero.jpeg`→`hero-*` + tratamiento CLARO texto-izquierda/oscuro + velo blanco + 2 CTAs + microcopy; clutter oculto, CMS intacto; SW v55). **Pend**: (B) tops compactos colecciones/contacto (padding 104→~64, lead 2 líneas, h1 intacto) · (C) dedupe buscador colecciones (mover filtro a `.cat-controls`, quitar banda fea). Cierre → ADR §156 + crudo a bóveda. | 🟡 | tops + buscador colecciones |
...
>
> **2026-06-29 (rediseño primera cara · §156 · TODO-61):** W-11 (arquitecto+comité×4+Gemini+mockup+gate navegador REAL Chrome). **HERO ✅ DESPLEGADO**: foto-héroe real (`images/hero.jpeg`→`hero-*` optimizada 10-40KB) + tratamiento CLARO (texto izquierda/oscuro + velo blanco; NO oscuro = evita "Frankenstein", Gemini frenó el "acortar para que asomen destacadas" = fast-fashion) + 2 CTAs + microcopy confianza; clutter (locator/manifesto/firma) oculto, CMS intacto. SW **v55** (LCP `hero-1200.webp`). Verificado en navegador REAL. Pend: tops + dedupe buscador colecciones → ADR §156 + crudo al cerrar.
