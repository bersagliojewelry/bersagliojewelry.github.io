...
>
> **🚦 Frente activo: TODO-37 PLAN MAESTRO DE COMERCIO.** **B0+B0.5 ✅** (§120): WhatsApp directo en la ficha (CTAs sin-precio/con-precio/404 → WhatsApp con la pieza escrita) + GA4 `whatsapp_click` + helper puro `waLink`; cache **v34**; build+266 tests+enlace E2E verdes; **pend. MERGE Daniel**. B0 limpieza: piezas `zzz-prueba-*` ya removidas (`pieces` vacía, verificado). **SIGUIENTE: B1 (corazón, MOSTRADOR)** — `pedidos` por CF callable + stock atómico + caja/arqueo + bruto/neto (spec §11). _Cerrados recientes en prod: **TODO-34 ficha §118** · **TODO-36 grilla/recos/GA4 §119** (mergeados PR #356-358) · §117 Storage rol catálogo._ Cola: **TODO-33** (parpadeo menú) · **TODO-35** (A2b/eventos/HUB).
