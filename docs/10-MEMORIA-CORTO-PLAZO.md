...
>
> **2026-06-29 (cont.):** **TODO-58 CERRADO → ADR §154.** Slices 2 (links `/p/<código>`, 32 stubs SSG OG+canonical+noindex+redirect) · 3 (buscador en el INICIO, banda discreta; `.bc*`→`components.css` compartido) · 5 (404 recupera por código → `/colecciones.html?q=`). Verif: build · test:codigo 11/11 · SSG_SELFTEST · navegador (redirect/submit/404 GET). Lecciones [[L-61]] · [[L-05]]. Sin cache bump. **Pend: DESPLEGAR** (merge Desarrollo→main). TODO-60 (autocomplete) sigue abierto.
