...
| TODO-34 | **Auditoría pág. PIEZA** (Daniel 2026-06-25). Fixes demo-críticos ✅ (`f2ec1ab`+`de9dbef`). · ✅ **"También podría gustarte" RESUELTO** (trap `?p=` viejo: related shell→shell hacía `pushState` sin re-render="no pasa nada"; arreglado `10a26bf` con URLs limpias; verif. en vivo). · ✅ **REDISEÑO ficha "Carta Gemológica"** implementado+verificado LOCAL 2026-06-25: `buildSpecs`→agrupado + `.pz-ficha` (`pieza.js`/`pieza.css`) + `gemFacetSVG`; gema-héroe + 3 grupos 2-col + sello GIA, tokens reales Bersaglio (CD vino beige→corregido), cero-demo, escala 2..11; Chrome OK 11-specs y 2-specs; **cache v32**. Iteración: show_widget→Claude Design→**refinado con CD handoff** (`~/Downloads/design_handoff_carta_gemologica/`: gema=octágono talla-esmeralda+lustre radial [antes hexágono plano], specs SIN líneas, valores Cormorant, sello pastilla; NO mirror, adaptado a tokens). **PEND**: merge Desarrollo→main (Daniel) + verif prod c/ piezas reales → **escribir ADR**. · Tail menor: reglas `sizes`·slug estable·collection enum·validación precio·caché stale·lightbox·cart sin-precio. (SEO→TODO-35.) | 🟡 | merge + ADR; tail menor |
