...
>
> **2026-06-12 (noche) · M4 FUNDACIÓN hecha y DESPLEGADA** (VIVO — aún sin ADR): reglas `conciliaciones/{mes}` (acta owner one-way) + `cortes/{mes}` (solo CF) desplegadas (136/136) · detectores PUROS `js/crm-auditoria.js` (11/11: tajadas vs tope mensual · corrección sin enlace · rechazada burlada · recaudo por medio · muestra trimestral sembrada · degradado SLA). Lo que FALTA → "Foco actual".
