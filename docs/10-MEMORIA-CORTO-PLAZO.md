...
> · **BUILD 2a** (Desarrollo; main atrás): ✅ P1 candado (POS 19/19) · ✅ P2 cripto `wompi-core` (11/11) · ✅ P3 `iniciarPagoWeb` (reserva→`pago_pendiente`+firma, callable PÚBLICA, wompi.integration 10/10). **SIGUIENTE**: webhook `confirmarPagoWompi` (HTTP, solo-APPROVED, idempotencia doble-llave) → reaper (Scheduler) → reglas(estado `pago_pendiente`)+índice → front(flag)+widget → deploy sandbox+secretos. Tests ROJO 1º · App Check=2c · legal bloquea cobro REAL.
