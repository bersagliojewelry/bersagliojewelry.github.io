...
| TODO-42 | **Wompi F2 (cobro web)** — 2a DESPLEGADA + GATE LIVE ✅ (§147). **Legal LISTO** (§157, TODO-49) + **consentimiento Habeas Data persistido** (§157.10, LEGAL-10, code+tests). **ENCENDER (próxima sesión, en orden)**: (1) **llaves prod Wompi** (Kary, panel `comercios.wompi.co` → Secret Manager) · (2) **precios reales** (Daniel/Kary; hoy temporales) · (3) `firebase deploy --only functions` (L-22; trae el consentimiento) · (4) reescribir Términos §03 a "la web cobra online" · (5) URL de Eventos prod + **subir flag** `WOMPI_WEB_ENABLED` (`carrito.js:49`). Opcional: App Check (TODO-14). SSoT → spec wompi-checkout-web-design. | 🟢 | ENCENDER (llaves Kary + precios) |
