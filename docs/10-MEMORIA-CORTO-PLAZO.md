...
> - **M2a EN CURSO** (slice más pesado; diseño+chunks → bóveda `fase-m-m2a-design.md` + tareas #1-#7). HECHO: contrato puro del gate (`js/crm-correccion.js`, 16/16 tests, compartido con M3) + capa de datos solicitudes/gestiones (`crm-service.js`, build verde). ⚠️ El par atómico de corrección está BLOQUEADO hasta ensanchar `anulacionValida` (+`motivoCategoria`/`corregidoPor` al hasOnly — EXPAND aditivo, pero regla de dinero → test+red-team+deploy = **M2a-1b**). **Siguiente: M2a-1b → par → UI (Corregir/anular/abono/pendiente/RECHAZADA) → smoke Kary.**
