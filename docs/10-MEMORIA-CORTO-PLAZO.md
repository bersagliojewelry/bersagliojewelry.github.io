...
> 🟣 **F-TESORERÍA (TODO-78) · B0-B4 + B5(D6·V1·V18·**V17**) ✅ EN PROD.** **V17 CERRADO + D9 ✅** 25jul: CFs `registrarAbonoCartera`/`anularAbonoCartera` + cierre en REGLAS + verificado en vivo (login owner).
>
...
>
> **SIGUE**: microcopy global + extender el cuadre diario 3:30 en Salud (comparar `saldoActual` vs recompute por cuenta) → **B6** (rompimiento adversarial) y cierre de F-TESORERÍA. ⚠️ **E2E de D9 pendiente de la 1ª cuenta REAL de Kary** (prod tiene 0 a propósito, V21): verificado hasta donde prod permite + 26 tests.
>
...
> 2026-07-25 · **[OPUS-5] TODO-79 ✅ + B5·V17 ✅ CERRADO (código + reglas + vivo)**. TODO-79 PRIMERO a propósito: un rechazo "abre la caja" que llega como "Ocurrió un error" empuja a marcar otro medio de pago — el control necesita voz antes que dientes. **Comité ×3 por iniciativa propia (§3.7)** + 2 peer reviews → 4 veredictos (spec §9). Verifiqué y **REFUTÉ** su "punto ciego fatal": ningún reporte suma los `ingresos` del turno como venta (único lector `hoy.js:166`). **E2E vivo (prod, owner)**: rechazo sin caja con motivo real (el modal conserva lo escrito) · abono $1.000 → el Mostrador lo espera · anular netea los DOS libros · cierre **Cuadra ✓**. Cazó 1 defecto que ningún test ve (clave cruda `abono_cartera` en la auditoría → etiqueta + test). **Cierre en REGLAS**: `movimientoValido` niega `abono+efectivo` desde el cliente (ni el owner) — cubre también `corregirMovimientoBatch`, que HEREDA el medio; guard de UI en `abrir()` (cubre los LEGADOS). Reglas 248/248. **D9 (3ª tanda)**: la transferencia entra al banco en la misma tx; "todavía no sé" queda listable y se cierra con `asignarCuentaAbono` desde el cuadre (sin esa lista sería un agujero). El test de PARIDAD cazó que el contrato de estados vive en los DOS lados y yo solo toqué uno. **Dudas declaradas (R7)**: sin flag de apagado a propósito (apagarlo REABRE el agujero; rollback = revert + CI) · abonos efectivo HISTÓRICOS sin pata = **línea de corte declarada, sin backfill** (inyectar efectivo en arqueos firmados = fabricar evidencia).
