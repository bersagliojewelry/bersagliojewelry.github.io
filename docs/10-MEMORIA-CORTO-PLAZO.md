...
| TODO-36 | **Responsive inteligente + recomendaciones relevantes + medición** (Daniel 2026-06-25). DISEÑO vía **flujo COMPLETO W-11** (1ª vez): comité ×4 (cazó 5 ajustes) + mockup + **consejo Gemini ⏳** (prompt entregado) + prompt de Chrome. **Decisión:** (1) grilla `balancedCols(n)=ceil(n/ceil(n/max))` (helper `js/core/grid-balance.js`) en destacadas/catálogo/relacionados vía `data-cols`+CSS attr (5→3+2,6→3+3,7→4+3,8→4+4; tope ancho 4K + `aspect-ratio` anti-CLS + `minmax(0,1fr)`); (2) recos por CONTENIDO (categoría=COMPUERTA · gema/metal/precio · precio-ausente=neutro · diversidad ligera), top-4, **nunca al azar**, copy honesto (Más de X / También en {gema} / Del atelier / ocultar) — estado-cero PRIMERO; (3) GA4 view_item+view_item_list(impresión)+select_item con `source_piece_slug` + **BigQuery export** (config GA, planificado) → siembra motor "también vieron" (DIFERIDO: arranque en frío). HECHO: helper + `data-cols` en featured.js/catalogo.js. PEND: respuesta Gemini→verificar→recos rewrite (pieza.js) + GA4 (analytics.js) + CSS (home/pieza/catalogo) + cache bump + Chrome live + ADR + CRUDO comité a bóveda. `[OPUS-4.8]`. | 🟡 | Gemini (Daniel) |
