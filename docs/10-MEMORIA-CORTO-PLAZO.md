...
> 2026-07-10 · **[OPUS-4.8] F-IA-2 B0·B1·B2 ✅ (B1+B2 EN PROD, SW v90)**. B1: "Negocio y equipo" = 4 pestañas (fusiona Parámetros+Usuarios; Sistema=2 ítems). B2: directorio `admin-clientes` ⇄ Cartera `admin-cuentas` (KPIs/mora intactos; modal+cumpleaños movidos; enlaces bidireccionales). Verif/bloque: tests+build+en-vivo (módulos sin error, HTML servido OK, cero residuos). ⏳ funcional-con-sesión→deploy. `[[feedback_opus_interino]]`
> 2026-07-10 · **[FABLE-5] decisiones D1-D7 CERRADAS** (detalle→spec §0.7). Para **B3-B5**: D3 Hoy sin "Consultas recientes" (pulso $ máx 5, read-only) · D4 "Cierre de mes" in situ en Salud (corte/acta viven en `salud.js:18`) · D5 B4 muda `aprobaciones.js`+`aprobarEventoCaja` (badge `aprob-badge`); Salud pierde sección aprobaciones. Opus ejecuta sin preguntar.
...
> 2026-07-10 · **[OPUS-4.8] SESIÓN CERRADA → relevo a conversación nueva**. Retomar con **"ejecuta B3"**: cargar `opus-interino-protocolo`+`asesor-critico-honesto`+`caza-bugs`, leer spec §5 (B3 "Hoy" $ read-only, D3) + §0.7. B1+B2 en prod (validar funcional-con-sesión cuando Daniel entre/conecte Chrome). Desarrollo≡main.
