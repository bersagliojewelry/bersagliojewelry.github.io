...
> 2026-07-11 · **[OPUS-4.8] SEO A2 ✅ (§184) — 9 landing pages de faceta** (/coleccion×5 + /gema×4) horneadas: title/H1/meta keyword-first + JSON-LD CollectionPage+ItemList+Breadcrumb + `<noscript>` + `__BJ_FACET` (hidrata grilla pre-filtrada; gema vía `tieneGema`). Umbral `FACET_MIN=2` (argollas 0=saltada). Footer +col "Por gema" + URLs bonitas; sitemap +9. **SIN cache bump**. Prod verificado (curl: title/robots/canonical/numberOfItems + catalogo.json 200/32). Hidratación visual = ambiental (L-05/Chrome-automatización no pintan catálogo; el normal queda idéntico). Deploy `922fabc`. **A3 GATED**: journal vacío → decisión Daniel. `[[feedback_no_demo_en_index]]`
> 2026-07-10 · **[OPUS-4.8] VISIBILIDAD SEO/AEO/GEO — auditoría 6-frentes + A1 + GSC** → spec `2026-07-10-visibilidad-seo-aeo-geo-DISENO.md` (SSoT; 47 findings; estado+RELEVO §3-4). **A1 ✅** titles/meta keyword-first (deploy `43e0666`; poema intacto). **GSC authuser=3**: sitemap OK, piezas EN COLA (NO bug); **7 URLs solicitadas** (home·colecciones·nosotros·4 joyas/gema). **Ciclo precio→sitemap VERIFICADO** (updatedAt server-side+cron; ≤24h). ⏳ Skills: prompt entregado. RELEVO sesión nueva: A2/A3 · B2 GBP · A4 · revisar GSC ~5d. Shopping⏳precios. `[[feedback_no_demo_en_index]]`
