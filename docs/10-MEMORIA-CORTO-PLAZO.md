...
| TODO-63 | **Rediseño UX checkout — CONSTRUIDO + DESPLEGADO (v63)** — diseño comité×4+Antigravity + gate ✅. Hecho: cimientos `countries`/`envio-config` (12/12) · paso **Entrega** (tipo+país/indicativo+legal_id+pickup-con-dirección+consentimiento al final) · **enfoque de pago** (ocultar otros+escape ≥44px) · Wompi **REDIRECT** (página segura, no widget; `customer-data:legal-id`) · §3 Términos GD-2 · pantalla "PAGO RECIBIDO". **✅ VERIFICADO LIVE en prod** (Chrome, v63 serving; deploy verde tras resolver divergencia PR #404): Entrega/3 tipos/36 países/legal_id/consentimiento + internacional-oculta-dirección + tienda-mantiene-dirección + resumen "Consultar precio" sin precio. **PEND**: gate-live del **redirect Wompi real** (Daniel: pieza ≥$1.500 + URL Eventos en panel → yo valido en Chrome) + (refinamiento) reserva-reintento-misma-sesión (backend). Spec `2026-06-30-checkout-redesign-design`. | 🟡 | gate-live redirect |
