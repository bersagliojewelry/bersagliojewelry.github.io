...
>
> **🧭 FOCO = PLAN ÚNICO ERP v4 (TODO-68, Daniel 2026-07-04: "un solo sistema, tú defines el orden")** — SSoT: spec `2026-07-04-plan-unico-erp-v4`. **F1 COMPLETO: PUENTE ✅ (§165) + CORE ✅ (§167, backend `994776a` + UI `d12be6f` + gate E2E 3 flujos EN PROD 2026-07-06)**. Restan F1-extras que NO gatean: push FCM A.6 · badge sidebar. **EN CURSO: F2.0 caja/bóveda — diseño CERRADO+gateado; implementación TDD B0-B5. B0 reglas ✅ + B1 turnos ✅ + B2 enlace venta↔turno ✅ → PRÓXIMO B3 (bóveda: traslado/reverso/recompute).** Precios NO bloquean; monitorear 1ª APPROVED sigue vivo. `[[project_comercio_pagos]]`
...
> 2026-07-06 (Opus · **F2.0 DISEÑO CERRADO → implementación**): diseño CERRADO (comité ×3 + Gemini + reencuadre roles: `owner`=Kary · rol NUEVO `caja`=cajera · Daniel=dev; decisiones §8.6 = bóveda $0 · fondo $200k · limiteCajon $4M · Dual-Approval Kary) → **spec ejecutable SSoT `docs/superpowers/specs/2026-07-06-f2-0-caja-boveda-DISENO.md §8-§10`** (6 bloques TDD B0-B5; invariantes duros + CRUDOs comité/Gemini viven allí). **✅ B0-B2 HECHO** (sin deploy aún): **B0** reglas (test:rules 233): ROOT+enum `caja`·`isCaja()`·`config/caja` owner-only(read cajera)·colecciones CF-only (`caja/estado` puntero#4·`turnos`+`movsCaja`·`boveda/*` D7·`bovedaMovimientos` append-only; desvío `movsCaja`→**L-73**). **B1** `caja-core.js` `abrir/cerrarTurnoCore` (puntero singleton→**L-74**·idempotencia opId·ecuación cierre #7, sin índice compuesto). **B2** `crearPedido` POS lee `caja/estado`+`config/caja` en tx (atomicidad #5·turnoId todos-medios §9.5·`enforceTurno`·cota `docsDelTurno`): caja.integration **14/14** + pedidos **24/24** + wompi **16/16** aislados (→**L-75**). **PRÓXIMO B3** (bóveda: traslado/reverso/recompute+checkpoint, §9.3). ⚑ `enforceTurno` OFF por defecto → cero impacto en prod hasta el runbook §9.4. L-72 residual `d244d77`. Pend F1: push A.6. ⚠️ Runbook siembra root.
