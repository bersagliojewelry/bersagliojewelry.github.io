...
> - **M2a EN CURSO** (slice más pesado; diseño+chunks → bóveda `fase-m-m2a-design.md` + tareas #1-#7). HECHO: contrato puro del gate (`js/crm-correccion.js`, 16/16) + capa de datos solicitudes/gestiones + **par atómico `corregirMovimientoBatch`** (`crm-service.js`) + **M2a-1b DESPLEGADO** (ensanche aditivo de `anulacionValida` §73; red-team W-01 0 bloqueantes; reglas 100/100). ⚠️ Forward-risk §73: anulaciones sin `motivoCategoria` entre M2a y M3 NO se cubren retroactivo → M2a-4 lo mete al modal de anular + M3/M4 usan `sin_clasificar_legacy`. **Siguiente: UI de Kary — M2a-3 (botón Corregir) → M2a-4/5/6 → smoke Kary.**
