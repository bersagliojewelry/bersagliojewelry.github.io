...
| TODO-68 | **PLAN ÚNICO ERP v4** (SSoT spec 2026-07-04): F1 ✅ · F2.0 caja ✅ · F2.1 vínculo ✅ · "caja tiempo real" ✅ (§172). **Sigue**: TODO-70 (POS pro) → F2.2 factura multi-línea (TODO-41) → F2.4 apartados (TODO-39) → 2.3 térmica (hardware, D-5) → F3 inventario → carril D → F4-F6. D-1 apartados=SÍ · D-2 flete aparte · D-5 sin comprar. §11 = estrategia de modelos. | 🟢 | TODO-70 → F2.2 |
...
| TODO-70 | **🔴 Profesionalizar el POS — caja OBLIGATORIA (prereq F2.2)** (Daniel 2026-07-07): `config/caja.enforceTurno=false` (prod) deja vender con caja CERRADA → venta `turnoId:null` FUERA de arqueo/auditoría (§172). **Fix**: (1) `enforceTurno:true` (decisión Daniel/Kary); (2) UI de estado (ocultar "Cerrar caja" sin turno · bloquear venta · limpiar "ventas recientes" con caja cerrada); (3) retirar Z legacy `cierreCaja`; (4) marcar `turnoId:null` como anomalía en Auditoría. Código YA soporta; falta encender+pulir. → **L-81**. | 🔴 | sesión fresca |
| TODO-41 | **Facturación multi-línea = F2.2** (Daniel: decisión **"AMBOS"** = catálogo servicios + línea libre). Spec ✅ + comité ×3 ✅ + **gate fiscal RESUELTO** (No responsable de IVA → sin lógica por línea; `naturaleza` future-proof). Detalle: spec `2026-07-07-f2-2-...§8` · `42-LEGAL §7` · CRUDO bóveda. Costura `items[]` ya en `pedidos-core.js:244`. **BUILD**: `servicios`(colección+reglas+CMS owner)·`crearPedido lineasExtra`(TDD)·POS multi-línea(colapsado+chips+línea libre blindada)·export `naturaleza`·gate. **Prereq: TODO-70.** | 🟢 | tras TODO-70 |
...
> 2026-07-07 · **Sesión "caja"** (detalle → ADRs/specs): (1) **"Caja en tiempo real" CERRADO → ADR §172** (F1 reactividad + F2 Auditoría owner, gate Chrome verde, L-80). (2) Fix seguridad **CSV-injection** en exports (SW v82). (3) **F2.2** diseñada + comité ×3 (CRUDO bóveda · síntesis spec §8) + **gate fiscal RESUELTO** (RUT: No responsable de IVA → sin lógica por línea; 42-LEGAL §7). (4) 🔴 **hueco POS** (TODO-70/L-81): `enforceTurno:false` deja vender con caja cerrada → venta huérfana fuera de arqueo/auditoría. **PRÓXIMA SESIÓN (fresca)**: TODO-70 (profesionalizar POS) → F2.2 build. SW **v82**·APP **v44**.
