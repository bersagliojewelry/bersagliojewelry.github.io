...
> 2026-07-07 (Opus · **F2.1 vínculo cliente EN CURSO**): Decisión Fuerte = contrato de identidad. Flujo completo (arquitecto+comité ×3+prompt consejo) → **SSoT spec `2026-07-07-f2-1-vinculo-cliente-DISENO.md`** (crudo comité en bóveda). Claves del comité (integradas): persona posee N documentos (alias, NO 1-clave=1-persona)+fusión CF día-1 · normalización SOLO-servidor · índice HMAC(pepper) · dedup blando teléfono · reservas F5 (contacto verificado/`authUid`) · consentimiento mostrador. **Forks (Daniel): opcional+post-cobro+cola · robusto de una.** **HECHO: `functions/identidad-core.js` (contrato normalizeLegalId/docHash/dedup) — 22 tests `test:identidad`.** RESTAN → tareas 2-5 (CFs+reglas+índices+UI+deploy+gate). ⚠️ ADR de F2.1 = **§171** (§170 lo tomó la otra sesión). Tag `[OPUS-4.8]`.
