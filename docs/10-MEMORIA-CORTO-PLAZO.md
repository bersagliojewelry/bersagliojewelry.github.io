...
>
> **🚦 Próximo**: (1) Daniel **mergea Desarrollo→main** → deploy fix VT + sonda → re-verifico en vivo (parpadeo ido + sonda). (2) **Fase 0**: A3 (esqueletos + paralelizar auth↔datos) · prefetch-asset · self-host fonts. (3) datos instantáneos = aligerar collectionGroup / panel-tipo-app. Lección a canonizar (prefetch-datos moot MPA · memoryLocalCache≠Auth/AppCheck · VT empeora latencia alta) al GC de `30`. Luego TODO-07 · TODO-14. **Regla**: `arquitecto-software` SIEMPRE · `[[feedback_workflows_acotados]]`.
