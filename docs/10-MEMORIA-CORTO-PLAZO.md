...
| TODO-30 | **§103 F2: router falso-SPA** (nav instantánea en memoria + prefetch-hover; ground-truth §103.2/.5). Toca TODA la navegación (forms/drawers/scroll/atrás-adelante/SEO) → Decisión cara → **comité #2 de casos borde + gate empírico** antes del merge. F1 (LQIP) ya en Desarrollo (§104). | 🔲 | sesión fresca dedicada |
...
>
> **▶️ RETOMA = sesión NUEVA dedicada al §103 F2** (navegación instantánea / router falso-SPA, **TODO-30**). **Prioridad** (Daniel 2026-06-23, con capturas): el **parpadeo al cambiar de menú** (Inicio↔Nosotros↔…) + la imagen que recarga = **cruce de shells = recarga completa** (§103.1b); **F1 NO lo arregla, F2 SÍ**. Toca TODA la nav (forms/drawers/scroll/atrás/carrito/SEO) → **comité #2 casos borde + gate**; ground-truth §103.2/.5. **Evaluar primero View Transitions nativas (cross-document, Chrome) vs el router** dentro de F2 (posible camino más liviano). **⚠️ F1 (EN PROD) solo cubre imágenes subidas DESPUÉS** → existentes: re-subir en panel o backfill (requiere CORS de Storage). Boot §G.1 + leer §103/§104/§105.
>
> **▶️ 2026-06-23 — sprint web** ([OPUS-4.8]): §100 cero-demo+móvil · §101 gate empírico · §102 carga fluida · QA público LIMPIO · §103 "app-like" DECIDIDA (router falso-SPA + LQIP; GO Daniel). Detalle → **99 §100-§103**. [[feedback_no_demo_en_index]].
>
> **▶️ 2026-06-23 — §104/§105 EN PROD; §106 REVERT** ([OPUS-4.8]): **§104** F1 LQIP (PR #306). **§105** fix recarga `reveal.js` (PR #307). **§106** = **REVERT del render LQIP**: Daniel reportó que F1 hacía **vacío→borroso→nítido (3 estados, PEOR)**; el LQIP llega con el getDoc, no antes (§103.3) → SUMA estado, no lo reemplaza. Vuelto a neutro→foto; **fontanería LQIP conservada para el F2** (la reactiva cuando la caché-memoria haga preceder el borroso). cache v26, tests 55/55, build ✓. **La molestia REAL de Daniel (parpadeo entre PÁGINAS) = F2, pendiente.** → 99 §104/§105/§106 · L-47..L-50/M-07.
>
> **🚦 COLA REAL — gated/pendiente**: (1) **§103 F2 navegación instantánea (router falso-SPA o View Transitions) = TODO-30, SESIÓN NUEVA, PRIORIDAD** (elimina el parpadeo entre páginas; **reactiva el render LQIP del §104** con la caché-memoria — revertido en §106 porque suelto sumaba un 3er estado; F3 SSG opcional); (2) **al reactivar LQIP**: backfill/re-subir imágenes existentes (las pre-F1 no tienen LQIP); (3) **alinear cerebro de cars** (sesión dedicada); (4) borrado auto imagen Storage (irreversible); (5) C1 (PERF-07); (6) responsive; (7) limpiar CSS muerto + `/img/*` demo; (8) re-verificar catálogo/pieza CON piezas cuando Kary cargue. **Regla**: `arquitecto-software` SIEMPRE. `[[feedback_workflows_acotados]]`. [OPUS-4.8].
