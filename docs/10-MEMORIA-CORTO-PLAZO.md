...
| TODO-42 | **Wompi F2 (cobro web)** — 2a en main, **dormida** (flag OFF `carrito.js:49`). **Re-verif. 2026-06-30: suite 36/36 VERDE** + aviso vendedor≠cobrador (`carrito.js:312`) + Habeas Data en checkout + legal §157. **ENCENDER = Fase 2c; runbook EXACTO en spec §12-§14**. Bloquea SOLO en HUMANO: **(A)** llaves prod (Kary, `comercios.wompi.co`→Secret Manager) · **(B)** precios reales (Daniel/Kary). Luego yo (runbook §13): secretos+`.env` prod → deploy functions → URL Eventos → §3 (borrador §14) → flag+build+SW → gate live PROD. SSoT → spec. | 🟢 | (A) llaves Kary + (B) precios |
...
| TODO-49 | **Legal e-commerce** ✅ LISTO + DESPLEGADO (§157.x): Términos+Privacidad reales (retracto/reversión Ley 2439/garantía/SIC/datos), IVA, identidad real (Kary=comerciante), consentimiento persistido (LEGAL-10, tests 11/11). Verificado por **comité ×3 + Antigravity + ChatGPT** (abogado OMITIDO por Daniel). **Resta menor**: confirmar garantía 12m (Daniel). Encender Wompi → TODO-42. Detalle → `42-LEGAL §4/§5`. | 🟢 | confirmar 12m |
...
>
> **2026-06-30 (Wompi — PREPARADO para encender, sin tocar prod):** Daniel "tú decides" → dejé todo listo. Re-verif. contra código: flag OFF, CFs+front+secretos OK, **suite 36/36 VERDE**, aviso vendedor≠cobrador + Habeas Data en checkout, `.env` gitignored. §3 ya menciona Wompi (matiz "Pagar ahora" → borrador spec §14). **Runbook de encendido exacto → spec §12-§14.** Bloquea SOLO en humano: llaves prod (Kary) + precios. Trabajo directo, sin workflow (`[[feedback_workflows_acotados]]`).
>
