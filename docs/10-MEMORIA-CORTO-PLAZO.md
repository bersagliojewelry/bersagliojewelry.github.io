...
>
> **🚦 Frente activo: TODO-37 PLAN MAESTRO DE COMERCIO.** **B0+B0.5 EN PROD ✅** (§120, PR #359 — Daniel mergeó en sesión): WhatsApp directo en la ficha + GA4 `whatsapp_click` + `waLink`; cache **v34**. **VERIFICADO EN VIVO** (emulador+seed): sin-precio→WhatsApp primario, con-precio→carrito+WhatsApp 2º, evento `whatsapp_click` con contexto, cero errores (L-58). **🌱 BANCO DE PRUEBAS EN PROD ✅** (§121, Daniel 2026-06-26): 9 piezas de prueba (`seedDemo:true`, 5 c/precio + 4 bajo consulta, colecciones reales de Kary) **subidas a PROD vía Firebase MCP** (ADC no escribe prod, §110.3) + **verificadas EN VIVO** en `bersagliojewelry.co` (9 renderizan, imágenes 200, 0 errores). **Decisión: pruebas en la WEB REAL** (excepción temporal a no-demo, fase pre-lanzamiento). Emulador-seed (`functions/seed-piezas.mjs`, L-58) sigue disponible. **🔒 Decisiones FIJADAS** (§121, NO re-preguntar): Wompi=cuenta Kary Persona Natural (NO PJ; topes $2.5M/$10M; aumento a 20 tx → Daniel avisa) · ADDI ❄️ congelado (Kary vincula). **🛠️ B1 EN CONSTRUCCIÓN** (diseño `2026-06-25-b1-mostrador-design.md`): **paso 1 ✅ §122** — extensión de inventario en `pieces` (stockType/cantidad/gender, reglas `pieceClassValid` DESPLEGADAS a prod + form admin + 201 tests; `estado/reserva*` diferidos al CF). **✅ paso 2 §124** — calculadora de precio por peso (valor-gramo = INPUT de Kary, VARÍA, no config; `calcularPrecio` puro + modal en Piezas, client-side; 6 tests). **SIGUIENTE paso 3: `crearPedido` CF** (entidad `pedidos`, stock atómico candado=pieza, recompute server-side + snapshot inmutable). **✅ §123 — 3 correcciones de Daniel** (form flotante en ficha con la pieza enlazada + 9 destacadas con guard máx-9 + canales debajo del form en contacto; cache v35; verificado en vivo; pend. merge). _Cerrados en prod: TODO-34 §118 · TODO-36 §119 · B0.5 §120 · catálogo prueba+decisiones §121 · §117._ Cola: TODO-33 · TODO-35. ⚠️ Gotcha dev (§123.4/L-58): carrera `data.load`↔`onChange` en emulador → forzar refresh para verificar (no afecta prod).
