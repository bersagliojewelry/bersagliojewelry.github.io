...
> 🟣 **RELEVO → LA NUEVA SESIÓN ARRANCA EN B5·V18** (zona caliente: test PRIMERO). **F-TESORERÍA (TODO-78) · B0-B4 + B5(D6·V1) ✅ EN CÓDIGO (no en prod)**. Daniel: desplegar el BUNDLE coherente al final (no bloque suelto). **Hecho**: B0 (auditado) · B1 (CFs+trigger+espejo) · B2 (página "Cuentas y bancos") · B3 (cuadre) · B4 (Bandeja "Tesorería" + badge · "Plata total" en Hoy · V9 socias) · **D6 (editor "Reglas del sistema", owner + audit)**. Suites verdes offline (integración 22/22). **Falta gate E2E vivo del bundle** → DEPLOY + login (L-05).
>
> **B5 EN CURSO (costuras, §6)**: D6 ✅ · V1 ✅. **SIGUE `V18`** ("Retiro de banco" = espejo de V1: `banco_a_boveda` +1 en `SIGNO_BOVEDA` + `PATA_POR_TIPO` ya listo para `retiro_efectivo_out` + UI en bóveda) → V17 → D9 + microcopy + cuadre diario 3:30 en Salud. **⚠️ Zonas calientes R3 = test PRIMERO** (V17 abono efectivo→caja · D9 abono→cuenta con flag off). **Mapa de ejecución (dónde vive cada cosa) + las 2 precisiones de D6 → spec §9.** Luego B6 (rompimiento adversarial read-only + Chrome holístico) → **DEPLOY MANUAL del bundle** (`firebase deploy --only firestore:rules,firestore:indexes,functions` + seed virtuales prod; SW ya v98) + E2E holístico Chrome.
>
...
> 2026-07-24 · **[OPUS-5] B5: D6 ✅ + V1 ✅ (P0)**. **D6**: "Reglas del sistema" EDITABLE owner (CF `actualizarConfigSistema` + whitelist `CAMPOS_CONFIG` + rangos + audit). **V1 (zona caliente, TDD: el rojo REPRODUJO el P0)**: `boveda_a_banco` con `cuentaId` escribe su pata `consignacion_in` en la MISMA tx (`{opId}-teso`, fuente SISTEMA) — cuenta inválida ⇒ aborta TODO · V4 por-libro: el replay crea la pata faltante · sin `cuentaId` = conducta previa EXACTA. UI de bóveda pide la cuenta. Gates: teso **27/27** · **caja 38/38 SIN regresión** · resto verde. **Detalle + 2 precisiones de D6 → spec §9 y commits.**
