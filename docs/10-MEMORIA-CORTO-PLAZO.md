...
| TODO-30 | **§103 F2 navegación/carga app-like** — F2.0 View Transitions ✅ (§107). **§108 caché inteligente: F1 ✅ (persistente SOLO-público; §108.11) · F3 LQIP piezas/colecciones ✅ (§108.14) · §111 LQIP en `siteContent` vía SWR cache-first ✅** (`getDocFromCache→getDocFromServer`+diff-gate por `version`; resuelve §110.2 sin repetir §106; commit `f6ff72e`, pendiente PR Daniel). **§111 SWR siteContent VALIDADO en prod ✅** (PR #321; 2 opiniones: Daniel + extensión Chrome). **§110.1 migración EJECUTADA ✅** (8 LQIP, §110.4). **Cobertura blur extendida** a catálogo/wishlist/detalle (111.8, `f8f79ec`, pend. merge — §108.14 solo había cableado el Home). F2 diff-gate piezas = OPCIONAL (no parpadea). Router F2.1 = innecesario. **§112 Cache-Control Storage** (blur en cada visita → RCA `private,max-age=0`; fix `_upload`+backfill 18/18 EJECUTADO; §112). **Único pend: merge `2eb71fe` (Daniel; incluye 111.8 + §112).** | 🟢 | merge 2eb71fe (Daniel) |
...
> Podada (GC) 2026-06-23. Histórico → **ADR §37-§111** (mapa `00`→detalle `99`; lecciones L-/M- en `30`/`31`).
>
> **▶️ §108-§111 EN PROD (saga caché/blur cerrada salvo 1 merge).** §108 F1/F3 + §109 + **§111 SWR siteContent** + **migración LQIP** (§110.4, 8 backfilled) desplegados; **§111 VALIDADO en prod** (Daniel: modelo mental exacto "blur en frío, instantáneo desde caché" + extensión Chrome ✅ consola/transiciones/2ª-carga). Detalle → ADR §108-§111 (+110.4, +111.8) y L-50(EXT)/L-52. **§112 (clave) — el blur salía en CADA visita** (Daniel; yo había malinterpretado su feedback): RCA medida = Storage servía `private,max-age=0` (subían sin `cacheControl`) → revalida cada visita. Fix `_upload` (`public,max-age=31536000`; URL versionada por token) + backfill `migrate-cache-control.mjs` **EJECUTADO 18/18 en prod** (verificado curl). El LQIP no estaba mal, le faltaba esto. **Pendientes: Daniel mergea `2eb71fe`** (incluye 111.8 `f8f79ec` + §112) → nuevas subidas cachean; las existentes ya backfilled (se auto-curan en 1 carga). SA key en Descargas → borrar/revocar. **Boot nueva sesión**: §112 + §111(.8) + §110.4 + L-53/L-50(EXT)/L-52.
>
