...
>
> **▶️ RETOMAR AQUÍ — PANEL v2: Morosos desplegado + F5 mergeado + F4 (Bandeja) construido; SIGUIENTE = F6 o F7** (2026-06-07):
> Panel v2 = **mini-ERP** (CRM/leads + cartera + facturación + inventario + pagos + trazabilidad), IA "C", construido por fases. **Norte**: spec `docs/superpowers/specs/2026-06-07-bersaglio-arquitectura-maestra-design.md` (v3, Consejo §16). **F-CHASIS-A** ✅ desplegado (ADR §50). **MOROSOS/VENCIDOS** ✅ **DESPLEGADO a prod** (ADR §51, TODO-13 cerrado): mora FIFO en vivo (`js/crm-estado-cuenta.js`, sin CF) + `fecha` real en movimientos + ficha "Vencido N días" + lista CxC (KPI cartera vencida + rangos, vencido en rojo, orden por mora). Reglas+functions desplegadas; `.co` 200. Spec `docs/superpowers/specs/2026-06-07-morosos-vencidos-design.md`.
> - **F5 (filtros) ✅ construido + MERGEADO por Daniel** (ADR §52): chips estado/rango de mora + select vendedora + contador. **Control de crédito DESCARTADO** (Daniel: sin cupo por cliente; va con F7).
> - **F4-leads (Bandeja) ✅ construido** (ADR §53, `Desarrollo` sin mergear): pipeline de 5 estados (Nuevo→Trabajando→Calificado→Cliente/Perdido) sobre `inquiries` (NO colección `leads` aún) + origen + antigüedad/demorado + **convertir a cliente** (admin) + alta manual + badge=nuevos. Helper puro `lead-format.js` (test 8/8). Front+servicio → live al mergear. **Gaps a F6**: reglas de `inquiries.update` (editor puede tocar `convertedTo`, impacto bajo) + colección `leads` formal + ingestión por CF + App Check.
> - **▶️ SIGUIENTE**: **F6** (escala + hardening: App Check/TODO-14, agregados, leads formales, paginación) **o** **F7** (ventas/factura/inventario — dinero nuevo: requiere Consejo Externo + PRE-infra TODO-14 antes).
