...
| TODO-49 | **Legal e-commerce** (prereq Wompi 2c). **§157 HECHO + DESPLEGADO** (comité×3 + fuentes oficiales `.gov.co`): retracto Art.47 + reversión Art.51 + garantía legal + enlace SIC en `terminos.js`; consultas10/reclamos15 + retención + Encargado en `privacidad.js` — válido HOY (aplica a venta a distancia). Checkbox Habeas Data + cookies ya OK. **RESTA (al encender Wompi / LEGAL-08)**: NIT + titular de cuenta (Kary) · aviso vendedor≠cobrador en el pago · reescribir §03 ("la web cobra online") · guardar prueba del consentimiento · disclaimer abogado. | 🟡 | datos Kary + encender Wompi |
