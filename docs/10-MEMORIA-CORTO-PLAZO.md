...
>
> **▶️ FOCO 2026-06-23 (sesión §111).** **git fetch**: Daniel YA mergeó **PR #318** → §108(F1/F3)+§109+**script migración** EN PROD (corregido el stale "pendiente PR" en `05`/`10` = reincidencia HA-01). **HECHO (§111, commit `f6ff72e` en `Desarrollo`, pendiente PR Daniel)**: **LQIP en `siteContent` vía SWR cache-first** — `data.loadSiteContent` = `getDocFromCache`→`getDocFromServer`+diff-gate por `version`; render blur en hero/editorial(home)+hero/atelier(nosotros). Reconcilia §110.2 con **L-50** (el blur ahora PRECEDE a la imagen porque la caché de §108 lo habilita; el 3er estado de §106 era por `getDoc` server-first POR VISITA). Build+45 tests+navegador (degrada limpio offline). **§110.1 migración EJECUTADA ✅** (§110.4): Daniel bajó SA key de bersaglio (guiado con widget visual) → `GOOGLE_APPLICATION_CREDENTIALS=<Downloads/...json>` → dry-run 8 (2 piezas+6 cols) 0 fallos → `--apply` 8 escritos → re-dry-run idempotente. Render piezas/cols (§108.14) ya en prod → **blur live SIN deploy**. SA key fuera del repo → Daniel la borra/revoca. **Único pendiente: Daniel mergea §111** (`f6ff72e`) → despliega el blur de `siteContent` (hero/nosotros). F2 diff-gate piezas = OPCIONAL (no parpadea, §108.13). **Boot nueva sesión**: §111 + §110.4 + §108.13-.16 + L-50(EXT)/L-52.
>
