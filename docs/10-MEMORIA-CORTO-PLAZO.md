...
>
> **🧭 FOCO = PLAN ÚNICO ERP v4 (TODO-68)** — SSoT spec `2026-07-04-plan-unico-erp-v4`. **F1 · F2.0 · F2.1 · caja-tiempo-real ✅ EN PROD (§165-§172)** · **TODO-70 POS profesional ✅ §173**. **F2.2 factura multi-línea ✅ OPERATIVO EN PROD (§176, cierra TODO-41)** — deploy backend + gate vivo Chrome verde (venta pieza+servicio $150k server-side → anular → arqueo $0). Falta cargar servicios reales (data). **⚠️ EN CURSO = CORRECCIONES URGENTES (Daniel 2026-07-08, PAUSAN F2.4)** tras validación Chrome no-holística (`[[feedback_validacion_chrome_holistica]]`): **TODO-72** (UX: carga/zoom/caja-cerrada, Grupo A) + **TODO-73** (POS money-model: datáfono+vouchers+venta-sin-pieza, Grupo B). **DESPUÉS**: F2.4 (TODO-39) → 2.3 térmica. `[[project_comercio_pagos]]`
...
| TODO-72 | **Correcciones UX urgentes (Daniel 2026-07-08, Grupo A, 1 deploy)**: (1) catálogo ~5s + brinca abajo→arriba → esqueleto + no animar sin datos (`catalogo.js`/`home.js`); (2) zoom pieza arrastre roto + selecciona "1/1" → `user-select`/`touch-action:none` en `.pz-zoom` + `preventDefault` pointerdown (`pieza.js`/`pieza.css`); (3) "elige pieza" visible con caja CERRADA → ocultar registro si `sinTurnoAbierto()` (`pos.js`). Gate Chrome HOLÍSTICO. | 🔴 | en curso |
| TODO-73 | **POS money-model pro (Daniel 2026-07-08, Grupo B, Decisión Fuerte+TDD)**: (a) medio **DATÁFONO** (tarjeta); (b) **cuadre de vouchers al cierre** (cajera cuenta #+suma vs ventas datáfono; **descuadre NO bloquea**, como el efectivo); (c) **venta SIN pieza** (`pieceId` opcional si hay `lineasExtra` → total=extras, sin decremento de stock; POS arranca sin pieza). Espeja `crearPedido`/`cerrarTurno`+reglas. Spec+comité+TDD (interinato: NO tocar webhook/firma/reaper/snapshot). Futuro: datáfono sincronizado con POS (dejar escrito). | 🔴 | tras A |
> ✅ **Cerrados** (detalle → ADRs/`99`): TODO-41 (§176 F2.2 facturación multi-línea OPERATIVO) · TODO-70 (§173 POS profesional/caja OBLIGATORIA ON) · TODO-69 (§172 caja tiempo real+auditoría) · TODO-37/65/63/49/42/66/64/21/40/32 + 62-44 (rango). Pend Daniel: precios + fotos IA.
