...
> 2026-07-06 (Opus · **F2.0 B5b-1 UI HECHA — build+test verdes, SIN deploy**): POS caja (turno abrir/estado/efectivo estimado+límite, movimientos, traslado obligatorio al superar límite, cierre enlazado) + Bóveda owner (saldo/consignar/reponer/conteo/reverso/**aprobaciones** Dual-Approval) + helper PURO `caja-format.js` (`test:caja-format` **9/9**) + listeners por rol + nav "Bóveda" owner + `APP_VERSION v39`. **Detalle → spec §10 checklist B5b-1.** Auto-crítica cazó 3 bugs (arreglados): `_isCaja` en import→tras-auth · dinero en localStorage→memoria (L-77) · `recalcTotal` en movsCaja. Efectivo del cajón = ESTIMACIÓN (autoridad=cierre server); owner usa el ledger real (L-77). 🚩 **Brecha rol `caja` (L-78)**: `crearPedido`/`pedidos`-read/`auth.js` sin `caja` → cajera aún no vende; Kary=owner opera todo. **PRÓXIMO B5c** (deploy+config/caja+gate Chrome+ADR). `[[project_comercio_pagos]]`
> 2026-07-06 (Opus · **F2.0 diseño + backend B0-B4 + B5a/B5b-0**): diseño CERRADO (comité ×3 + Gemini + reencuadre roles `owner`=Kary/`caja`=cajera/Daniel=dev; §8.6 = bóveda $0 · fondo $200k · limiteCajon $4M · Dual-Approval Kary) → SSoT + evidencia por bloque en **spec §8-§10 checklist**. B0-B4 backend TDD verde (**caja.integration 31/31** + pedidos 24/24 + wompi 16/16 → L-73/74/75/76), SIN deploy · B5a wrappers onCall · B5b-0 transporte cliente. ⚑ `enforceTurno` OFF → cero impacto prod hasta runbook §9.4. ⚠️ Runbook siembra root pend.
