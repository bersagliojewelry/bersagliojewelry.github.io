...
> 2026-07-06 (Opus · **F2.0 DISEÑO CERRADO → implementación**): comité ×3 (9 defectos) + **consejo externo Gemini** (5 aportes) + **reencuadre de roles** → **diseño v3 + spec ejecutable SSoT `docs/superpowers/specs/2026-07-06-f2-0-caja-boveda-DISENO.md §8-§10`** (CRUDOs comité+Gemini en bóveda). ⚑ **Modelo REAL (Daniel: no es dueño, es el dev)**: `owner`=Kary (dueña) · rol NUEVO `caja`=cajera del mostrador · Daniel=dev. SoD natural (cajera opera / Kary supervisa+aprueba). Decisiones: bóveda $0 · fondo $200k · limiteCajon $4M · arqueo=mostrador solo (web aparte) · **Dual-Approval SÍ (Kary aprueba ajustes/reversos)**. Invariantes duros: puntero singleton turno · idempotencia opId · recompute síncrono · turnoId en toda venta POS · reverso-no-borra · cota turno 500-lecturas · alerta owner hardcoded. **SPEC EJECUTABLE = 6 bloques TDD B0-B5 (§10)**: B0 rol caja+reglas → B1 turnos por puntero → B2 enlace venta↔turno → B3 bóveda → B4 Dual-Approval+alertas → B5 UI+gate. **✅ B0+B1 HECHO** (sin deploy aún): **B0** reglas (test:rules 233): ROOT+enum `caja`·`isCaja()`·`config/caja` owner-only(read cajera)·colecciones CF-only (`caja/estado` puntero#4·`turnos`+`movsCaja`·`boveda/*` D7·`bovedaMovimientos` append-only; desvío `movsCaja`→**L-73**). **B1** `functions/caja-core.js` `abrir/cerrarTurnoCore` (puntero singleton→**L-74** · idempotencia opId · ecuación cierre #7): caja.integration **8/8** + pedidos **24/24** no-regresión. Cierre filtra estado en JS → **sin índice compuesto** (evita trap §8.3). **PRÓXIMO B2** (enlace venta↔turno: `crearPedido` guarda turnoId todos-los-medios-POS §9.5 + flag `enforceTurno` + cota; TOCA dinero → TDD estricto, 24/24+wompi 16 verdes). L-72 residual `d244d77`. Pend F1: push A.6. ⚠️ Runbook siembra root para la entrega.
