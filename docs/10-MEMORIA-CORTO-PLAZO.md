...
| TODO-76 | **F-IA-2 + DEPURACIÓN (INTERINATO Opus 4.8)** — spec: `docs/superpowers/specs/2026-07-10-f-ia-2-DISENO.md`. **B0 ✅ auditado [FABLE-5] + decisiones CERRADAS (§0.7 D1-D7)**. EJECUTAR: **B1** (4 pestañas Negocio·Cobranza·Equipo·Usuarios + tarjeta solo-lectura; Kardex+Pendientes fuera) → B2 Split Clientes/Cartera → B3 "Hoy" $ (sin Consultas recientes) → B4 Bandeja (mudar módulo + `aprobarEventoCaja`; badge) → B5 Salud "Cierre de mes" in situ + microcopy. CERO functions/dinero nuevo; gates Chrome holísticos. `[[feedback_reorganizar_no_es_depurar]]` | 🟡 | Opus ejecuta |
...
> 2026-07-10 · **[OPUS-4.8] B0 F-IA-2 ✅** (inventario 17 págs + H1-H8 en la spec; H1 `plazoDefault` huérfano [grep 0 lectores]; sin tocar prod). `[[feedback_opus_interino]]`
> 2026-07-10 · **[FABLE-5] AUDITORÍA B0 + CIERRE TOTAL (spec §0.7 D1-D7)**: interinato LIMPIO (commits solo-docs). D1=4 pestañas+tarjeta solo-lectura (H2 delegada por Daniel) · D2=Bandeja única (Daniel) · D3=Hoy sin "Consultas recientes" · D4=corrección de spec: corte/acta YA están en Salud (reescribir in situ) · D5=B4 muda `aprobaciones.js`+usa `aprobarEventoCaja` existentes (controlador `bandeja.js`; badge patrón `inq-badge`) · D6 sin "Volver" · D7 Cobranza fuente única. **Opus ejecuta B1→B5 SIN preguntar.**
