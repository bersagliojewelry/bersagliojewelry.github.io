...
>
> **▶️ RETOMAR AQUÍ — construir ACUERDOS DE PAGO (spec `2026-06-12-acuerdos-de-pago-design.md`; P1=A·P2=A·P3=B/24m respondidas)**: **slice 1 ✅** (fórmula TRAMOS + `acuerdoEsValido` en paridad; 15/15 nuevos, suite vieja intacta, INERTE en prod). Sigue: **slice 2 reglas** (construir+emulador OK; el DEPLOY espera integrar el Consejo Externo — prompt en bóveda, Daniel lo pega en Gemini cuando pueda) → 3 corte → 4 UI (+bump) → 5 detectores. **Cadencia (memoria `feedback_verificacion_por_hitos`)**: tests/emulador/build por commit SIEMPRE; verificación experta + prueba en vivo POR HITO al cerrar la feature. ADR §81 al cerrar.
