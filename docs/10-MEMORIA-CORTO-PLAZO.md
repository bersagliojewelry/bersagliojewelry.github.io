...
>
> **▶️ RETOMAR AQUÍ — M2b CERRADO EN PROD (ADR §76 + §76.8); lo siguiente es M3 candado** (tren acoplado: misma ventana que M2a/M2b — no dejar pasar semanas):
> - **M2b cerrado 2026-06-11**: PR #226 mergeado · guion 5/5 ejecutado por Claude vía navegador de Daniel (Chrome MCP; él solo inició sesión) · auditoría interna 10/10 (`functions/seed-guion-m2b.mjs --verificar`) · fixtures limpiados y verificado cero rastro. La cola quedó "Al día".
> - **M3 candado (reglas RESTRICTIVAS)** — prerrequisitos YA cumplidos: M2a y M2b verificados en prod ✓ · preguntas 2/3/5 respondidas ✓. PASOS: (1) **Consejo Externo Gemini ANTES** (`15-CONSEJO-EXTERNO §0` — Decisión Fuerte); (2) implementar el contract del plan (`fase-m-plan.md` M3, CON la enmienda: regla COINCIDENCIA por TIPO de solicitud, NUNCA literal "tipo/monto/motivo idénticos"; whitelist hasOnly debe incluir correccionDe+solicitudId+medioPago+motivo+nota — deferidos 1 y 4 §74); (3) suite exhaustiva de emulador (casos rojos del comité, plan C) + red-team W-01 por agente DISTINTO; (4) deploy gated anunciado + smoke del flujo diario EN PROD el mismo día. Luego M4 auditoría. Resto (M2c, M5-M7) diferible.
...
| TODO-09 | **Fase M** (movimientos robustos): M0→M2b ✅ EN PROD Y CERRADOS (§76.8); **siguiente M3 candado** (tren acoplado, no demorar) → M4 auditoría + B6 | 🟡 | M3 = Consejo Externo ANTES |
