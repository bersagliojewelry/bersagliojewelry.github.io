...
> **Avance B1 (detalle → ADRs §122-§130)**: pasos 1-6 + COP **MERGEADOS a prod ✅** (PR #369/#370; `git fetch` 2026-06-26: `Desarrollo`==`main`); **Mostrador OPERABLE + export contador** (crear · confirmar pago · anular/VOID · arqueo Z · bruto/neto CSV; 15/15 integración + 207/207 reglas + 5/5 fiscal). CFs `crearPedido`/`confirmarPago`/`anularPedido`/`cierreCaja` + reglas DESPLEGADAS. **➡️ EN DISEÑO: paso 7 `catalogo.json` a CDN** (Decisión Fuerte; spec `2026-06-26-b1-paso7-catalogo-cdn-design.md`; **comité ×3 ✅** movió B→**C** = SSG→Pages + rebuild on-demand; **pend. consejo Gemini** (`...-PROMPT-CONSEJO-EXTERNO.md`) **+ luz verde Daniel — NO codear aún**) · decisión 4b apartados (TODO-39).
> _§128-§130 **YA EN MAIN/PROD ✅** (PR #369/#370; verificado `git fetch` 2026-06-26). ⚠️ **Pruebas en vivo DIFERIDAS al final del plan+rediseño** (Daniel §130.4; NO probar pieza por pieza — build+tests por commit = red de seguridad). Gotcha dev L-58._ Cola: TODO-39 · TODO-33 · TODO-35.
...
> ✅ **Cerrados recientes**: histórico completo → ADRs §88-§130 + `00`/`99` (no re-listar aquí; últimos: §128 confirmar pago · §129 anular VOID + cierre de caja · §130 bruto/neto + export contador).
...
>
> **🚦 Próximo**: **paso 7 `catalogo.json` a CDN — EN DISEÑO** (spec `2026-06-26-b1-paso7`; comité ×3 → veredicto **C** = SSG→Pages + rebuild on-demand + verdad-de-stock-en-checkout; CRUDO→bóveda `2026-06-26-comite-b1-paso7-*`). **Bloqueo: consejo Gemini + luz verde Daniel antes de implementar 7a-7d** (`feedback_flujo_completo_nunca_parcial`). Pend. decisión 4b apartados (TODO-39) · IVA/retenciones reales (contador). Cola: TODO-33 (parpadeo) · TODO-35 (A2b/eventos/HUB). **Regla**: `arquitecto-software` SIEMPRE · **Bersaglio = 100% COP (§127)** · **pruebas en vivo SOLO al final del plan+rediseño (§130.4)** · **W-11** en diseño/decisión cara · `[[feedback_workflows_acotados]]`.
