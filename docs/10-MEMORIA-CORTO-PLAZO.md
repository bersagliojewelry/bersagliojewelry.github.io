...
| TODO-64 | **Bug LOGIN admin — parpadeos** (Daniel 2026-07-03). **DIAGNOSTICADO ×2 adversarial (Fable, 2026-07-03)**: H1 timeout 500ms `login.js:10` pierde carrera con el `getDoc` que escribe `bj_auth` → loop login↔admin; H2 ping-pong del rol `catalogo` (Kary) `auth.js:231` (requireAuth borra bj_auth→login→fallback admin.html); H3 `admin-login.html:15` replaceState mata `?error=`. **FIX = A0** del plan (sessionReady() aditivo + form-first + requireAuth ruta-por-rol + quitar inline). SSoT → `2026-07-03-auditoria-holistica-plan-fable §2`. | 🟡 | Opus impl A0 |
