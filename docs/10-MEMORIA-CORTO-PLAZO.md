...
>
> **🧭 FOCO = PLAN ÚNICO ERP v4 (TODO-68, Daniel 2026-07-04: "un solo sistema, tú defines el orden")** — SSoT: spec `2026-07-04-plan-unico-erp-v4`. **F1 COMPLETO: PUENTE ✅ (§165) + CORE ✅ (§167, backend `994776a` + UI `d12be6f` + gate E2E 3 flujos EN PROD 2026-07-06)**. Restan F1-extras que NO gatean: push FCM A.6 · badge sidebar. **EN CURSO: F2.0 caja/bóveda — backend B0-B4 ✅ + B5a wrappers server ✅ + B5b-0 transporte cliente ✅ (`pedidos-service.js`, build verde; L-72 ya resuelto d244d77). PRÓXIMO B5b-1 PANTALLAS (POS caja apertura/cierre/movimientos/modal traslado + vista Bóveda owner + panel aprobaciones) → B5c SW bump + deploy + gate Chrome + ADR. B5b-1 = UI que EXIGE sesión admin autenticada en Chrome → sesión fresca.** Precios NO bloquean; monitorear 1ª APPROVED sigue vivo. `[[project_comercio_pagos]]`
...
> 2026-07-06 (Opus · **F2.0 DISEÑO CERRADO → implementación**): diseño CERRADO (comité ×3 + Gemini + reencuadre roles: `owner`=Kary · rol NUEVO `caja`=cajera · Daniel=dev; decisiones §8.6 = bóveda $0 · fondo $200k · limiteCajon $4M · Dual-Approval Kary) → **spec ejecutable SSoT `docs/superpowers/specs/2026-07-06-f2-0-caja-boveda-DISENO.md §8-§10`** (6 bloques TDD B0-B5; invariantes duros + CRUDOs comité/Gemini viven allí). **✅ B0-B4 HECHO** (backend TDD COMPLETO, sin deploy; evidencia por bloque → spec §10 checklist): B0 reglas · B1 turnos puntero singleton · B2 `crearPedido` enlaza turnoId (`enforceTurno` OFF-default·cota) · B3 bóveda recompute-no-incrementa+reverso inmutable §9.3+checkpoint · **B4** Dual-Approval (reverso/ajuste nacen pendientes → recompute los EXCLUYE hasta firma owner; caja NO aprueba; egreso→alerta; alerta=callback inyectable, FCM real con A.6) + `movimientoCajaCore`. Verde: **caja.integration 31/31** + pedidos 24/24 + wompi 16/16 (→L-73/74/75/76). **B5a** wrappers server ✅ (`pedidos.js`/`index.js`: 7 onCall + trigger recalcBoveda · roles isCaja/isOwner · alertas→saludEventos · L-72 resuelto `d244d77`) + **B5b-0** transporte cliente ✅ (`pedidos-service.js`: 7 callables, build verde). **PRÓXIMO B5b-1 pantallas** (POS caja + vista Bóveda + panel aprobaciones; EXIGE Chrome autenticado) → **B5c** SW bump+deploy+gate Chrome+ADR. ⚑ `enforceTurno` OFF → cero impacto prod hasta runbook §9.4. Pend F1: push A.6. ⚠️ Runbook siembra root.
