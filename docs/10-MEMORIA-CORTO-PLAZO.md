...
| TODO-26 | **CAZA-BUGS cross-repo (§G) → cars-operador** (aporte de bersaglio a la pasada Gemini única, como TODO-22/23): cars añade a la `§G.4` byte-idéntica ×4 el "Reflejo de Caza-bugs" + propaga; texto exacto + gate opcional del kernel en **ADR §90.8**. Skill `caza-bugs` ya creada (dependencia cubierta). | 🔲 | Gemini (cars consolida) |
...
> **2026-06-21 · CAZA-BUGS (§90, TODO-25)**: panel adversarial 9 ag. (recortó sobre-ingeniería) → reflejo del **camino vivo desde estado-cero** + **W-10** + **skill `caza-bugs`** + **gate L-42 ×5 secciones del home** (verificado por EJECUCIÓN: bug §89 inyectado→rojo→restaurado→10/10). `§G` cross-repo → cars (**TODO-26**, §90.8); shard 30 → **TODO-27**. build+10/10✓. [OPUS-4.8].
> **2026-06-21 · BUG Categorías (§89, `6b327a0`)**: no aparecía al crear la 1ª colección desde 0 ítems (render no montaba la `<section>`; L-42). SW v21. **Resta: Daniel mergea cliente v21.** [OPUS-4.8].
