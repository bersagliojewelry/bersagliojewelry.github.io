...
> - **P4 (editor de listas, Nosotros) — DISEÑO ✅ + decisiones de Daniel ✅** (workflow `wf_159130b5`, 7 agentes; CRUDO→bóveda). Enfoque = **field-type `list` en singleton** (reuso total; listas en sub-mapas ya `is map` → cero cambio de reglas en camino crítico). **Decisiones (2026-06-18)**: add/quitar/reordenar SÍ · lista vacía→sección DESAPARECE · reseñas a mano · equipo sin fotos (después). **▶️ SIGUIENTE = IMPLEMENTAR** (sesión fresca): F0 refactor `nosotros.js`→renderers+`nosotros-defaults`+`mergeNosotros` (verificar web=idéntica con doc vacío) → motor `list` (core+UI add/del/up/down+reindex+tests) → `nosotros-preview`+descriptor+pestaña. **ANTES del deploy**: 2ª opinión externa del MODELO de datos (cara de revertir; patrón del proyecto, `docs/15`). Plan completo → spec §P4 (+CRUDO bóveda).
