...
>
> **🧭 FOCO = PLAN ÚNICO ERP v4 (TODO-68, Daniel 2026-07-04: "un solo sistema, tú defines el orden")** — SSoT: spec `2026-07-04-plan-unico-erp-v4` (comité ×3 integrado; sucede al plan-maestro-v3 y absorbe el remanente Fable como carril D). **En ejecución: Ítem 0 (decisiones D-1 apartados / D-2 flete preguntadas a Daniel) → F1-PUENTE (lista+detalle read-only de pedidos, días) → F1-CORE (máquina de estados por tabla + logística)**. Precios llegan en cualquier momento (NO bloquean; monitorear 1ª APPROVED sigue vivo). `[[project_comercio_pagos]]`
...
| TODO-37 | **PLAN MAESTRO COMERCIO v3** — SUCEDIDO por el plan v4 (TODO-68); queda como referencia de decisiones de fondo (§3-§11 de su spec siguen válidas). | 🟢 | — |
...
| TODO-65 | **Plan Fable §158-§161** — técnica CERRADA; el remanente (D 7b-7d/D.5/D.6 + E.4 + P3 §161.7) queda ABSORBIDO por el plan v4 (carril D + F1-extras de TODO-68). | 🟢 | ver TODO-68 |
...
> ✅ **Cerrados** (detalle → ADRs/`99` + lecciones): TODO-63 (checkout v63/v64; A.2→§161, gate-live→§164) · TODO-49 (legal e-commerce §157 → `42-LEGAL`) · TODO-42 (encendido A.9 §164) · TODO-66 (shard 30→34, §161) · TODO-64 (login §159 → L-66) · TODO-21 (auditoría §158) · TODO-62 (§156.18 → L-62) · TODO-61 (§156) · TODO-60/59/58/56/55/54/53/52/51/46/45/44 · TODO-40+32. Pend Daniel: precios + fotos IA.
...
> 2026-07-04 (2ª sesión, parte 2): **PLAN ÚNICO ERP v4 DEFINIDO (TODO-68)** — Daniel pidió consolidar todo (POS+pedidos+logística+facturación+contabilidad+inventario+CRM) en un solo plan priorizado. Proceso: specs vivas + estudio ALTORRA (agente: patrones ingestión/dedup/reportes; ALTORRA NO tiene facturación/logística) + comité ×3 acotado (unánime APROBAR_CON_CAMBIOS; P0: flete post-pago como CARGO aditivo, snapshot intacto · slice puente anti-ventana-ciega · costuras items[]/costoSnapshot/clienteId sembradas en F1 · ruta corta POS · Bloque D → carril paralelo). CRUDO → bóveda. D-1=SÍ apartados · D-2=flete cobrado aparte (Daniel). **Ampliación (screenshots Changarrito de Daniel)**: F2.0 sesión de caja completa · F2.3 impresión térmica ESC/POS + cajón (v1 print CSS → v2 WebSerial) · regla §9.7 sello servidor · radar 8b (gastos/proveedores-RUCOM/comisiones/descuentos/reparaciones). Modelo: sigue FABLE (decisión delegada; interinato Opus queda como fallback probado §158/§161).
