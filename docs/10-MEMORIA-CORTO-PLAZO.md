...
| TODO-49 | **Legal e-commerce** (prereq Wompi). **§157 HECHO+DESPLEGADO**: retracto Art.47/reversión Art.51/garantía legal/SIC + privacidad (consultas10/reclamos15, retención, Encargado) — válido hoy. Identidad RESUELTA (Kary=la comerciante). **Consejo ×2 (ChatGPT+Antigravity) RECIBIDO+integrado §157.8** (claims de código verificados ✓; puente §03 ya corregido; aviso del cargo ya existe `carrito.js:311`). IVA + identidad ✅ (§157.9). Consentimiento Habeas Data ✅ CÓDIGO+TESTS (§157.10, LEGAL-10; integración 11/11 emulador). Abogado **OMITIDO** (Daniel: 3 IA bastan). **RESTA solo ENCENDER Wompi**: `firebase deploy --only functions` (LEGAL-10) + reescribir §03 + **llaves prod (Kary)** + **precios reales (Daniel)** + RUCOM. `LEGALES/` blindada. | 🟡 | encender (llaves Kary + precios) |
...
>
> **2026-06-30 (§157 — legal pre-Wompi, DESPLEGADO):** Decisión Fuerte (skill `legal-colombia` + fuentes `.gov.co` + comité ×3 + **consejo externo ×2 ChatGPT+Antigravity §157.8**). `terminos.js`/`privacidad.js`: retracto/reversión/garantía/SIC/datos en vivo. Pendientes vivos → TODO-49. Detalle → ADR §157 / `42-LEGAL`.
> _(§156.1–.19 + TODO-62 podadas — consolidadas en ADR §156.x / L-62 / L-63. Daniel confirmó el pinch-zoom OK en su iPhone. TODO futuro vivo: botón "Hablar con el Atelier" en la ficha — ADR §156.12.)_
