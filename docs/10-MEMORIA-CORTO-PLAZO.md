...
| TODO-49 | **Legal e-commerce** (prereq Wompi). **§157 HECHO+DESPLEGADO**: retracto Art.47/reversión Art.51/garantía legal/SIC + privacidad (consultas10/reclamos15, retención, Encargado) — válido hoy. Identidad RESUELTA (Kary=la comerciante). **Consejo ×2 (ChatGPT+Antigravity) RECIBIDO+integrado §157.8** (claims de código verificados ✓; puente §03 ya corregido; aviso del cargo ya existe `carrito.js:311`). **RESTA** (→`42-LEGAL §5`): **2 decisiones Daniel** = IVA §02 (LEGAL-09) + publicar identidad real (reemplaza "S.A.S." falsa de `privacidad.js:30`) · al encender Wompi = persistir consentimiento (LEGAL-10, code+deploy functions) + reescribir §03 · RUCOM (origen mineral) · abogado. `LEGALES/` blindada. | 🟡 | 2 decisiones Daniel |
