...
> · **2a CODE-COMPLETE ✅** (Desarrollo; prod intacta): backend P1-P5 `pedidos-core`+`wompi-core` (**54 tests**) + reglas `webhookEvents` CF-only (214/214) + front `pago-web.js`+carrito (flag `WOMPI_WEB_ENABLED=false`, build verde) + índice `pedidos(estado,reservaExpira)`. Todo commit+push. Reglas duras: webhook=verdad (solo APPROVED; DECLINED audita) · reaper OBLIGATORIO · monto vs congelado.
> · **SIGUIENTE = DEPLOY 2a** (operativo·Daniel·money-critical): secretos TEST del panel (integridad/eventos/privada/pública — Daniel pega, Claude NO teclea tokens)→Secret Manager · `firebase deploy functions,firestore:rules,firestore:indexes` sandbox (L-22) · URL Eventos en panel · encender flag + CSS `.ck-pay-legal` · **gate live** (tarjetas prueba + endpoint by-reference del reaper). 2c=legal(TODO-49)+llaves prod(Kary)·App Check.
