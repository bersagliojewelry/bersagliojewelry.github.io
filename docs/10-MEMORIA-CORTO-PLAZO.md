...
> 2026-07-06 (neurogénesis §G.5): **shard `20`→`21-ESPACIAL-ADMIN`** (20 reventó su cap con §166): lo espacial del PANEL (CRM/POS/Pedidos/roles) vive en la hija; `20` queda con lo público + puntero. Registrada en CLAUDE.md §0 + mapa/ruteo de `00`.
> 2026-07-06: **§166 código público BJ-XXXX-XXXX con Fable** (directiva Daniel: el #N era anti-corporativo y filtraba volumen) — comité ×3 acotado (unánime opción A; CRUDO→bóveda) · generador crypto alfabeto-29 + unicidad transaccional `codigosPedido` · fuga DevTools cerrada (payload público sin `numero`) · buscador tolerante · backfill 6/6 vía MCP (⚠️ ADC gcloud caducado en esta máquina) · TDD 40 puras + 40 emulador + 221 reglas · deploy functions+rules+Pages → ADR §166. Pend menor: guía de dictado Kary → F2.3.
