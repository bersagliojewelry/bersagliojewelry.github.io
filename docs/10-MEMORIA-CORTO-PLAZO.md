...
> - **FIXES HECHOS+COMMITEADOS** (`Desarrollo`, [OPUS-4.8]): HIGH clamp acotado por reducción FIFO probada (`Σ orig−pendiente`) → `be342aa` · BLOCKER gate del corte + knob en corte → `6d127ba` · MEDIUM knob `acuerdoIncumplidoCuotas` cableado en panel (cuentas/cuenta/auditoria) → `bff9972`. Cada uno con test + build/suites verdes.
> - **RESTAN antes de R6**: (a) MEDIUM `acuerdosLargos` (`crm-auditoria.js:208-223`) excluye la clienta ENTERA → excluir por movId cubierto (+ corregir el test 250-258 que consagra el hueco) · (b) LOWs: UI owner "Cancelar acuerdo" (`anularAcuerdo` en `crm-service.js` SIN callsite) · guard `navigator.onLine` en modal factura/cuotas (`cuenta.js` wireModal) · `acuerdoAlCorte` para plan honrado (`deudaCubierta≤0`) · test conductual de paridad con `opts.acuerdos` · (c) **script de encendido TIPADO+verificado** de los 3 flags (el seed NO los siembra; sin él se enciende a mano sin validar tipos — `'true'` string pasa la regla pero la fórmula lo trata como ausente) · (d) **CENSO** `collectionGroup('acuerdos')`=VACÍO en prod ANTES de encender (Admin SDK salta reglas). Luego: re-verificar (comité+emulador) → consolidar ADR §81.9 + CRUDO comité a bóveda → R6 (deploy reglas+functions+índices ANTES de bandera · encender con el script · prueba en vivo con Daniel · `git fetch` L-26).
> - **CRUDO comité (archivar a bóveda antes de cerrar)**: `…/tasks/w71voltb9.output` (degradada, GO falso) + `…/tasks/wh5hsk9is.output` (completa, NO-GO, 15 confirmados). ⚙️ todo [OPUS-4.8] interino. Detalle → ADR §81.
