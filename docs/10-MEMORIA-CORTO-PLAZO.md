...
| TODO-35 | **Visibilidad SITE-WIDE** (SEO·AEO·GA4·GSC·Maps·IA → #1; vía SSG portado de Altorra=HUB; spec/research→bóveda `2026-06-25-*`). **🟢 A1 (piezas) HECHA — EN PROD ✅ (PR#345)**: generador `scripts/generate-pieces.mjs` (`/pieza/<slug>.html` Product+Breadcrumb+OG+noscript+PRERENDERED; guards selftest+bake-integrity; `noindex→index`) + `js/core/urls.js` SSoT + 9 callsites→URL limpia + `404.html` fallback + `robots.txt` bots-IA + cron diario. **🟢 FASE B (marca/Maps) HECHA — EN PROD ✅ (PR#345)** (datos reales de Daniel): `tenant_config.json`→JewelryStore+WebSite JSON-LD en `index.html` (NAP/sameAs reales; geo omitido cero-demo) + `seller` en piezas; **FIX live**: IG/FB rotos→reales (`@bersaglio_jewelry`/profile) + TikTok + dirección/horario. **A2a HECHA** (landings catálogo+journal indexables: CollectionPage/Blog+noscript+sitemap; 0 ítems hoy). **Falta**: A2b por-cat(`/coleccion/<slug>`)+artículo+migrar `?col=`→limpio · C GA4 (⚠️ **2 IDs**: analytics.js G-HS26X60DK3 vs Firebase G-F0CEWY7SP1; Consent Mode v2/banner falta)+GSC · **push+merge** (locales) · prompt HUB. ⚠️ App Check Enforce (TODO-14) bloquearía `generate` anónimo → `FIREBASE_SA_KEY` (ya soportado). | 🟡 | A2 + C + deploy |
