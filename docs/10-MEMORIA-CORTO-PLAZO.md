...
>
> **▶️ RETOMA (chat nuevo)**: **§103 F1 (LQIP "blur-up") HECHA** (§104, en Desarrollo; unit 257/257 + reglas 187/187 + build verde · cache v24 · **sin cambio de reglas**, whitelist es por-sección L-48 · pendiente merge de Daniel). **RETOMA = §103 F2 (router falso-SPA)** (TODO-30): toca TODA la nav (forms/drawers/scroll/atrás-adelante/SEO) → **comité #2 de casos borde + gate empírico**; ground-truth en §103.2/.5. Boot §G.1 + leer §103/§104. Resto de cola ↓.
>
...
>
> **▶️ 2026-06-23 — §104+§105** ([OPUS-4.8]): **§104** §103 F1 LQIP "blur-up" (Desarrollo; cache v24; sin cambio de reglas L-48; + reparado test-rot §102 en `no-demo-home`→M-07). **§105** bug Daniel (prod): imágenes con "zoom/asentamiento" en RECARGA = la entrada `.reveal` replay sobre secciones YA visibles (NO resize — caja medida estable); fix SISTÉMICO en `reveal.js` (lo visible se asienta sin animar `.reveal-static`) → cubre todas las secciones con imágenes. Detalle → **99 §104/§105** · L-47/L-48/L-49. Pendiente: merge de Daniel.
>
> **🚦 COLA REAL — gated/pendiente**: (1) **§103 carga app-like — F1 (LQIP) ✅ §104** (en Desarrollo); **F2 router falso-SPA = TODO-30** (toca TODA la nav → comité #2 casos borde + gate empírico; F3 opcional SSG-bake) → **sesión fresca**; (2) **alinear cerebro de cars** (sesión dedicada); (3) borrado auto imagen Storage (irreversible); (4) C1 (PERF-07); (5) responsive; (6) limpiar CSS muerto + `/img/*` demo; (7) re-verificar catálogo/pieza CON piezas cuando Kary cargue. **Regla**: skill `arquitecto-software` SIEMPRE al construir. `[[feedback_workflows_acotados]]`. [OPUS-4.8].
