...
> - **F3 (barandas) ✅ construido** (`3c91ecf`, en `Desarrollo`, pendiente merge): aviso `beforeunload` + confirm al cambiar de pestaña + indicador "Publicado ✓ / ● Cambios sin publicar" en todo el CMS singleton. Daniel hace smoke en vivo tras merge.
> - **SIGUE (continuar el plan)**: `global` (datos de contacto compartidos ~5 comp.) · `hideWhenEmpty` general · usuarios/SPA P5. **Pendiente GC**: ADR WYSIWYG (F1+F2) + P3.5 + F3 a `99` (aún sin consolidar).
>
