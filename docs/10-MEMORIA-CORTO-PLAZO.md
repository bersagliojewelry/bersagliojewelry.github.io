...
>
> **▶️ RETOMA = §108 FASE 2 (diff-gate)**. El workflow completo (mandato Daniel) ya corrió: comité ×5 + Gemini doble-ciego → **4 bloqueantes cazados ANTES de codear** (§108.7-.12; CRUDO+síntesis en bóveda `2026-06-23-108-cache-inteligente-*`). Veredicto = **GO en 3 fases**. **F1 ✅ implementada+verificada local** (`firebase-config.js`: caché persistente SOLO-público + feature-detect; CRM/dev=memoria — el bloqueante #2 es que la caché es global y contagiaría el CRM de dinero). **AHORA F2**: diff-gate por sección, firma `id+_version+StorageURL` ordenada (ante duda RE-RENDERIZA, I1), envolviendo mount+`observeReveals` (sin regresión §105). Luego **F3**: Cache-Control+URL versionada (APLAZAR LQIP §106). **PENDIENTES de Daniel**: merge `Desarrollo`→`main` (despliega §107 F2.0 + revert §106 + v27 **+ F1 §108**) · verificar UX F1 con catálogo real / gemelo. Leer **§108.7-.12** + L-52 EXT antes de F2.
>
