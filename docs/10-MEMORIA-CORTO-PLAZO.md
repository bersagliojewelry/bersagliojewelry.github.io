...
>
> **▶️ RETOMAR AQUÍ — PANEL v2: F-CHASIS-A DESPLEGADO; siguiente F1** (2026-06-07, **ADR §50**):
...
> - **▶️ SIGUIENTE = F1** (dominio `estadoCuenta`: helper puro espejo de `saldo.js` + centralizar signo/color/etiqueta) → su propio spec→plan→build. Luego F2 (fecha real/aging) … F7 (ventas+factura+pagos, **gate Consejo Externo**) … F9. Roadmap completo en el spec §10.
