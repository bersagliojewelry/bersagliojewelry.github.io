...
>
> **▶️ 2026-06-23 — reconciliación de relevo** ([OPUS-4.8]): el relevo previo marcó como "PRÓXIMO" 2 tareas que ese MISMO día ya había cerrado (auditoría semántica=§97, NO vencida; F2-CRM colas mudas=§98, `crm-service.js` ya usa `subscribeWithRetry`). **Meta-lección**: cruzar el relevo vs `deepAudit`+ADRs del propio turno antes de cerrar.
>
> **▶️ 2026-06-23 (cont.) — §100 cero-demo en multimedia** (`36da485`, local Desarrollo, **pendiente merge Daniel**): el §99 (flash Nosotros) tenía **RCA ERRADA** y su fix se REVIRTIÓ. RCA real (verif. navegador real): el "foto vieja" era un **fondo CSS demo** (`.abt-hero-image-bg`→earrings); el `_siteReady`/`withoutImages` del §99 lo empeoraba. Auditadas todas las secciones: 3 fondos CSS demo (earrings/ring/model) + `FALLBACK_IMG` categorías → superficie de marca (cero-ficción). Móvil: `.abt-hero-image` 1×2→340×425 (`+width:100%`). Cache v22. Lección **L-45** (corrige L-44). [[feedback_no_demo_en_index]].
>
> **🚦 COLA REAL — gated en Daniel**: (1) **confirmar §100 en prod** — tras merge, Daniel abre Nosotros y confirma que NO parpadea + foto visible en móvil; (2) **§101** — Daniel corre el prompt Gemini (`2026-06-22-consejo-externo-proceso-prompt.md`) → veredicto + cierra `proceso-decision-fuerte`; (3) **decisión banner-hero/gema** (¿marca o demo a quitar? hero Home + joya atelier Home, conservados por ahora); (4) **borrado auto de imagen vieja de Storage** (feature aparte: diff URLs + `deleteObject` solo tras `setDoc` OK; IRREVERSIBLE → gate empírico, NO con contexto saturado); (5) **arranque C1** (PERF-07); (6) responsive fino. Housekeeping: borrar archivos `/img/*` demo (sin refs en prod). `[[feedback_workflows_acotados]]`. [OPUS-4.8].
