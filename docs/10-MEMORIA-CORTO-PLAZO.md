...
>
> **🔴🚦 FRENTE VIVO (relevo): TODO-40 MODELO INVENTARIO — Fase 1 EN CURSO.** **Backend COMPLETO+verde: B1 migración + B2 reglas + B3 CFs**. **B4 admin UI ✅** (`98220b1`: enum3+visibilidad+cantidad CF-only+ajustar/cambiar-tipo vía CF; build+17t verdes). **Siguen B5 SSG+cliente · B6 verif E2E** (⚠️ gate deploy en la fila: B4+B5+migración = 1 unidad; NO merge a main hasta B5). Precede la carga masiva.
> _Frente previo **TODO-37 paso 7** (catalogo.json 7b/c/d) PAUSADO; pasos 1-6 mostrador EN PROD (§122-§130). Decisiones pago §121 → `[[project_comercio_pagos]]`. Banco de pruebas: 9 piezas `seedDemo:true` en prod (§121)._
> ⚠️ **Pruebas en vivo DIFERIDAS al final del plan+rediseño** (§130.4; build+tests por commit = red). **Deploy** (L-22): reglas/functions = manual mío; sitio+merge a `main` = PR de Daniel (`git fetch`); Admin SDK = ADC. Cola: TODO-39 · TODO-33 · TODO-35.
...
| TODO-40 | **MODELO INVENTARIO MULTI-TIPO** (Decisión Fuerte, PRECEDE la carga masiva). Diseño v3 §12 (comité ×4+Daniel+Gemini+comité v2 ×5). **F1 BACKEND COMPLETO+VERDE**: **B1** migración (`a45d44a`, `js/admin/inventario-model.js` modelo puro enum3 `finito`/`finito_refabricable`/`encargo`+`derivarEstado`+invariantes; runner `migrate-inventario-v3.mjs`; 17t) · **B2** reglas (`f255141`, enum3+`visibilidad`+`cantidad` CF-only por IGUALDAD update / permitida create + read blindado fail-closed; 212t) · **B3** CFs (`e45ef00`+`78c6ffe`: `crearPedido` decrementa+estado derivado+ledger `pieces/{id}/movimientos`; `ajustarStock`/`cambiarTipoPieza`; 19+9 int). **B4 admin ✅** (`98220b1`: `admin-piezas.html`+`piezas.js` select enum3+`visibilidad`; cantidad CF-only — editable solo al CREAR, en edición read-only + control "Mover stock"→CF `ajustarStock`; cambio de tipo→CF `cambiarTipoPieza`; `js/inventario-service.js` NUEVO; build+17t verdes). **Siguen: B5 SSG+cliente** (`generate-pieces` filtra `visibilidad!='privada'`+PreOrder refab+excluir sitemap; query pública filtra) · **B6 verif E2E**. ⚠️ **GATE DEPLOY**: migración `--apply` + B5 (query filtra) ANTES de desplegar reglas v3 (si no, catálogo público se rompe/filtra); B4+B5+migración = 1 unidad, NO merge a main hasta B5. | 🟡 | F1 B5-B6 |
...
>
> **▶️ Vivo**: TODO-40 Fase 1 (inventario v3). **Backend ✅** B1+B2+B3 (`a45d44a`/`f255141`/`e45ef00`/`78c6ffe`; 17+212+19+9 tests). **B4 admin ✅** (`98220b1`). **Siguiente: B5 SSG+cliente** (`generate-pieces.mjs` filtra `visibilidad!='privada'` + `availability:PreOrder` para `finito_refabricable` + excluir privadas del sitemap; query pública en vivo filtra) → B6 verif E2E (§12.3). Crudo+síntesis → bóveda `2026-06-26-*inventario*`.
