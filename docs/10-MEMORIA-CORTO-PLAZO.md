...
> **Avance B1 (detalle → ADRs §122-§130)**: pasos 1-6 + COP **EN PROD ✅** (PR #369/#370; Mostrador OPERABLE: crear·pago·VOID·arqueo Z·bruto-neto·export contador). **➡️ paso 7 `catalogo.json` a CDN — DISEÑO v3 LISTO** (flujo fuerte COMPLETO: arquitecto+comité ×4+Gemini; spec `2026-06-26-b1-paso7-catalogo-cdn-design.md`; veredicto **C**; Gemini cazó bug letal SEO [SSG hornea vendidas como `InStock`] + contrato incompleto + revalidación-ficha). **Pend. SOLO: luz verde Daniel + decisión negocio 10.1** (pieza vendida en el listado) → codear 7a-7d. · decisión 4b apartados (TODO-39).
