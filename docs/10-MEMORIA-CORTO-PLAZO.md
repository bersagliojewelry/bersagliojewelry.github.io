...
>
> **TODO-24 (index 100% gestionable + CERO ficción) — Fase A+B EN PROD** (§88; `feedback_no_demo_en_index`). Fase A `4ae6c0f` + Fase B (PR #272 mergeado, v18) + **href obligatorio en Redes** (`8abaab4`, decisión Daniel). **Reglas DESPLEGADAS y verificadas en vivo 2026-06-20** (films/socialPosts/journal-gate/href; censo prod = 0 publicados → sin lockout). **▶️ RESTA SOLO**: Daniel mergea `Desarrollo→main` el v19 cliente (href form + SW v19) — la web v18 ya está en prod; las reglas ya. Luego Kary carga Videos/Redes.
>
...
> **2026-06-20** (cont. "continua"): **TODO-24 Fase B CONSTRUIDA (§88)** — SSoT umbrales/completitud · panel Videos/Redes (`select`) · puerta reglas films/social + journalValid endurecido (`.trim()`) · defensa en profundidad (render re-filtra) · UX Kary · gate barrera #5 · SW v18. Review 4 ag. → HIGH+MED corregidos. `e3406ab`+`c56bb47`, **mergeado PR #272 (v18 en prod)**. [OPUS-4.8].
> **2026-06-20** (cont.): **href obligatorio en Redes** (decisión Daniel) — socialValid + isSocialComplete + form `required` + SW v19 (`8abaab4`). **Reglas DESPLEGADAS a prod + verificadas en vivo** (`firebase deploy --only firestore:rules`; censo prod journal/films/social publicados = 0 → sin lockout). rules 186✓ puros 21✓ build✓. Resta: Daniel mergea v19 cliente (la web v18 y las reglas ya están en prod). [OPUS-4.8].
