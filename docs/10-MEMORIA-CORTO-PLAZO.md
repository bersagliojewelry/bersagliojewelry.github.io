...
| TODO-28 | **Correcciones web** (Daniel 2026-06-21 navegando prod; plan 5 fases + 4 decisiones → recon 7 ag. en bóveda). **F1 ✅** (FAQ · journal vacío · will-change `efeb13c` + entrada falso-404 `69b3a94`; verif. navegador). **F2 público ✅** (`69b3a94`): helper `subscribeWithRetry` (backoff + retry al `online`) en el único `onSnapshot` → colecciones/piezas/journal/films/social + reviews + inquiries robustos; revisión adversarial ×2 (C-1 idempotente/C-2/H-2 corregidos). **Pend. F2**: colas mudas CRM `crm-service.js` (~11 listeners, L-40, dinero → pasar por el helper). **F3** CMS: foto Editorial home + Atelier + FAQ Contacto editables. **F4** perf: `.glass` blur fuera de listas N (catálogo/wishlist/admin). **F5** comité+consejo: borrado-colección = **bloquear si tiene piezas** (decidido) + carga/orden/motion copiando cars+insema **mejorado**. | 🟡 F1 hecho | sigue F2 |
...
>
> **2026-06-19/20** (§85-§88, todo en ADR): CMS `global` inc3/4 (prod) · ACUERDOS R6 A8 resuelto (§87, gated) · **ÍNDICE 100% CERO-FICCIÓN CERRADO** (§88; reglas desplegadas+verificadas, censo prod vacío; SW v20) · Daniel **reset-a-cero** + prioridad index (TODO-24). (≤18-jun → §37-§84.) [OPUS-4.8].
> **2026-06-21** (todo en ADR): **CAZA-BUGS** (§90/TODO-25: reflejo camino-vivo + W-10 + skill `caza-bugs` + gate L-42 ×5; propagada `§G`→cars TODO-26 ✅; shard→TODO-27) · **bug Categorías** (§89 `6b327a0`: render no montaba `<section>` desde 0 ítems). **Resta: Daniel mergea cliente v21.** [OPUS-4.8].
> **2026-06-21 · CORRECCIONES WEB (TODO-28)**: recon 7 ag. (bóveda) → 5 fases. **F1 ✅** (`efeb13c`+`69b3a94`) · **F2 público ✅** (`69b3a94`: listener live-sync robusto re-suscribe; rev. adversarial ×2). Verificado en navegador. Pend. F2 = colas mudas CRM; sigue F3 (CMS fotos+FAQ). [OPUS-4.8].
