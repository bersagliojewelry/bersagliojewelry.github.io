...
| TODO-40 | **MODELO INVENTARIO MULTI-TIPO** (Daniel 2026-06-26, Decisión Fuerte, PRECEDE la carga masiva): diseño + comité ×4 + Daniel ✅ → **Gemini ✅** (RECHAZÓ §10; 13 claims VERIFICADOS, 2 bombas reales) → **visión Daniel "web cobra AUTO Wompi/PSE, Mercado Libre"** (reactiva reaper) → **modelo v2 §11** → **comité v2 ×5 ✅** (`aprobado_con_cambios`; C1-C5) → **modelo v3 §12**. Crudo+síntesis → bóveda (`2026-06-26-gemini-*`+`-comite-v2-*`). **Plan F1 (precede carga): migración legacy PRIMERO + cantidad CF-only + visibilidad blindada + enum3 + ajustarStock/ledger + tests-en-rojo.** | 🟡 | **OK Daniel → implementar F1** |
