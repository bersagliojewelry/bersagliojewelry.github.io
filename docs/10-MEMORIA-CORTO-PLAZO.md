...
> 2026-07-24 · **[OPUS-5] B5 · D6 ✅ en código**: tarjeta "Reglas del sistema" (Negocio y equipo) pasa a EDITABLE owner: CF `actualizarConfigSistema` + whitelist cerrada `CAMPOS_CONFIG` (7 campos caja/fiscal) + rangos + MERGE + audit en `saludEventos`. TDD 7 tests ANTES del código → integración **22/22**. **2 precisiones**: `reteIcaXMil` valida 0-100 ‰ (NO fracción 0-1, lo habría roto) · audit nace `resuelto:true` (si no, cada cambio encendía una alarma falsa en "Avisos" del Hoy). **Zonas calientes V1/V18/V17/D9 NO tocadas.**
> 2026-07-23 · **B4 ✅ en código**: Bandeja sección "Tesorería" (aprobar/rechazar-con-motivo) + badge · "Plata total" en Hoy (`sumaSaldosReales`, inv.2) · V9 socias (`throughputAnio`). Gates: build · vista 8/8 · paridad 5/5. **Dudas (R7)**: V9 = informativo (tope 200 movs → dice "o más"), NO cifra fiscal · rechazo por `window.prompt` · E2E vivo deferido al deploy (L-05).
