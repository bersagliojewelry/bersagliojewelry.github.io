...
>
> **▶️ RETOMA — TODO-28 web (perf + UX móvil) ✅ EN MAIN** (PRs #289-292; `git fetch` 2026-06-22). Sesión cerró: `.bj-lite` por capacidad · carga fluida del index (reservar alto NO skeleton, 3 estados, cap 6 colecciones centradas) · fix del flash en TODAS las páginas data-dependientes (readiness real + watchdog) · scrollRestoration manual · header auto-oculto + iconos a la derecha en móvil · 3 bugs móvil (franja blanca `100lvh`, espacio hero/header, touch dock) · auditoría responsive (5 págs sin scroll horizontal). Comité ×3 + Gemini (bóvedas perf-v4 + carga-fluida-v3); detalle `45` PERF-04/05. **PRÓXIMO**: (1) **consolidar esta sesión a ADR `99` + fila `00`** (deuda doc); (2) **fuentes** (quitar pesos no usados — invisible, quick win); (3) **arranque C1** (pre-render above-the-fold — estructural, remate Gemini); (4) TODO-28 **F2-CRM colas mudas** + **F5** borrado-colección-bloquea-si-piezas; (5) responsive fino = device-driven. Lección: `[[feedback_workflows_acotados]]`. [OPUS-4.8].
