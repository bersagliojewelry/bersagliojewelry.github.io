...
> - **P4 (Nosotros editable) ✅ EN PROD** (§83; validado Daniel): listas + modelo PLANO 12 claves (2ª opinión Gemini) + headers editables + guards poison-pill + cap reglas 24 + **imágenes** (hero/atelier/fotos equipo) — todo mergeado. Detalle → §83 + spec §P4.
> - **F3 barandas ✅ + `global` inc 1-2 ✅ construidos**: F3 (aviso al salir sin publicar + "Publicado ✓") · `global` = datos de contacto **FUENTE ÚNICA** (whatsapp/email/instagram en `global.contacto`; footer + página Contacto derivan; **corrige el WhatsApp falso** duplicado). Pestaña "Datos globales". inc1 (`7174348`) mergeado; **inc2 (`f52de0e`) PENDIENTE del último merge de Daniel** (sin deploy de reglas).
> - **SIGUE (plan)**: `global` inc 3 (wa.me restantes: FAQ CTA de contacto, carrito, lista-deseos, wishlist) · `hideWhenEmpty` general · usuarios/SPA P5. **GC pendiente**: ADR a `99` de WYSIWYG (F1+F2) + P3.5 + F3 + global (hoy solo P4 tiene ADR).
>
