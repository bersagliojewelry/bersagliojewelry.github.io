...
| TODO-42 | **Wompi F2 (cobro web)** — 2a DESPLEGADA + **GATE LIVE PASADO ✅ (§147, 2026-06-28)**: e2e real en sandbox (Playwright→Widget Wompi→`4242` APPROVED→pedido `pagado` por webhook). Bug `wompiEligible` (`available`→`cantidad`) arreglado. **Falta 2c**: legal (TODO-49) + llaves prod (Kary) + App Check + quitar precios temp + encender flag en Pages. SSoT → spec `2026-06-28-wompi-checkout-web-design`. | 🟢 | 2c (legal+llaves prod) |
...
| TODO-57 | **Modelo de datos de la GEMA** (Decisión Fuerte §150 + consejo §151, modelo PLANO `badgeGem`/`gemFilterIds`). **HECHO**: fundación (`gem-taxonomy.js`+`gem-badge.js`, test 7/7) + **form admin** (select "Gema principal"+filtros+rename stone; escribe `specs.badgeGem`/`gemFilterIds`; reglas OK `specs is map`; SW v54/APP v31). **Pend**: prueba live del form (login admin) · backfill 32 (batch; fallback las cubre meanwhile) · `settings/gems` editable + hornear catalogo.json · JSON-LD canónico · filtros TODO-50 + retirar fallback. SSoT → spec. | 🟡 | login + batch migración |
