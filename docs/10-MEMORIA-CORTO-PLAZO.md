...
| TODO-58 | **Buscador por código** (spec `2026-06-29-buscador-codigo`). Catálogo HECHO ✅ (evolucionó a filtro inteligente, TODO-60). **Ref. ficha ✅**. **Pend (NEXT=slice 2: link `/p/<código>` SSG stubs OG)** · acceso inicio · 404. | 🟡 | slice 2 |
...
> **§153 (TODO-59):** selector de **color del oro** (amarillo/blanco/rosa) en el form admin → `specs.metalColor`; display "Oro blanco 18k" (ficha+JSON-LD). `metal.js` puro test 5/5. Sin cache bump.
