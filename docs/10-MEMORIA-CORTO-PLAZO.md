...
| TODO-68 | **PLAN ÚNICO ERP v4** (SSoT: spec 2026-07-04): Ítem 0 → F1-PUENTE → F1-CORE (pedidos+logística) → F2 POS completo → F3 inventario → carril D → F4 reportes → F5 CRM360 → F6. Comité ×3 ✅ (CRUDO en bóveda); prompt consejo externo listo (opcional, no bloquea). | 🟢 | D-1/D-2 (solo F1-flete y F2.4) |
| TODO-39 | **Apartados/abonos (= F2.4 del plan v4)**: D-1 preguntada a Daniel 2026-07-04. Si sí → pagos 1..N REUSANDO cartera CRM; spec (Decisión Fuerte) en paralelo al cierre de F1. C.2 prerequisito ✅. | 🟡 | respuesta D-1 |
...
> 2026-07-04 (2ª sesión, parte 2): **PLAN ÚNICO ERP v4 DEFINIDO (TODO-68)** — Daniel pidió consolidar todo (POS+pedidos+logística+facturación+contabilidad+inventario+CRM) en un solo plan priorizado. Proceso: specs vivas + estudio ALTORRA (agente: patrones ingestión/dedup/reportes; ALTORRA NO tiene facturación/logística) + comité ×3 acotado (unánime APROBAR_CON_CAMBIOS; P0: flete post-pago como CARGO aditivo, snapshot intacto · slice puente anti-ventana-ciega · costuras items[]/costoSnapshot/clienteId sembradas en F1 · ruta corta POS · Bloque D → carril paralelo). CRUDO → bóveda. D-1 (apartados) y D-2 (flete) preguntadas a Daniel.
