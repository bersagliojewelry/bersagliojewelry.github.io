...
| TODO-58 | **Buscador por código** (spec `2026-06-29-buscador-codigo`). Catálogo HECHO ✅ (filtro inteligente, TODO-60) · **Ref. ficha ✅** · **link compartible `/p/<código>` (stubs OG) ✅** (SSG hornea `dist/p/<code>.html` con OG real+canonical+noindex,follow+redirect; `404.html` rutea `/p/`; SSG_SELFTEST+build+redirect probado en navegador vía `vite preview`). **Pend (NEXT=slice 3: acceso en INICIO)** · slice 5 = 404 polish. | 🟡 | slice 3 |
...
>
> **2026-06-29 (cont.):** **TODO-58 slice 2 ✅** — links compartibles `/p/<código>` (32 stubs SSG: OG+canonical+noindex,follow+redirect; `404.html` rutea `/p/`; sin cache bump). Verif: SSG_SELFTEST + build + **redirect probado en navegador** (`vite preview`/`dist/` → `/p/0581.html`→pieza). Detalle → spec §7 · lección [[L-61]]. NEXT = slice 3 (acceso en inicio).
