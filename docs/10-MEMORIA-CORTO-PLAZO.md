...
> **2026-06-20** (cont.): **href obligatorio en Redes** (decisión Daniel) — socialValid + isSocialComplete + form `required` + SW v19 (`8abaab4`). **Reglas DESPLEGADAS a prod + verificadas en vivo** (`firebase deploy --only firestore:rules`; censo prod journal/films/social publicados = 0 → sin lockout). rules 186✓ puros 21✓ build✓. [OPUS-4.8].
> **2026-06-20** (cont. "continua"): **Destacadas — último hueco del index** (`a566dc5`). Censo prod = catálogo VACÍO (0 piezas/0 colecciones) → el home mostraba el placeholder "afilando la curaduría". `featured.js` ahora aplica MIN_FEATURED y oculta sin placeholder (espejo Films/Redes); comentario stale de `categories.js` corregido (no hay fallback demo). SW v20 · gate no-demo Destacadas · no-demo 5✓ build✓. **Índice 100% cero-ficción cerrado**; resta merge cliente v20 (Daniel). [OPUS-4.8].
