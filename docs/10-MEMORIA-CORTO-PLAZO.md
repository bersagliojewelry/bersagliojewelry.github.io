...
> 2026-07-06 (parte 3): **UI F1-CORE hecha con FABLE** (Daniel extendió Fable; relevo Opus queda de reserva): módulo Pedidos operable (chips estado+canal/medio/fecha · KPI totalesFiltro · colas Por verificar/Revisar/Despachar · acciones espejo TRANSICIONES_PEDIDO con mini-forms → `avanzarPedido` · historial · aviso despacho · resolver sin-stock vía `anularPedido` [enmienda spec §4.3] · instructivo Kary) + POS checkbox requiereEnvio+hints. Paridad cliente↔CF por test (deepEqual, 16/16); suites locales 49 verdes; build OK; SW v74+APP v37. **FALTA: merge→main (deploy Pages) + gate E2E 3 flujos en prod + ADR (F1 se consolida al cerrar el gate) + push A.6.**
> 2026-07-06 (parte 2): **F1-CORE BACKEND EN PROD con Fable** (`994776a`, deploy 4 CFs): avanzarPedido por TABLA + historial/dayKey + flete aditivo/merma/POD + cancelado-repone + **fix P0 arqueo (ESTADOS_CON_DINERO)** + ruta corta POS (efectivo=entregado; requiereEnvio→flujo) + costuras items[]/costoSnapshot:null/clienteId:null. Tests 55 puras + 53 integración verdes.
