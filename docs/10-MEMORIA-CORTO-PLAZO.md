...
>
> **2026-06-10 (10)** · **M1 construido, cerrado SIN red-team** (commit en `Desarrollo`): solicitudes (SoD one-way) + gestiones (inmutables) + índice group; 98/98. El workflow red-team murió ×2 por rate-limit del SERVIDOR (no de la cuenta) → sesión cerrada en relevo curado para probar API fresca. ⚠️ Lección operativa: un workflow con `failures` y 0 tool_uses NO es un veredicto — releer `<failures>` antes de aceptar `confirmados: []`.
...
> **2026-06-10 (2-7)** · **F6 cerrado de punta a punta** (consolidado en **ADR §64-§68**): frente D reconciliación+Salud (§64, PR #214) · frente B RBAC claims (§65) · fix Desactivar+frontera users (§66, L-36) · CI reparado Java 21 (§67, L-37) · alerta de truncado (§68). Revisiones adversariales 24+19 agentes (L-34/L-35); deploys con OK de Daniel; 11 functions vivas. Detalle → `00-INDICE` → `99`.
