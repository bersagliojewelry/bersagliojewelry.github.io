...
> 2026-07-09 · **Auditoría Antigravity → §180** (ADR): hidratación sin skeleton ARREGLADA+VERIFICADA (`window.__BJ_CATALOG` inline + `data.hydrateFromBake`; preview 16 cards/0 skel) + copy verdad-de-marca (fuera superlativo falso). **DESPLEGADO** (`f0e9fe7`, cache v85). Copy nuevo en CMS (`siteContent/{home,nosotros}` MCP). **LECCIÓN (§180.7)**: el CMS Firestore SOBRESCRIBE los `*-defaults.js` → editar defaults NO cambia el sitio en vivo. **Pend TODO-47** (Kary): claims CMS sin verificar/inconsistentes (43 vs 12/40 años · +12.000 vs 5000/800 piezas · 12-40 países).
