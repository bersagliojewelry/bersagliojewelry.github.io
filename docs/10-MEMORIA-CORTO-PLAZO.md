...
> 2026-07-06 (parte 2): **F1-CORE BACKEND EN PROD con Fable** (`994776a`, deploy 4 CFs): avanzarPedido por TABLA + historial/dayKey + flete aditivo/merma/POD + cancelado-repone + **fix P0 arqueo (ESTADOS_CON_DINERO)** + ruta corta POS (efectivo=entregado; requiereEnvio→flujo) + costuras items[]/costoSnapshot:null/clienteId:null. Tests 55 puras + 53 integración verdes. **FALTA de F1: UI del módulo (spec §4, LISTA para Opus: filtros/totales/colas/acciones/historial + checkbox requiereEnvio en POS + SW bump) + gate E2E 3 flujos en prod + push A.6.** Sin ADR aún: F1 se consolida al cerrar el gate.
