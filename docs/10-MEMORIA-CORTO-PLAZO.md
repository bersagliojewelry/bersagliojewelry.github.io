...
>
> **🔴🚦 FRENTE VIVO: TODO-40 MODELO INVENTARIO — Fase 1 CÓDIGO-COMPLETA (en `Desarrollo`).** B1-B3 backend + **B4 admin** (`98220b1`) + **B5 SSG+cliente** (`3545065`) ✅. **B6 = coherencia E2E ✅ estática** (traza model↔reglas↔CFs↔admin↔SSG↔cliente; build+SSG-selftest+17t verdes); **pruebas en VIVO DIFERIDAS** (§130.4, dueño). **PENDIENTE = DEPLOY gated** (ver fila TODO-40): orden migración `--apply` → merge B4+B5 a main → deploy reglas v3. NO desplegado aún. Precede la carga masiva.
...
| TODO-40 | **MODELO INVENTARIO MULTI-TIPO** (Decisión Fuerte; diseño v3 → `docs/superpowers/specs/2026-06-26-modelo-inventario-multitipo-design.md §12`; comité ×4+Gemini+comité v2 ×5). **F1 CÓDIGO-COMPLETA en `Desarrollo`** (detalle por bloque → commits): B1 migración `a45d44a` · B2 reglas `f255141` · B3 CFs `e45ef00`/`78c6ffe` · **B4 admin `98220b1`** · **B5 SSG/cliente `3545065`** · B6 coherencia E2E ✅ (build+SSG-selftest+17+212+19+9 tests verdes; live DIFERIDA §130.4). **PENDIENTE = DEPLOY 3 pasos gated** (L-22): (1) `node scripts/migrate-inventario-v3.mjs --apply` (backfill `visibilidad='publica'` — sin esto el SSG aborta y la query `!=privada` vacía el catálogo) → (2) merge B4+B5 a `main` (sitio) → (3) `firebase deploy --only firestore:rules,functions`. Al cerrar deploy → ADR `99` + lección L-NN (query debe igualar el set legible de una regla row-level, o falla entera) + distilar `30`/`31`. PRECEDE la carga masiva. | 🟡 | DEPLOY (3 pasos) |
...
>
> **▶️ Vivo**: TODO-40 Fase 1 (inventario v3) **CÓDIGO-COMPLETA en `Desarrollo`** — B1-B3 backend + B4 admin (`98220b1`) + B5 SSG/cliente (`3545065`) + B6 coherencia E2E ✅ estática. **Único pendiente = DEPLOY en 3 pasos gated** (migración `--apply` → merge a main → reglas+functions; ver fila TODO-40). Pruebas en vivo DIFERIDAS (§130.4). NO desplegado. Cuando se cierre el deploy → ADR a `99` + fila `00` + retirar de aquí. Crudo+síntesis → bóveda `2026-06-26-*inventario*`.
