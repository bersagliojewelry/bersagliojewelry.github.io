...
| TODO-28 | **Correcciones web** (Daniel 2026-06-21; 5 fases + 4 decisiones → recon 7 ag. en bóveda). **F1✅ F2-público✅ F3✅** (`efeb13c`·`69b3a94`·`ce0175f`; verif. navegador): FAQ/journal-vacío/will-change/entrada-404 · listener live-sync robusto `subscribeWithRetry` (re-suscribe; cubre colecciones/piezas/journal/films/social+reviews+inquiries; rev. adversarial ×2) · fotos Editorial+Atelier editables (XSS verif.); FAQ Contacto ya era editable. **Pend.**: F2 colas mudas CRM (`crm-service.js` ~11 listeners, dinero → al helper) · **F4** perf `.glass` blur fuera de listas N (catálogo/wishlist/admin) · **F5** comité+consejo: borrado-colección=**bloquear-si-piezas** (decidido) + carga/orden/motion cars+insema mejorado. | 🟡 F1-3 ✅ | sigue F4 |
...
> **2026-06-21 · CORRECCIONES WEB (TODO-28)**: recon 7 ag. (bóveda) → 5 fases. **F1 ✅** (`efeb13c`+`69b3a94`) · **F2 público ✅** (`69b3a94`: listener live-sync robusto re-suscribe; rev. adversarial ×2). Verificado en navegador. Pend. F2 = colas mudas CRM. **F3 ✅** (`ce0175f`: fotos Editorial+Atelier editables; Contacto FAQ ya editable). Sigue F4 (perf cristal). [OPUS-4.8].
