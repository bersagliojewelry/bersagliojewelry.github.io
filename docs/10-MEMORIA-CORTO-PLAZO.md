...
> **Avance B1**: paso 1 inventario §122 · paso 2 calculadora §124 · **paso 3 §125-§126 = CERRADO (código)** — backend `crearPedido` CF (candado atómico=pieza→imposible doble venta; recompute server-side; snapshot inmutable; idempotente; núcleo `pedidos-core.js` testeable; 206 rules + 6 integración) **+ POS UI `admin-pos.html`** (`pos.js`+`pedidos-service.js`; Kary elige pieza→precio fijo/por peso→medio→confirma→`crearPedido`; UI ESPEJA a la CF; menú `role:catalogo`). **Reglas + CF `crearPedido` DESPLEGADAS a prod ✅**. **➡️ SIGUIENTE: paso 4 `registrarPago`** 1..N + comprobante por-verificar (spec §7).
> _**PEND. MERGE Daniel**: B1 paso 3 (`e6ab0a1` backend + `02ab6a7` POS UI — código en `Desarrollo`, NO en main). La **CF `crearPedido` YA está desplegada** a prod; falta el merge `Desarrollo→main` para que la PÁGINA (Pages) llegue a prod → luego Kary registra una venta de prueba EN VIVO (pieza `seedDemo`). Gotcha dev L-58._ Cola: TODO-33 · TODO-35.
...
| TODO-38 | **Unidad del precio fijo (USD vs COP)** (§126.6): el form de Piezas rotula `price` como "Precio USD" pero la CF `crearPedido` lo cobra como **COP enteros**. Aclarar/normalizar (rótulo · convertir · o quitar el campo si todo va por peso). NO bloquea el POS (mayoría de piezas = por peso, sin `price`). | 🔲 | decisión Daniel |
...
>
> **▶️ Hitos**: detalle en ADRs §88-§126. Vivo: **TODO-37 B1 mostrador — paso 3 (pedido) CERRADO en código** (§125 backend + §126 POS UI; CF `crearPedido` desplegada; pend. merge Daniel + verif. en vivo).
>
> **🚦 Próximo**: **B1 paso 3 (POS Mostrador) CERRADO en código + CF desplegada §126** → falta **merge de Daniel a `main`** (la página a prod) + **verif. en vivo** (Kary registra venta de pieza `seedDemo`). Luego **paso 4 `registrarPago`** 1..N + comprobante por-verificar (spec §7). Cola: TODO-33 (parpadeo) · TODO-35 (A2b/eventos/HUB) · TODO-38 (precio USD/COP). **Regla**: `arquitecto-software` SIEMPRE · flujo COMPLETO **W-11** cuando toque diseño/decisión cara · `[[feedback_workflows_acotados]]` · `[[feedback-reintentar-agentes-no-saltar-flujo]]`.
