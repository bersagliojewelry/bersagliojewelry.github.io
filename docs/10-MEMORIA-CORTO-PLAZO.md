...
| TODO-39 | **Apartados/abonos (= F2.4)**: **SPEC Decisión Fuerte HECHA 2026-07-09** (`specs/2026-07-09-f2-4-apartados-DISENO.md` + `-PROMPT-CONSEJO-EXTERNO.md` + mockup; comité ×3/17-ag → CRUDO+DIGEST en bóveda `2026-07-09-comite-f2-4-apartados-*`). Modelo: apartado=pedido `estado:apartado`+`turnoId:null`; anticipo=**PASIVO segregado** (no fiado — `computeSaldo` los excluye); **2 planos** (tesorería/turno vs revenue/entrega, libro→F4); stock=**hold**; IVA a la entrega; v1=pieza única/POS/penalidad-OFF. **Pend: decisiones dueño (§7 spec: régimen legal/cancelación/RBAC) + params Kary (anticipo 20%/plazo 60d) + consejo externo (Daniel corre) → implementar.** | 🟡 | decisiones dueño |
...
> 2026-07-08 · TODO-73/74/72/41 cerradas → ADRs §176-§179 (detalle allá). `main`≡`Desarrollo`. Gotchas: `test:rules` falla si `:8080` ocupado (matar PID); pestaña `hidden` throttlea skeleton (no bug).
> 2026-07-09 · **F2.4 apartados — SPEC Decisión Fuerte** (flujo fuerte W-11 COMPLETO): evidencia (3 lectores acotados + legal-CO + lectura `pedidos-core`/`caja-core`) → arquitecto → **comité ×3 acotado (5 expertos/17 agentes, inline+schema sin tools; CRUDO+DIGEST en bóveda)** → veredicto. Comité cazó **P0s reales** (anticipo=PASIVO no fiado — sino contamina `saldoActual`/aging; revenue sin hogar con `pagos:[]`; stock=hold no consumo; idempotencia faltante; `cerrarTurnoCore` SÍ se toca) → **verificados en `saldo.js`/`caja-core.js`**; **moderé sobre-ingeniería** (difiere `libroReconocimiento`/outbox/5-reconciliadores a F4 — L-50). 3 artefactos entregados: DISEÑO + PROMPT-CONSEJO (`specs/2026-07-09-*`) + mockup. Decisiones §7: **cancelación RESUELTA** (comité enfocado 4-crít + evidencia legal `.gov.co`: art.43.5 prohíbe decomiso → reembolso 100%/dinero + saldo-a-favor VOLUNTARIO **sin vencimiento**; costo-oport. se ataca upstream; §7.1 spec + EVIDENCIA-LEGAL en bóveda) · **RBAC=cajera** · **20%/60d** (Daniel 2026-07-09). **Falta: Daniel corre consejo externo + gate abogado CO + implementar.**
