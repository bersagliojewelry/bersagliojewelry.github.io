...
>
> **▶️ 2026-06-23 — §104 (EN PROD, PR #306) + §105 (desplegado v25)** ([OPUS-4.8]): **§104** = F1 LQIP placeholder (⚠️ solo imágenes subidas DESPUÉS; existentes sin LQIP). **§105** = fix "zoom/asentamiento" en RECARGA (`reveal.js` asienta sin animar lo visible; NO era resize — caja medida estable). Daniel: NO percibió mejora porque su molestia real = **parpadeo entre PÁGINAS** = F2 (pendiente), no F1/§105. Detalle → **99 §104/§105** · L-47/L-48/L-49/M-07.
>
> **🚦 COLA REAL — gated/pendiente**: (1) **§103 F2 navegación instantánea (router falso-SPA o View Transitions) = TODO-30, SESIÓN NUEVA, PRIORIDAD** (elimina el parpadeo entre páginas; F1✅ en prod §104, §105✅ desplegado v25; F3 SSG opcional); (2) **backfill/re-subir imágenes existentes** → que tengan LQIP (las subidas pre-F1 no lo tienen); (3) **alinear cerebro de cars** (sesión dedicada); (4) borrado auto imagen Storage (irreversible); (5) C1 (PERF-07); (6) responsive; (7) limpiar CSS muerto + `/img/*` demo; (8) re-verificar catálogo/pieza CON piezas cuando Kary cargue. **Regla**: `arquitecto-software` SIEMPRE. `[[feedback_workflows_acotados]]`. [OPUS-4.8].
