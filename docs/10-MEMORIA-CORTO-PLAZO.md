...
> ✅ **Cerrados**: **TODO-60 (§155 buscador UNIFICADO: pseudo-input global + overlay = el autocomplete)** · **TODO-58 (§154 código + links `/p/`)** · TODO-59 (§153) · TODO-56/55/51/52/53/54/44/45/46 · TODO-40+32 → ADRs/`99`. Pend Daniel: precios + fotos IA. (Pulido OPCIONAL del buscador: highlight `<mark>` · fuzzy.)
...
>
> **2026-06-29 (buscador UNIFICADO · §155):** Decisión Fuerte W-11 (arquitecto+comité×4+Gemini+mockup+gate navegador). Veredicto post-Gemini: **pseudo-input GLOBAL en el header + overlay único** → corrige el híbrido (lands-anywhere=visibilidad global; cero duplicación de DOM). Ícono lupa→pseudo-input visible (móvil "Buscar", ≤480 monograma B); **eliminada la banda del index (§154 slice-3 SUPERSEDED) + "Buscar" del dock**; overlay endurecido a11y/móvil (focus-trap·inputmode·aria-live·scroll-lock iOS·contraste). Gate: desktop 1024/1280 overflow 0, móvil 390/360 "Buscar" visible 94px. TODO-60 cerrado (el overlay ES el autocomplete). Crudo→bóveda. **Pend: DESPLEGAR.** Cosmético a revisar Daniel: monograma B móvil ≤480px.
> **2026-06-29 (cont.):** **TODO-58 CERRADO → ADR §154.** Slices 2 (links `/p/<código>`, 32 stubs SSG OG+canonical+noindex+redirect) · 3 (buscador en el INICIO, banda discreta; `.bc*`→`components.css` compartido) · 5 (404 recupera por código → `/colecciones.html?q=`). Verif: build · test:codigo 11/11 · SSG_SELFTEST · navegador (redirect/submit/404 GET). Lecciones [[L-61]] · [[L-05]]. Sin cache bump. **DESPLEGADO ✅** (merge Desarrollo→main `d359e65`; prod verificado: `/p/0581` stub + 404 con buscador vivos). TODO-60 (autocomplete) sigue abierto.
