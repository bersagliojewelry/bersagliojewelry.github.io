...
| TODO-28 | **Correcciones web** (Daniel 2026-06-21; 5 fases + 4 decisiones → recon 7 ag. en bóveda). **F1✅ F2-público✅ F3✅** (`efeb13c`·`69b3a94`·`ce0175f`; verif. navegador): FAQ/journal-vacío/will-change/entrada-404 · listener live-sync robusto `subscribeWithRetry` (re-suscribe; cubre colecciones/piezas/journal/films/social+reviews+inquiries; rev. adversarial ×2) · fotos Editorial+Atelier editables (XSS verif.); FAQ Contacto ya era editable. **Pend.**: F2 colas mudas CRM (`crm-service.js` ~11 listeners, dinero → al helper) · **F4** 🟡 perf (look IDÉNTICO, dir. Daniel; → `45` PERF-04): `content-visibility` en listas — catálogo ✅ `a32c56d`; falta deseos/journal/panel + aligerar aurora · **F4 perf ✅ §93** · **fuentes ✅ §94** · **F5 ✅ §95** (`2de1397`): borrado-colección bloquea-si-piezas (guard de integridad, conteo defensivo). **Pend. real**: **F2 colas mudas CRM** (dinero, verif. experta) · **arranque C1** (decisión Daniel) · responsive fino. | 🟡 | F2-CRM + C1 |
...
>
> **▶️ RETOMA — HECHO ESTE TURNO** (en Desarrollo, pendiente push+merge de Daniel → deploy Pages): **§94 fuentes VF** (`f926eca`: Google Fonts lista discreta→rango = fuente variable; cobertura idéntica, woff2 ~14→6; → `45` PERF-06) · **§95 F5 borrado-colección bloquea-si-piezas** (`2de1397`: guard de integridad en `colecciones.js`). **PRÓXIMO**: (1) **arranque C1** (pre-render above-the-fold — estructural, **Decisión de Daniel**, NO invisible); (2) **F2-CRM colas mudas** (`crm-service.js` ~11 listeners sin error-cb, dinero → `subscribeWithRetry`; verif. experta); (3) responsive fino. ⚠️ **TODO-27 (shard `30`) urgente** (~18c del tope). `[[feedback_workflows_acotados]]`. [OPUS-4.8].
