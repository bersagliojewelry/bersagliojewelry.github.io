...
>
> **≤2026-06-20** (todo en ADR §37-§90, detalle → `00`→`99`): CMS global §85-86 · cero-ficción índice §88 (reset-a-cero) · acuerdos R6 §87 (gated) · **CAZA-BUGS §90/TODO-25** (skill `caza-bugs` + gate L-42 ×5; cars §G ✅ TODO-26; shard→TODO-27) · bug Categorías §89. [OPUS-4.8].
> **2026-06-21 · CORRECCIONES WEB (TODO-28) + CIERRE DE SESIÓN**: recon 7 ag. (bóveda) → 5 fases. **F1✅ F2-público✅ F3✅ F4-catálogo✅** (`efeb13c`·`69b3a94`·`ce0175f`·`a32c56d`; verif. en navegador). GitHub Secret Scanning revisado (§41: key web **pública = benigno**; Daniel cierra las 3 alertas). Todo **merged a main** (`de20b0f`). **▶️ RETOMAR AQUÍ**: F4 resto = `content-visibility` en deseos/journal/relacionados/panel + aligerar aurora (look IDÉNTICO, dir. Daniel; receta → `45` PERF-04) → luego **F2-CRM** colas mudas (`crm-service.js`, dinero, al helper) → **F5** (borrado-colección=bloquear-si-piezas [decidido] + carga/orden/motion cars+insema mejorado, comité+consejo). [OPUS-4.8].
