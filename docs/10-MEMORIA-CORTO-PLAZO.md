...
> 2026-07-11 · **[OPUS-4.8] Journal ✅ pulido visual + CMS colocación (§186 · SW v95 · merge `0b6ef72`)** — capital 54px/1-col + `.hj-trio` centrado; campos `cover`(portada)/`order`+`featured`=destacada-franja; panel miniatura/orden/casillas; reglas `journalValid` DESPLEGADAS (manual); 6 docs migrados (portada Cómo-elegir + 3 destacadas Muzo/Oro/Anillo + orden 1-6). Tests 6/6 + reglas 243/243 + composición determinista real ✅; CSS live prod. **PEND (SEO A3, TODO-35)**: hornear `/journal/<slug>` Article schema (reemplaza `entrada.html?e=` noindex). Guías borrador → `specs/2026-07-11-journal-guias-A3-DRAFT.md`.
