...
>
> **▶️ RETOMAR AQUÍ — ACUERDOS DE PAGO: 5 SLICES CONSTRUIDOS ✅ (spec con checklist tickeada)**. Fórmula TRAMOS (15/15) · reglas (emulador 143/143, SIN desplegar) · corte (insumos 3/3) · UI Kary gateada por `config/cartera.acuerdosActivos` (apagada en prod) · vigilancia (16/16). **FALTA para encender**: (1) Consejo Externo (prompt en bóveda — Daniel lo pega en Gemini) → integrar adoptado/refutado; (2) `firebase deploy --only firestore:rules,firestore:indexes,functions`; (3) encender `acuerdosActivos=true` + `horizonteAcuerdoDias=730`; (4) verificación POR HITO (cadencia Daniel, memoria `feedback_verificacion_por_hitos`) + ADR §81 + CRUDOs + docs 20/50. PR del tren v16 cuando Daniel quiera (seguro pre-deploy: la bandera apaga todo).
