...
| TODO-69 | **Vista de AUDITORÍA de caja** (Fase 2 de "caja en tiempo real", Daniel 2026-07-07): línea de tiempo owner por turno (apertura/cierre/movs/ventas/traslados) en vivo, UID→nombre. Daniel eligió **pantalla dedicada**. **CONSTRUIDO** (`admin-auditoria.html`+`js/admin/auditoria.js`+nav owner+CSS `aud-*`, SW v81); front-end puro sobre data ya grabada. **Pend**: validación live Chrome + ADR + close. Futuro posible: log de acciones NO-caja (piezas/precios/usuarios) = `auditLog` central. | 🟡 | validación live |
...
> 2026-07-07 · **"Caja en tiempo real" (Daniel)** — **F1 reactividad ✅** (ventas recientes EN VIVO `onUltimasVentasChange` + panel de caja OPTIMISTA al abrir; `pos.js`+`pedidos-service.js`, SW v80). **F2 AUDITORÍA ✅ construida** (TODO-69): `admin-auditoria.html`+`auditoria.js` — vista OWNER solo-lectura, línea de tiempo EN VIVO por turno (quién abrió/movió/vendió/cerró + hora), UID→nombre con fallback que respeta root; nav owner; sobre la data que ya graban las CFs (cero backend nuevo). SW **v81**·APP **v43**. Build+brain verdes. **Falta**: validación live Chrome (Daniel refresca) + ADR del bloque + close TODO-69. Front-end puro (no toca `functions/`).
