...
| TODO-40 | **MODELO INVENTARIO MULTI-TIPO** (Daniel 2026-06-26, Decisión Fuerte): el negocio NO es todo único — lote/serie (POS decrementa `cantidad`) · única/a-medida · por encargo · **agotado-refabricable** (NUEVO). El POS debe decrementar (no marcar vendida toda) + escasez honesta. **PRECEDE idealmente la carga masiva de inventario** (mucho por cargar). Diseño + comité ×4 ✅ (spec `2026-06-26-modelo-inventario-multitipo §9`: "3 campos" NO basta → variantes/talla·precio-gramo·a-medida). | 🟡 | resp. Daniel A/B/C → consejo |
