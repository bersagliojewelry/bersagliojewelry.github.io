...
>
> **▶️ RETOMA — sprint perf + UX móvil ✅ EN MAIN** (PRs #289-293; §93). Paso (1) consolidar = ✅ hecho (§93, ya en main). **Paso (2) FUENTES = ✅ HECHO ESTE TURNO** (§94 · código `f926eca` en Desarrollo, **pendiente push+merge de Daniel** → deploy Pages): Google Fonts de listas de pesos discretas → **sintaxis de rango = fuente variable** por familia; cobertura de pesos IDÉNTICA (cero cambio visual) pero `@font-face` 72→25 / woff2 ~14→6. Verif. `document.fonts`+curl. SW sin bump (cross-origin). Detalle → `45` PERF-06.
> **PRÓXIMO** (resta de la fase perf): (3) **arranque C1** (pre-render/state-injection above-the-fold — estructural, "remate" de Gemini; Decisión aparte, NO invisible); (4) TODO-28 **F2-CRM colas mudas** (`crm-service.js` ~11 listeners sin error-cb, dinero → al helper `subscribeWithRetry`) + **F5** borrado-colección-bloquea-si-piezas; (5) responsive fino = device-driven. Lección: `[[feedback_workflows_acotados]]` (trabajo directo, no workflow desbocado). [OPUS-4.8].
