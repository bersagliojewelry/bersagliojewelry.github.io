...
> 🎉 **CRM + Fase M + Wompi web EN PROD** (§47-§82, §164). ⚙️ **OPUS 4.8 INTERINATO** (Fable sin cuota; Daniel avisa el retorno): commits `[OPUS-4.8]` + footer real + TDD estricto en `functions/` + deploy manual + NO tocar webhook/firma/reaper/snapshot. `[[feedback_opus_interino]]`
>
> **🧭 FOCO = PLAN ÚNICO ERP v4 (TODO-68)** — SSoT spec `2026-07-04-plan-unico-erp-v4`. **F1 COMPLETO** (§165-§168). **F2.0 caja + bóveda antirrobo COMPLETO ✅ EN PROD (§169, gate Chrome owner verde 2026-07-07)**: backend + UI desplegados (8 CFs + `config/caja` enforceTurno:false·fondo200k·límite4M) + 2 fixes post-gate (reponer turnoId · opId perezoso), SW v77. **PENDIENTE OPERATIVO (no dev)**: runbook §9.4 = Kary activa `enforceTurno` al empezar. **B5d DIFERIDO** (Daniel 2026-07-07: la cajera con su usuario la crea Kary después; todo se prueba desde el usuario del dueño → L-78 no se cierra ahora). **SIGUE en F2 (orden del comité, plan v4 §3)**: **2.1 vínculo pedido↔cliente** (buscar/crear cliente en mostrador + match web; clave canónica = `legal_id` normalizado dedup estilo ALTORRA = contrato del portal F5 → **Decisión** a documentar) → **2.2 factura multi-línea (TODO-41)** → **2.4 apartados/abonos (TODO-39)** (apartados EXIGE 2.2 que EXIGE 2.1 — NO saltar a 2.4). 2.3 térmica = BLOQUEADO por hardware (D-5). Precios NO bloquean; 1ª APPROVED viva. `[[project_comercio_pagos]]`
> _MCP Firebase=escritura prod · **push+merge a main=Claude sin preguntar** (Daniel 2026-07-07, deploy autorizado `[[feedback_claude_deploy_autorizado]]`) · consejo externo read-only · Claude valida en Chrome (`[[feedback_validacion_chrome_directa]]`)._
...
> 2026-07-07 (Opus · **§170 F2.0 pulido revisión adversarial UI — 4 hallazgos BAJOS, build+test verdes**): (a) `pos.js` handleCierre re-deriva el turnoId VIVO antes de escribir (no cierra un turno ya cerrado en otra sesión → avisa); (b) `boveda.js` salida/conteo con opId PROPIO+perezoso limpiado al éxito (+guard consignar↔reponer no reusa el opId de la otra); (c) `pos.js` auto-traslado inhibido también por mov-modal/confirm-dialog (`anyOverlayOpen`); (d) 6 listeners de caja (5+onConfigCaja) con `subscribeWithRetry` (bóveda queda cruda a propósito: onErr=toast se apilaría). SW **v78**/APP **v41**. Sin gate Chrome (edges no observables; L-05). ADR §170 + fila §00. ⚠️ Reconcilié: mi worktree salió de `main` (atrás de Desarrollo que ya tenía §169) → renumeré §169→§170, reparenté sobre Desarrollo. `[[project_comercio_pagos]]`
> 2026-07-07 (Opus · **F2.0 caja/bóveda COMPLETO → §169**): diseño (comité ×3 + Gemini) + backend B0-B4 + UI B5b-1 + deploy prod + **gate Chrome owner VERDE** + 2 fixes post-gate. Detalle → **ADR §169** + spec §8-§10. Lecciones L-73..L-78. Pend operativo (Kary): runbook §9.4. B5d rol `caja` diferido.
