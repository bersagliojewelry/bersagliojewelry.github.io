...
>
> **2026-06-29 (§156 rediseño primera cara — CERRADO/TODO-61):** hero inmersivo (v56) + B tops compactos + C buscador compacto en `.cat-controls`/dedupe. Verif. navegador REAL; B/C `e093ba8` SIN bump (chunks hasheados). Detalle + causa raíz (doble-padding) → ADR §156.5. Dead code (`renderBuscadorCodigo`/banda `.bc`) → **limpiado en §157**.
> **2026-06-29 (§157 limpieza anti-código-muerto — CERRADO):** DEL `js/core/buscador-codigo.js` (cero importadores; `js/home/codigo.js` ya no existía) + retiro del CSS de la banda en `components.css`; conservadas `.bc-input`/`.bc-msg`/`.bc-recientes`/`.bc-rec-label`/`.bc-chip` (las reúsa `.cat-search`). `codigo-util.js`+test INTACTOS. Build+`test:codigo` 11/11+`brain:check` verdes; sin cache bump (§156 B/C). ADR §157 · L-62.
