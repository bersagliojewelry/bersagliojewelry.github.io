...
> 2. **🛡️ Semana 1 del plan §57** (bóveda `plan-operacion-robustecimiento-2026-06.md`): ✅ HECHO: forms endurecidos (§59) · 2FA · backup diario (§60) · **GEMELO VIVO** (§61: `bersaglio-gemelo.web.app`, sembrado, login aula verificado E2E; credenciales aula → seed-gemelo.mjs). **✅ PRE-1 CERRADO (§63): restauración PROBADA** (702/702 docs al gemelo, vista por Daniel, gemelo limpiado y re-sembrado; 1ª copia fuera → `C:\Users\romad\Documents\BersaglioBackups\`, rutina viernes). PENDIENTE: alertas de presupuesto GCP (Daniel) · recorrido del kill-switch (Daniel, bóveda `runbook-interruptor-emergencia.md`) · firmar contrato (bóveda) · talonarios/arqueo (Kary). **✅ Reconciliación+Salud DESPLEGADA Y MERGEADA (§64, PR #214)**: 10 functions vivas; 1ª corrida 3:30 AM (o "Forzar ejecución"); vista Salud en prod. **✅ RBAC claims CONSTRUIDO (§65, `3e4d4e2`) — ⏳ deploy PENDIENTE de OK de Daniel** (orden CRÍTICO §65.7: `firebase deploy --only functions` → `node functions/backfill-claims.mjs --aplicar` → `firebase deploy --only firestore:rules` → preflight; el backfill toca cuentas de acceso → pedir OK explícito). **Restante F6 técnico: paginación cursor → luego FASE M.** **⏸️ DIAN/factura electrónica PAUSADA por Daniel** (solo si Kary la pide; ventas = "tirilla" comprobante interno rotulado "no es factura electrónica"; supuesto planeación ~$120-150M/año < umbral $183M; detalle → bóveda LEGALES). Decisiones 1-9 respondidas ✅ · **política de cartera v1 APROBADA** (→ config en Fase M) · contrato dueños = entregable pendiente.
...
>
> **2026-06-10 (5)** · **CI de reglas REPARADO §67** (`8b12fc4`): Daniel vio los merges en rojo. RCA grounded → el emulador Firestore de firebase-tools 15.x exige **Java 21**, el CI usaba 17 → `UnsupportedClassVersionError`, exit 1 antes de los tests; rojo desde 2026-06-05 (toolchain sin pin). Fix: Java 21 + pin `firebase-tools@15.18.0`. **Post-mortem**: §62 dio el CI por "verde" sin verificar el run real (L-37). ⏳ verificar que el nuevo run pasa.
