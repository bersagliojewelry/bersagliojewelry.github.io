...
> ✅ **Cerrados**: **TODO-61 (§156 rediseño primera cara: hero + B tops compactos + C buscador compacto en `.cat-controls`/dedupe — DESPLEGADO `e093ba8`, verif. navegador real)** · **TODO-60 (§155 buscador UNIFICADO: pseudo-input global + overlay = el autocomplete)** · **TODO-58 (§154 código + links `/p/`)** · TODO-59 (§153) · TODO-56/55/51/52/53/54/44/45/46 · TODO-40+32 → ADRs/`99`. Pend Daniel: precios + fotos IA. (Pulido OPCIONAL del buscador: highlight `<mark>` · fuzzy.) (Dead code huérfano tras §155+§156: `renderBuscadorCodigo`/`wireBuscadorCodigo` + `.bc`-banda → limpieza anti-codigo-muerto, ver task.)
...
>
> **2026-06-29 (§156 "primera cara" — serie §156.1–.10 DESPLEGADA):** hero inmersivo + más bajo, tops compactos (colecciones/contacto), buscador unificado/compacto, marquee retirado, limpieza de código muerto. Todo en prod, sin cache bump (chunks hasheados). Detalle → ADR §156.x. **ABIERTO**: (a) **hero móvil iOS — 2º fix desplegado (`020d272`, §156.13)**, a confirmar en iPhone de Daniel: esquinas (causa real = `perspective` del stage, removido + radios cuadrados) + crash de pinch-zoom (auroras `bj-world` reducidas en móvil) + eyebrow legible; WebKit-only, no reproducible aquí; (b) **ATAJOS → acceso "Asesoría privada" ✅ DESPLEGADO** (flujo W-11 ×2 + Antigravity): isla eliminada; cristal marfil abajo-derecha, ícono WhatsApp esmeralda línea-fina + "Asesoría privada" (serif), aparece tras 30% scroll → WhatsApp. Rediseñado 3× (verde/invisible/OK); el consejo corrigió al comité (fuera avatar+punto). ADR §156.11-12 + CRUDO. **TODO**: botón "Hablar con el Atelier" en la ficha.
