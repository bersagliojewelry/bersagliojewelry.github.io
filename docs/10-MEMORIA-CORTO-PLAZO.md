...
> - **▶️ SIGUIENTE = F5 completo** (pedido por Daniel): sobre la lista CxC ya hecha, falta **chips/filtros/orden** (por estado vencido/al-día/a-favor · por vendedora · por rango de mora) + **control de crédito** (§10.4: `clientes.lineaCredito` + cupo usado/disponible + alerta de sobre-cupo; la validación en venta llega con F7). **Pendiente confirmar con Daniel**: ¿la tienda usa límite de crédito por cliente? Luego: F4-leads (Bandeja). Antes de F7: Consejo Externo + PRE-infra (TODO-14).
