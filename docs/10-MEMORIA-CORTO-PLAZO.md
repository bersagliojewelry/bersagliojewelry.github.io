...
| TODO-76 | **F-IA-2 + DEPURACIÓN (INTERINATO Opus 4.8)** — spec: `docs/superpowers/specs/2026-07-10-f-ia-2-DISENO.md`. **B0·B1·B2·B3 ✅** (§0.7 D1-D7 cerradas por Fable). EJECUTAR: **B4** Bandeja (mudar `aprobaciones.js` + `aprobarEventoCaja`; badge `aprob-badge`) → B5 Salud "Cierre de mes" in situ + microcopy. CERO functions/dinero nuevo; gates Chrome holísticos. `[[feedback_reorganizar_no_es_depurar]]` | 🟡 | Opus: B4 |
...
> 2026-07-10 · **[OPUS-4.8] F-IA-2 B3 ✅ (SW v91)** — "Hoy" gana el pulso del negocio: 5 señales read-only (Vendido hoy · Efectivo del turno · Cobros de la semana · Por entregar · Avisos) + fila owner (efectivo total · último arqueo), gateadas por rol (sin permission-denied). D3: fuera "Consultas recientes"; Acciones rápidas debajo. `dashboard.js`→`hoy.js` + helper puro `hoy-format.js`. **Dedup money-safe**: `ventasEfectivoTurno`→`caja-format` (POS y Hoy comparten fórmula). Verif: build + test:hoy(8)/caja-format(11)/pedidos(16)/sidebar(12) + traza adversarial (conservación · estado-cero). ⏳ funcional-con-sesión→deploy (L-05). Notas de auditoría → spec §B3. `[[feedback_opus_interino]]`
> 2026-07-10 · **[OPUS-4.8] B0·B1·B2 ✅** (detalle→spec §0.7/checklist): B1 4 pestañas "Negocio y equipo"; B2 directorio `admin-clientes` ⇄ Cartera. EN PROD (SW v90), ⏳ funcional-con-sesión→deploy.
> 2026-07-10 · **[FABLE-5] D1-D7 CERRADAS + previo (→spec §0.7 · ADRs §181-183)**: §181+§182 en prod · 1ª APPROVED real ✅ · TODO-75 ✅ (prompt ALTORRA→bóveda) · rail v2 → `[[feedback_reorganizar_no_es_depurar]]`. Opus ejecuta B4-B5 sin preguntar. ⚠️ **Pend Daniel**: cartera migrada (spec v5 §8) · consejo+abogado apartados (TODO-39). Precios = paso FINAL.
