...
>
> **▶️ FOCO — CMS WEB PÚBLICA ✅ (fase visual CERRADA, §83+§84)**: journal · P0 · Home · Contacto · **Nosotros** (P4: listas + modelo plano 12 claves + imágenes) · WYSIWYG (F1 preview fiel + F2 clic-editar) · P3.5 imagen · F3 barandas ("Publicado ✓" + aviso al salir) · `global` datos de contacto **FUENTE ÚNICA** (footer+Contacto derivan; corrige WhatsApp falso). **TODO EN PROD salvo `global` inc2 (`f52de0e`) — PENDIENTE del último merge de Daniel** (sin deploy de reglas). Specs SSoT: `docs/superpowers/specs/2026-06-1{4,5}-cms-*`; detalle → §83/§84.
> - **SIGUE (plan)**: `global` inc3 (wa.me restantes → derivar de `global.contacto`: FAQ CTA de contacto · carrito · lista-deseos · wishlist) · `hideWhenEmpty` general · usuarios/SPA P5.
>
