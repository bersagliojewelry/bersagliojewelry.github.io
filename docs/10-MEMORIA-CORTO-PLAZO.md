...
>
> **🧭 FOCO = PLAN ÚNICO ERP v4 (TODO-68)** — SSoT spec `2026-07-04-plan-unico-erp-v4`. **F1 ✅ (§165-§168) · F2.0 caja+bóveda ✅ EN PROD (§169-§170) · F2.1 vínculo cliente ✅ EN PROD (§171, flag LIVE)**. **AHORA = "caja en tiempo real" (Daniel 2026-07-07)**: **Fase 1 reactividad ✅** (ventas EN VIVO + panel optimista, SW v80) → **Fase 2 = vista de AUDITORÍA (TODO-69)**, scope→Daniel. **Luego, orden plan v4 §3**: 2.2 factura multi-línea (TODO-41) → 2.4 apartados (TODO-39, EXIGE 2.2). 2.3 térmica BLOQUEADA por hardware (D-5). F2.1b match web dormido. Precios NO bloquean; 1ª APPROVED viva. `[[project_comercio_pagos]]`
...
| TODO-69 | **Vista de AUDITORÍA de caja** (Fase 2 de "caja en tiempo real", Daniel 2026-07-07): bitácora consolidada quién+cuándo por turno (apertura/cierre/movimientos/ventas/traslados/aprobaciones), en vivo, UID→nombre legible. La DATA ya existe a nivel de doc (`turnos.aperturaPor/cierrePor+Ts`, `movsCaja.autor+ts`, `bovedaMovimientos.autor+ts`, `pedido.autor`+`historial`); falta la VISTA + resolver UID→nombre (`users`). Fase 1 (reactividad) ✅. Decisión de scope (owner-only vs Kary · retención · export contador) → Daniel. | 🟢 | scope Daniel |
...
> 2026-07-07 · **Mostrador reactividad money-safe (Fase 1 "caja en tiempo real")** — (1) ventas recientes EN VIVO (`onUltimasVentasChange`, antes fetch de una vez → exigía refresco); (2) panel de caja OPTIMISTA al abrir (siembra `_turno` con el dato de la CF → sin parpadeo ni doble viaje del puntero). Front-end puro (`pos.js`+`pedidos-service.js`), SW **v80**·APP **v42**. Build+tests+brain verdes. **Falta**: validación live Chrome + **Fase 2 = vista de AUDITORÍA (TODO-69)**. Disparó Daniel (ventas pegadas al cerrar · panel lento al abrir · "todo debe ser auditado + quién aperturó/hizo qué"). ADR al cerrar el bloque.
