...
| TODO-41 | **Facturación multi-línea = F2.2 del plan v4** (Daniel 2026-06-26): factura/POS cobra pieza + servicios/modificaciones. **Decisión Daniel 2026-07-07: "AMBOS"** = catálogo de servicios (código+precio fijo, Kary gestiona) + **línea libre** (concepto+precio a mano) para trabajos únicos. **Decisión Fuerte** (toca `crearPedido`=motor de cobro + modelo `servicios` + UI POS multi-línea) → flujo COMPLETO (spec+comité+consejo+CF TDD estricto+gate live). Spec base `modelo-inventario §10`. **Próximo frente.** | 🟢 | arrancar spec F2.2 |
