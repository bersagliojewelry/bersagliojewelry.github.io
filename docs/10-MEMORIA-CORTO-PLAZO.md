...
>
> **▶️ FOCO ACTIVO — CMS WEB PÚBLICA (2026-06-14, urgente · Kary pide la web lista)**: hacer TODO el contenido público administrable desde el panel (piezas, colecciones, banners, journal, videos, redes, textos) + sync admin↔público + SEO/UX/redes. **Diseño completo → `docs/superpowers/specs/2026-06-14-cms-web-publica-design.md`** (auditoría multi-agente `whppptwso`: 109 items + modelo de datos tipado + cola). Autónomo nocturno (commits + tests propios). **HECHO**: bug REAL del sync corregido — vínculo pieza↔colección tolerante slug/id (`js/core/collection-match.js`, `47b07b8`, test 5/5); el admin guardaba `id`, el público filtra por `slug` → catálogo/relacionados vacíos en silencio. Prod = **0 piezas** (catálogo real sin cargar = tarea aparte). **COLA**: comité del modelo de datos → skill `cms-dinamico`+agente content-section-builder → P1 (categorías del Home desde `collections`, `siteContent/home`, `journal`) → P2 (nosotros/reviews, films/social, footer+sitemap) → SEO. ⚠️ El `noindex` del catálogo/journal es **soft-launch DELIBERADO** (ADR SEO-02 `3f654e1`); el flip a `index` = **Decisión Fuerte gated** a contenido real + Consejo (`docs/15`). Patrón: docs tipados público-read/`editor`-write + live `onSnapshot` (extiende el de `pieces`).
>
