...
>
> **▶️ FOCO ACTIVO — CMS WEB PÚBLICA (Kary, urgente)**: todo el contenido público administrable desde el panel + SEO/UX. **Specs (leer al reanudar)**: `2026-06-14-cms-web-publica-design.md` (P0-P1.3, estado por ítem en su Checklist) + **`2026-06-14-cms-gran-plan-dinamismo-design.md`** (gran plan: modelo 2-formas, **builder recortado 10× por FORMULARIO** no lienzo, no-demo, SPA/SSR, roadmap P0→P5; **consejo externo Gemini INTEGRADO**: SSG build-bake+auto-rebuild = sustrato SEO/0-lecturas-runtime · CSP estricta vía `<meta>` (tarea: auditar inline-scripts) · storage rule claim ROL · flujo invitación P5; REFUTADO "DOMPurify siempre"). Deliberaciones → bóveda (`2026-06-14-comite/consejo-*`). **HECHO** (rama Desarrollo, SIN desplegar): motor CRUD (servicio+UI) + panel Contenido web + **journal E2E** (`8b507cb`/`67fc21e`/`66dfd04`) · **P0** bloqueantes B1/B2/B4/B5 (`c257dca`) · **B3-categorías** no-demo (`9f8c86f`) · **P1** siteContent servicio+reglas (`c98f77d`) + **UI textos Home editables** (`905330c`: scaffold singleton `singleton-admin`+núcleo puro, pestaña "Textos del Home" hero/editorial, render público merge(DEFAULTS,doc) con refresh). Verif: build verde · 202/202 no-emulador · **160/160 reglas**. Prod 0 piezas/0 journal → baked (sin cambio visible). **Home 100% editable** (hero/editorial/atelier/cta, `95c5b1f`) + **Contacto copy editable** (hero/proceso/faq, `ba46db7` — NO toca el formulario). **Reglas siteContent ya POR-PÁGINA** (`siteContentKeys(page)`: home/contacto/nosotros/global definidos; rechaza cross-page + páginas desconocidas) → nosotros/global solo necesitan descriptor+render. **▶️ REANUDAR EN P2**: **nosotros** (copy ~11 secciones, mismo patrón singleton) + **global** (datos de contacto compartidos: canales/horarios/dirección footer+contacto) + **UNDO** ("restaurar versión" del audit log, snapshot ya en `firestore-service.js`) + `hideWhenEmpty` en el modelo. (**P0.5 SEED** = coordinado con Daniel, escribe a prod vía Admin SDK · **B3 films/social + P3.5 imágenes + P4 sections[] + P5 usuarios/SPA** → spec.) **Lecciones→`30`**: `.innerHTML`→`mount()` · falso positivo hook con exec de RegExp→`String.match()` · núcleo PURO `-core.js` · `singleton=getDoc cacheado / lista=colección lazy` (Spark). **DEPLOY journal+P0 pendiente**: merge Daniel (L-26) + `firebase deploy --only firestore:rules,storage` (B1+B5) + cache bump v16→v17.
>
