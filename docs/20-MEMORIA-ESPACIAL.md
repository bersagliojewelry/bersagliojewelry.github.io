...
*   **`admin-cuenta.html` + `js/admin/cuenta.js`** (✅, singular = ficha): saldo en vivo (`onClienteChange`) + historial + registrar factura/abono + anular + **corregir saldo** (ajuste delta → saldo correcto) + **editar datos del cliente** (admin). El saldo lo pone la CF; "corregir" registra un `ajuste` auditable.
*   **Editabilidad (directiva del cliente)**: todo lo migrado debe poder corregirse → editar cliente + corregir saldo (ficha) + **días de plazo de atraso** editable en Configuración (`config/negocio.diasPlazo`).
