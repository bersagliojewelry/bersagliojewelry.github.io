...
`brain-check.mjs` lee `defined` SOLO de `30` y `referenced` de todo el cerebro MENOS las hijas. Mover una lección referenciada en `99` a una hija (`31`) la deja COLGANTE. → el shard TODO-27/§96 deja el **header-stub `### L-NN` en `30`** (defined lo cuenta, refs resuelven) y el CUERPO en `31`. Soporte real multi-archivo (que `defined` lea `3*-LECCIONES*.md`) = cambio de KERNEL → cars-operador (L-31), NO unilateral desde bersaglio; aporte para la pasada Gemini (con TODO-22/23). **Generaliza (§140)**: igual acoplamiento en el **check #3** (lee solo `00-INDICE.md`) → el range-shard §≤115→`00a` deja esas filas sin desync-validar (bajo riesgo: offsets congelados) → **TODO-55** (cars-operador). [HONOR]
