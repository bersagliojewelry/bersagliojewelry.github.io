...
Síntoma: tras añadir clases por JS, `getComputedStyle` da el snapshot inicial (un `style.opacity` inline lee "0"); IntersectionObserver y **`requestAnimationFrame` NO disparan si la pestaña está `hidden`** (→ el código en rAF, p.ej. wiring, no auto-corre; `renderAll()` síncrono SÍ pinta); `preview_screenshot` hace timeout con mucho `backdrop-filter`. Causa: UN pase inicial, sin recalc/paint/rAF en vivo. **Receta**: verifica lo dinámico (reveals, hover, scroll, IO, wiring) por CÓDIGO + DOM (`preview_eval`), o invoca el handler a mano (`import()`+call); NO por screenshot ni post-mutación. Lo visual real, en `npm run dev`/deploy.
