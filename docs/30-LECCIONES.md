...
### L-62: Retirar un componente compartido → borrado PARCIAL del CSS (sus clases base pueden vivir en un hermano)
Disparador: estrenas algo que REEMPLAZA un componente compartido (skill `anti-codigo-muerto`) y limpias su CSS/JS. **Trampa**: la "banda" muere (cero importadores) pero sus clases base siguen VIVAS porque un hermano las REUSA — §157: `js/core/buscador-codigo.js`+`.bc` mueren, pero `.bc-input`/`.bc-msg`/`.bc-recientes`/`.bc-chip` las usa el buscador compacto `.cat-search`. **Receta**: grep el reúso de CADA clase (`catalogo.js`+`catalogo.css`) y retira solo las huérfanas (contenedor/lead/row/ico/btn + media query). El módulo ESM sin importadores se borra directo (Vite lo tree-shakea); `_legacy/` solo aplica sin grafo (script global/backend). → ADR §157.
...
### L-60: Importar datos reales de fuente externa (certificados QR→SPA) — pipeline reusable. Detalle completo → **ADR §132** (jsqr+sharp decodifica el QR de la foto · scrape del SPA con Chrome MCP navegando como usuario `pushState`/`popstate`, SIN replicar credenciales · identificador EXACTO del SPA, no OCR frágil · clasif. visual por montage 1-lectura-para-N · carga vía MCP Firestore · no inventar lo no certificado). Gotcha de reúso (NO en §132): `jsqr`/`tesseract` se instalan `--no-save` → se PODAN en el siguiente `npm i`; reinstalar antes de reusar. Artefactos efímeros → scratchpad.
