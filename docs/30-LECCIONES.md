...
**Disparador**: añadir un LQIP/placeholder "premium" a imágenes del CMS en un sitio estático (GitHub Pages, datos por `getDoc`). **Lección (revert §104, feedback Daniel "ahora es peor")**: el LQIP guardado en Firestore llega CON el `getDoc`, NO antes del 1er paint → la secuencia es neutro(defaults) → borroso → nítida = **3 estados**, PEOR que el neutro→nítida previo. Un placeholder solo reduce churn si está disponible ANTES que la imagen (la **sustituye**, no la **suma**): eso exige caché-memoria con prefetch (router/F2) u horneado SSG (F3). Sin eso, el LQIP empeora. **Reglas**: (1) el **feedback de usuario manda sobre el beneficio teórico**; (2) al optimizar la carga, cuenta los ESTADOS visibles (¿reemplazo o suma?) antes de darla por buena; (3) patrón de revert sano: quitar SOLO el render visible y **conservar la fontanería de datos** (generación + almacenamiento + tests) lista para cuando exista la pieza que la habilita (F2). Relacionado [[L-47]] (LQIP), [[L-46]] (placeholder neutro), §103.3.
