...
### L-44: Página CMS client-rendered → gatear las IMÁGENES hasta el doc real de Firestore (ADR §99)
**Disparador**: una página editable (siteContent) que pinta con defaults/memoria y re-pinta al resolver `loadSiteContent` (getDoc one-shot). **Lección (bug Daniel 2026-06-22)**: el texto default es seguro, pero una IMAGEN provisional (default/stale-en-memoria/bfcache) que luego "salta" a la real se ve como **"imagen VIEJA → nueva"**. Fix: gate `_siteReady` — hasta confirmar el doc, pintar el **fondo reservado** (sin `<img>`), no una imagen provisional; al llegar el valor real, recién pintar la imagen (una vez). NO es caché de imagen (las URLs de Storage son únicas, `Date.now()`) ni SW (cross-origin pass-through): es el DOBLE-PAINT. §93/PERF-05 lo hizo para catálogo/pieza/journal/lista-deseos pero dejó **Nosotros/Contacto fuera** (válido para texto, no imágenes). Aplica a CUALQUIER página con imagen CMS (Contacto/Home pendientes).
