...
### L-81: Un sistema de sesión/turno de caja con el gate en OFF (`enforceTurno:false`) NO es "suave", es un HUECO: permite ventas HUÉRFANAS (`turnoId:null`) que no entran al arqueo del turno ni a la auditoría por-turno → plata cobrada FUERA del control de caja. Un POS profesional exige turno abierto para vender (toda venta amarrada a turno + cajero; caja cerrada = solo "abrir", ni vender ni cerrar). Doctrina: al construir un feature de caja/arqueo/auditoría, el gate DEBE quedar ON — con él OFF toda la contabilidad del turno miente y la auditoría tiene un punto ciego. Corolario: una vista por-turno debe VERIFICAR que no existan documentos fuera de turno (o marcarlos como anomalía visible), no asumir que todos pertenecen a uno. → TODO-70 (POS profesional, prereq F2.2) · `pos.js` `ventaBloqueadaPorCaja` · `config/caja`
