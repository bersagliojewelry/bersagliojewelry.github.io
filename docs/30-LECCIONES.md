...
### L-82: HUECO EN BLANCO en carga fría → SKELETON (reusa el componente real, no reserva-en-blanco); NO acelerar con live-upgrade sobre PRECIOS (bait-and-switch). → ADR §178 ⇒ **migrada al maestro**: [[BERS:L-82]] · cuerpo en `/_legacy/LECCIONES-MIGRADAS-MAESTRO.md` (raíz del repo)
### L-86: Cuando un flujo gana un LIBRO nuevo, el camino de DESHACER lo hereda en el MISMO commit — y un vigilante que compara cada libro consigo mismo jamás ve una fuga ENTRE libros. → `35-LECCIONES-DINERO` ⇒ **migrada al maestro**: [[BERS:L-86]]
### L-85: Idempotencia con destino TEMPORAL (el "turno abierto") NO se copia de una con destino determinista: hay que ANCLAR el destino en el doc de la 1ª escritura. → `35-LECCIONES-DINERO` ⇒ **migrada al maestro**: [[BERS:L-85]]
...
### L-26: `origin/main` puede avanzar sin ti — `git fetch` SIEMPRE antes de afirmar nada de `main`
**VIGENTE — C0 orden nº 2 (4-sep-2026): el merge `Desarrollo→main` lo hace CLAUDE** (commit + push + merge + deploy en los CUATRO repos, INSEMA incluido; ⛔ nunca abrir PR sin permiso → router §Reglas git). Lo que esta lección conserva NO es el reparto sino el REFLEJO: **`git fetch` antes de afirmar nada** de `main`/deploy — la ref local es STALE (§3.3). El **sitio** se despliega con ese merge (CI on-push-a-main), pero **reglas/functions NO** (L-22) → deploy manual. **HISTÓRICO (hasta el 4-sep-2026)**: dos veces (lanzamiento PR #189; Fase R PR #191) `origin/main` avanzó solo mientras yo trabajaba, porque Daniel mergeaba el PR en GitHub — a veces ANTES de que yo terminara de verificar (§75: el PR #223 publicó M2a con 2 bugs de dinero). Casos: ADR §47, §49. **Corolario que NO caduca — UI de dinero: verifica ANTES de pushear a `Desarrollo`; tras el merge, comprueba el bundle EN PROD y rushea el fix.**
...
> Cómo opera/falla el cerebro (Autocrítica §G.4). Sharded a `34` (§161); stubs aquí (kernel lee `### M-NN` de `30`, M-06). **M-01..M-06, M-08, M-09 y M-23 migradas al maestro** (lote 13; allá queda su stub) — en `34` solo M-07 y L-31 conservan cuerpo.

### M-01: No imprimir un campo de estado del manifest como "hecho" sin gate que verifique su artefacto → 34 ⇒ **migrada al maestro**: [[BERS:M-01]]
### M-02: Una lección sobre estado verificable-por-comando (git/build) debe volverse GATE, no prosa [HONOR] → 34 ⇒ **migrada al maestro**: [[BERS:M-02]]
### M-03: Un campo `last` de tracking nace null/baseline, nunca con fecha que finja una ejecución → 34 ⇒ **migrada al maestro**: [[BERS:M-03]]
### M-04: La memoria del harness deriva en silencio (fuera de `docs/`, el linter no la cubre) → 34 ⇒ **migrada al maestro**: [[BERS:M-04]]
### M-05: Edité un subsistema bajo UNA lente y lo di por bueno sin probar el camino vivo (§89 · caza-bugs/W-10) → 34 ⇒ **migrada al maestro**: [[BERS:M-05]]
### M-06: El kernel acopla `### L-NN` a `30` → shard = stub-en-30 + detalle-en-hija (§96) [HONOR] → 34 ⇒ **migrada al maestro**: [[BERS:M-06]]
### M-08: El `05` no FIJA a mano hechos verificables-por-comando (hash PROD) — stale ×3, git=SSoT (§114) [HONOR] → 34 ⇒ **migrada al maestro**: [[BERS:M-08]]
### M-07: Los node:tests NO corren en CI (solo `test:rules`) → test-rot silencioso tras refactor de render (§104) [HONOR] → 34
### M-09: Muestrear ≠ contar — extrapolé de la 1ª pág de una lista paginada (GBP: "mayoría de 85 sin responder"; real 74/85 sí) [HONOR] → 34 ⇒ **migrada al maestro**: [[BERS:M-09]]
### M-23: Sello del 05 caducó ×2 (A2-§175 reincidente §192) — re-sellar ≠ fix; gate kernel sello-vs-git → TODO-71 → 34 ⇒ **migrada al maestro**: [[BERS:M-23]]
