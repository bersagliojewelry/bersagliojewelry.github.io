...
En un `update`, `request.resource.data` es el estado **resultante completo** del doc (merge ya aplicado), NO solo el delta. Por eso: exige obligatorios SOLO en `create` (`d.name is string && d.name.size()>0`); en `update` valida **tipos solo-si-presente** (`d.x == null || d.x is T`) — así un patch parcial (p.ej. solo `images`) y los docs legacy NO se rompen (los campos heredados ya tienen tipo válido). Acceder a una clave ausente con `data.foo` devuelve `null` (no lanza), a diferencia de `get(doc).data` sobre doc inexistente. Caso real: S6 en `firestore.rules` + `tests/firestore-rules.test.mjs`.
