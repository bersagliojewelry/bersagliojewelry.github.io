...
- **2026-07-18 · Cuentas de socias como canal de ventas (F-TESORERÍA, consejo externo + comité)**: recibir pagos de clientas en cuentas/Nequi personales de las socias expone a: (a) **exógena/art. 594-3 ET** — consignaciones >~1.400 UVT/año obligan a la socia a declarar renta y la presunción DIAN corre contra ella; (b) **SARLAFT bancario** — joyería = sector alto riesgo; depósitos comerciales recurrentes en cuenta personal → riesgo de bloqueo (plata del negocio atrapada); (c) precisión: no existe "cuenta de la empresa" (persona natural) — la meta es concentrar en cuentas de la **titular del RUT**. **Mitigación en software** (spec F-TESORERÍA V9): acumulado anual visible por cuenta de socia + advertencia en UI. **⚠️ ACCIÓN DEL DUEÑO (pendiente, no-software)**: formalizar con abogado/contador un **contrato de cuentas en participación o mandato** con las socias mientras se migran las cuentas [a verificar figura idónea con abogado CO]. Disclaimer §0 aplica.
