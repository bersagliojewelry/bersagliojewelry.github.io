...
| **LEGAL-09** | **IVA en Términos §02** — el sitio dice "precios incluyen IVA 19%" pero la comerciante es **No responsable de IVA** → corregir a "precio final en COP, sin discriminar IVA". **Consejo ×2 (ChatGPT+Antigravity) lo marcó FATAL** (info engañosa + anunciar impuesto no recaudado). Confirmar con Daniel. Toca `js/pages/terminos.js §02`. | 🔲 Confirmar Daniel |
| **LEGAL-10** | **Persistir el consentimiento Habeas Data** (Dto.1377 art.5) — VERIFICADO (Antigravity): `carrito.js:440` valida `_habeas` pero NO se envía a `iniciarPagoWeb` ni se guarda en el pedido → sin prueba jurídica. Enviar `_habeas`+timestamp+versión por `carrito.js→pago-web.js→pedidos-service→iniciarPagoWeb` y guardarlo en `functions/pedidos-core.js`. Deploy functions (L-22). Prereq encender Wompi. | 🔲 Al encender Wompi |
