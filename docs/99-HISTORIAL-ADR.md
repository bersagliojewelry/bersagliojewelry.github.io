...
- **145.6 Verificación**: build verde + sidebar test. Gestos táctiles → dev usa EMULADORES (no levantados, L-58) → verif. en el sitio REAL tras deploy: (a) Playwright con pointer events SINTÉTICOS (el modelo es pointer-events → testeable: 2 punteros→scale↑ y NO cierra; 1 puntero con zoom→translate↑; tap→cierra) + (b) Daniel en su dispositivo (la superficie táctil reportada). [HONOR — se reporta lo recorrido, no "verificado" sobre estático].
