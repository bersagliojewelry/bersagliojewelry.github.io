...
**64.7 Doctrina + estado**: upgrade path anotado (reconciliación INCREMENTAL por `saldoActualizadoEn` cerca de ~25k docs; purga de `saludEventos` resueltos antiguos). **Deploy de reglas+functions PENDIENTE de OK de Daniel** (bloqueado por el clasificador de permisos; tests verdes, listo para `firebase deploy --only firestore:rules,functions`). Lección L-34 → `30`. Restante F6: RBAC claims (frente B) · paginación cursor.
