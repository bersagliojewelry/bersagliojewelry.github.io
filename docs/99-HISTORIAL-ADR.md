...
**88.7 Doctrina + estado**: cero-ficción por **defensa en profundidad** (la regla impide ESCRIBIR ficción; el render re-valida para no pintar la legacy) · **SSoT** que sincroniza panel↔home↔reglas · `.get(k,'')` (idiom S6) y `.trim()` en reglas · comité decide / Claude ejecuta+verifica (red-team de la única frontera server-side). **cache bump v18** (§4). EN `Desarrollo`, **PENDIENTE deploy** (merge Daniel→main + `firebase deploy --only firestore:rules`, L-22) **+ censo legacy**: antes/después de desplegar, revisar journal/films/socialPosts con `published==true` SIN sus campos reales → despublicar o completar (la puerta bloquea re-guardarlos; el render ya los oculta). Pendiente menor de diseño: ¿`href` obligatorio al publicar en Redes? (hoy opcional por decisión del comité; flag a Daniel).
