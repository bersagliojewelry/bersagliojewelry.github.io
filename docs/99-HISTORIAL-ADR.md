...
- **120.6 Archivos**: `js/core/global-defaults.js` (`waLink`), `js/pages/pieza.js` (CTAs+helpers `asesorWaText`/`asesorWaHref`), `js/analytics.js` (`whatsapp_click`+FB Lead), `public/sw.js` (v34), `tests/wa-link.test.mjs` (nuevo). Commits `[OPUS-4.8]` en `Desarrollo` → **pend. merge Daniel**.
