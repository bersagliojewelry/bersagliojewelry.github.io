...
## 2026-06-27 — §140 Cerebro: range-shard del índice sináptico → `00a-INDICE-HIST` (§1–§115); 00-INDICE descongestionado [OPUS-4.8]

> Disparador: `00-INDICE.md` crónicamente sobre su tope blando (~28.000c; **30.787c** al abrir esta sesión), obligando a podar filas viejas a mano en CADA ADR nuevo — fricción recurrente (el cap se cruzó ~5× en la tanda §134-§139). Ejecuta el **Reflejo de Sharding** (§G.5) que el propio manifest ya tenía planificado (`_caps_trinquete`: *"range-shard 00a/00b = futuro a ~28k"*) — completa el range-shard que **TODO-32 (ya ✅)** había dejado diferido como *futuro*.

- **140.1 Causa raíz**: el índice mezcla DOS poblaciones de dinámica opuesta — (a) filas históricas FROZEN (§1–§115: su nº de línea en `99` no cambia, los ADR son append-only) y (b) filas recientes que crecen sin parar (cada ADR nuevo ≈ +1 fila verbosa ~200c). Mantener ambas en un archivo de tope fijo garantiza el desbordamiento periódico. El parche previo (subir el tope 16k→24k→28k, §92/TODO-32) era explícitamente provisional.
- **140.2 Solución estructural**: range-shard por antigüedad (NO compresión con pérdida — el índice es navegación exacta). §1–§115 se extraen a una neurona hermana nueva `docs/00a-INDICE-HIST.md` (hija de `00`). `00-INDICE.md` conserva el enrutamiento semántico (síntoma→neurona) + las filas RECIENTES §116–§140 + un **PUNTERO** a `00a` para §≤115. Corte en §116 = frontera de era limpia (2026-06-25+: visibilidad/ficha/comercio B1/inventario/catálogo = lo "vivo" en `05`/`10`). Post-shard: `00`≈9k, `00a`≈20k (ambos holgados bajo 28k).
- **140.3 No-regresión**: filas movidas **verbatim** (offsets intactos; verificado `grep "^## " 99` → §116=1715…§139=1979, 100% match). Cero IDs/funciones/callsites tocados (es doc del cerebro, no web). `00a` registrada en CLAUDE.md §0 + mapa de neuronas de `00` + cap en manifest → **cero huérfanas** (check #1/#10). **Kernel NO tocado** (`brain-check.mjs`/`brain-diff.mjs`, byte-identidad ×3, L-31): `brain-diff` no lee el índice; `brain-check` #3/#9 ahora validan solo las filas que QUEDAN en `00` (las recientes = las de riesgo de drift real).
- **140.4 Tests / verificación**: `npm run brain:check` **SANO**; `00`/`00a` ambos dentro de tope (check #2); ruteo §→línea revalidado con grep (pista, no verdad — 100% resuelto); filas §116–§140 de `00` apuntan a headers `##` válidos en `99` (check #3 verde).
- **140.5 Anti-patterns evitados**: NO edité el kernel "para que funcione ya" (habría roto byte-identidad ×3 sin coordinar con cars-operador, L-31) → en su lugar TODO accionable. NO comprimí/borré filas históricas (límite de guardián: mover/apendar, no destruir). NO subí el tope (trinquete §G.5: bajar libre, subir exige ADR). **Limitación consciente**: las filas §≤115 en `00a` pierden la validación automática del check #3 (que hardcodea `00-INDICE.md`) — riesgo BAJO (sus líneas están congeladas) → mitigación determinista = **TODO-55** (extender el check a globear `00*-INDICE*.md`, trabajo de cars-operador).
- **140.6 Archivos**: NUEVO `docs/00a-INDICE-HIST.md`. MODIFICADOS `docs/00-INDICE.md` (§1–§115 fuera + puntero + mapa de neuronas), `docs/.brain-manifest.json` (cap de `00a` + nota trinquete a pasado), `CLAUDE.md §0` (fila `00a` + regla anti-saturación menciona ambos índices), `docs/10-MEMORIA-CORTO-PLAZO.md` (bitácora + TODO-55 nuevo), `docs/05-ESTADO-GLOBAL.md` (fila Cerebro). INTACTOS `scripts/brain-check.mjs`, `scripts/brain-diff.mjs` (kernel), `99` (solo append de este §140), toda la web.
- **140.7 Doctrina + cache**: §G.5 Reflejo de Sharding (neurogénesis por saturación) — la madre deja puntero a la hija, la hija se registra en los 3 sitios (§0 / mapa de neuronas / manifest). Sin cache bump (cambio de docs, no de shell). Completa el range-shard que **TODO-32** (ya ✅) dejó diferido; abre **TODO-55** (kernel). [OPUS-4.8] [HONOR]
