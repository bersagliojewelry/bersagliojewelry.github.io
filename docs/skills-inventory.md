...
| `asesor-critico-honesto` **v2 (2026-07-10)** | Crítica con MÉTODO, no solo tono: verificar antes de opinar (§3.3), steelman+auto-refutación, calibración [COMPROBADO/PROBABLE/OPINIÓN], desacuerdo obligatorio, anti-complacencia. v1 (solo "sé directo") producía críticas firmes pero vacías. | ✅ repo+user |
...
| `opus-interino-protocolo` (2026-07-10) | Protocolo del MODELO INTERINO (Opus 4.8 cuando Fable no tiene cuota): puntos ciegos conocidos (§158/§161/§181) → reglas vinculantes (marcar commits, TDD dinero, zonas calientes, verificación viva, escalada honesta a 2 fallos, Decisiones Fuertes en cola, bitácora) + checklists de arranque/cierre para auditoría rápida del titular. Pedida por Daniel. PORTABLE. | ✅ repo+user |
| `pos-facturacion-retail` (2026-07-10) | Cómo funcionan los POS/facturación REALES: VOID (solo turno abierto) vs DEVOLUCIÓN (doc nuevo aprobado) vs corrección contable; sesión de caja y arqueo a ciegas; split-tender; DIAN CO (POS electrónico 5 UVT, FE, nota crédito, bancarización 771-5); anti-patrones amateur. Origen: pregunta de Daniel sobre "Anular" (§182). PORTABLE. | ✅ repo+user |
| `auditoria-financiera` (2026-07-10) | **Método forense anti-fugas de dinero**: 7 invariantes (conservación · mismo-número-en-todas-las-vistas · idempotencia real · deshacer-netea-todo · estados muertos · SoD/ledger inmutable · anomalías que gritan) + 4 fases (mapa → auditores paralelos ACOTADOS → verificación adversarial propia → fix+blindaje+prueba viva) + vectores de robo enmascarable + checklist de pasarela. Origen: auditoría real 2026-07-10 (4 P0 cazados: doble reintegro stock, reversas Wompi tragadas, doble cobro, arqueo ciego a splits). PORTABLE. | ✅ repo+user |
| `caza-bugs` (2026-06-21) | Reflejo al TOCAR/ROZAR un subsistema: recorrer su CAMINO VIVO end-to-end desde el estado-cero (vacío→1 y N→vacío + recarga), no solo el diff; + escalada calibrada (N0 barato / N1 pesado, cita dueños) y blindaje con test estado-cero. Frontera por MOMENTO: al tocar (≠ `systematic-debugging` = bug ya visible, ≠ `verification-before-completion` = claim final). **+§2b checklist del DINERO (2026-07-10)**: ida-y-vuelta con recarga, foto incompleta, conservación 3-vistas, deshacer-netea, negativos visibles, doble sesión. PORTABLE. Origen TODO-25/ADR §90 · receta `60-WORKFLOWS` W-10. | ✅ user-level |
| `proceso-decision-fuerte` (2026-06-23) | Pipeline de validación multi-capa para DECISIONES FUERTES (arquitectura/datos/seguridad/legal/irreversible/multi-subsistema): GATE→verificar→arquitecto→comité acotado→consejo externo (Gemini)→verificar cada claim→veredicto→gate empírico por fase. ACOTADO (inline+schema, sin tools — L-50). NO para lo trivial. Origen §101. Usada en §108. | ✅ user-level |
| `anti-codigo-muerto` (2026-06-23) | Al estrenar/fix código que REEMPLAZA algo viejo: garantizar que lo nuevo RETIRE lo viejo (no apile); detectar muerto/huérfano (paths sin ref, flags reutilizados, deploy incompleto ×N), cuarentenar (`_legacy/`) > borrar; verificar el camino vivo con el motor ACTUAL. Síndrome Knight Capital. Gate `deadcode:check`. PORTABLE. | ✅ user-level |
| `validacion-live-chrome` (2026-06-23) | POST-merge/deploy (cambios YA en vivo): genera prompt/checklist autocontenido para la extensión "Claude in Chrome" del dueño → él recorre el camino vivo EN PROD (logueado/no), pega la observabilidad (consola/network/DOM/estado), yo diagnostico→fix→re-valido. Cierra el hueco de verificación live (L-08); Claude NUNCA maneja credenciales. Instrumento de `caza-bugs` en prod. | ✅ user-level |
...
> **Dueño único: `brain-private/skills/`** (D-C4-1). Estas 9 skills no vivían en este repo:
