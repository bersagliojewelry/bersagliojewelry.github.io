...
- [ ] **GATE deploy**: 2ª opinión EXTERNA (Gemini/Antigravity, solo crítica — `docs/15 §6`) sobre el modelo + validación manual de Daniel del editor en vivo (panel requiere login) + `firebase deploy --only firestore:rules` (Fase 4) + deploy sitio. NO mergear `Desarrollo→main` antes de este gate.
