...
**⚖️ DECISIONES ABIERTAS (Daniel)**: **(1)** cardinalidad — ¿Kary añade/quita items? (recomendado SÍ → motor completo, evita migración) · **(2)** lista vacía → ¿la sección desaparece de la web (**A**, recomendado: "lo que ve es lo que se publica") o reaparecen los textos de fábrica (B)? · **(3)** reseñas reales → ¿pegar a mano ahora (recomendado) o integración Google después? · **(4)** fotos de equipo → ¿subir reales? (después, no bloquea P4) · **(5)** el modelo de datos es **cara de revertir** → 2ª opinión externa (`docs/15`) antes de congelar la forma del doc.
