...
- [ ] **GATE deploy**: (1) 2ª opinión EXTERNA del modelo (Gemini/Antigravity, solo crítica — `docs/15 §6`) — **prompt anti-anclaje LISTO** en bóveda `2026-06-19-consejo-externo-p4-modelo-prompt.md` (falta que Daniel lo corra y traiga la respuesta) · (2) validación manual de Daniel del editor en vivo (panel requiere login) · (3) `firebase deploy --only firestore:rules` (Fase 4) + deploy sitio. NO mergear `Desarrollo→main` antes de este gate.
