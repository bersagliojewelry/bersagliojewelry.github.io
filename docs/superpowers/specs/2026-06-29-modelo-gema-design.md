...
## 7. Estado
DECISIÓN CERRADA (4 capas: arquitecto + comité ×5 + **consejo externo verificado §8** + gate Daniel). Modelo PLANO (`specs.badgeGem`+`specs.gemFilterIds`). Fundación de código HECHA (`gem-taxonomy.js` + `gem-badge.js` plano, test 7/7). **Form admin HECHO ✅** (§151, en main vía PR #395: select "Gema principal" + checkboxes filtros + rename stone→"Descripción", deriva `gemFilterIds`, respeta `_editingVersion`). **Backfill de las 32 APLICADO + VERIFICADO ✅** (2026-06-29, gate Daniel: dry-run read-only → 32 writes MCP por field-path → re-lectura 32/32 con `badgeGem`+`gemFilterIds`, 0 no-match; `specs.stone`/carat/cert/`_version` intactos; respaldo en scratchpad). **JSON-LD canónico HECHO ✅** (2026-06-29): helper `gemDisplayName(specs)` en `gem-badge.js` (badgeGem→label "Esmeralda", fallback a la prosa) consumido por el SSG (`generate-pieces.mjs` `buildAdditionalProperty`) y la ficha cliente (`js/core/schema.js`); test `tests/gem-badge.test.mjs` 9/9; build VERDE + `npm run generate` verifica que la pieza esmeralda emite `additionalProperty Gema:"Esmeralda"` (no "Esmeralda Natural"). **Sin cache bump**: `schema.js` es chunk hasheado (cache-first por hash), NO `SHELL_ASSET`, y el HTML es network-first → el cambio se sirve fresco sin invalidar caché. Pend IMPLEMENTACIÓN (TODO-57): `settings/gems` + hornear en catalogo.json + filtros TODO-50 + índice `array-contains` + (luego) retirar regex fallback + prueba live del form (login). → ADR §150/§151.
