...
- **2.3 Recibo/factura interna imprimible** (print CSS + numeración `contadores` existente) para mostrador;
  el comprobante web **reusa el artefacto por-token de §164** (no crear un segundo recibo paralelo).
...
- [ ] F2 completa (vínculo cliente + multi-línea + recibo + apartados si D-1=sí + kardex)
