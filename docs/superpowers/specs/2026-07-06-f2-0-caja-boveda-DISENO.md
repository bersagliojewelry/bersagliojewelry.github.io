...
- [~] B5 UI + gate prod (Chrome) + SW bump + ADR — **B5a wrappers onCall ✅** (`pedidos.js`+`index.js`: `abrirTurno`/`cerrarTurno`/`movimientoCaja`/`registrarTraslado`/`reversoTraslado`/`ajusteBoveda`/`aprobarEventoCaja` + trigger `recalcBoveda`; roles `rolDeCaja`/`rolDeOwner`; alertas→`saludEventos` vía `notificarCaja`; `emitirAlerta` durable; smoke-test de carga ✓). **L-72 ya resuelto en `d244d77`** (verificado `pos.js:430` usa `estadoPedido().label` — el "residual" era stale). **Pend: B5b UI** (POS caja + vista Bóveda owner + panel aprobaciones) → **B5c** SW bump + deploy manual + gate Chrome + ADR.
