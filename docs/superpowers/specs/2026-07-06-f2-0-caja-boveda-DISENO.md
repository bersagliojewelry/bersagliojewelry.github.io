...
- [x] B0 reglas rol caja + config/caja + append-only — evidencia: test:rules 233/233 (B0a root+enum + B0b colecciones; commit `[OPUS-4.8]`). ⚑ Desvío: subcolección `movsCaja` (no `movimientos`) para no contaminar el CG de cartera → L-73. · ADR §169
...
- [x] B3 bóveda traslado/reverso/recompute/checkpoint — evidencia: `caja-core.js` (`registrarTrasladoCore`/`reversoCore`/`recalcBovedaCore`+`saldoBovedaDesdeLedger`): recompute-no-incrementa (delta firmado) · reverso INMUTABLE §9.3 (no borra, delta=−original, anti-doble-reverso) · checkpoint `sellTs` acota el barrido · guards monto(int>0)+saldo-insuficiente · idempotencia opId · recompute síncrono in-tx (§8.1.3). caja.integration **23/23** (flake idempotencia concurrente endurecido→**L-76**). Commit `[OPUS-4.8]`.
- [x] B4 Dual-Approval + alertas — evidencia: `caja-core.js` (`reversoCore` pendiente · `ajusteCore` · `aprobarEventoCajaCore` owner-only · `movimientoCajaCore`): eventos destructivos (reverso/ajuste) nacen `pendiente_aprobacion` → el recompute los EXCLUYE hasta que el owner firma (SoD §9.1); caja NO aprueba (rol≠owner rechaza); egreso→alerta (§8.5); alerta = callback INYECTABLE (owner hardcoded §9.8; transporte FCM real con A.6). caja.integration **31/31** estable (×2). Commit `[OPUS-4.8]`.
- [x] **B5 COMPLETO ✅ (ADR §169, 2026-07-07)** — UI + deploy manual (rules+indexes+8 CFs + `config/caja` sembrado) + **gate prod verde (Chrome owner)**: abrir turno→ingreso→traslado→cerrar (esperado $249k=contado "Cuadra ✓")→bóveda→reverso pendiente (saldo intacto)→aprobar (saldo $0). SW v76→v77. Revisión adversarial acotada cazó 2 fixes de dinero desplegados: reponer cambio lleva turnoId (cierre correcto) + opId perezoso (no duplica al reabrir). Runbook §9.4 (activar enforceTurno) + B5d (rol caja end-to-end) DIFERIDOS. Detalle abajo:
- [~] B5 UI + gate prod (Chrome) + SW bump + ADR — **B5a wrappers onCall ✅** (`pedidos.js`+`index.js`: `abrirTurno`/`cerrarTurno`/`movimientoCaja`/`registrarTraslado`/`reversoTraslado`/`ajusteBoveda`/`aprobarEventoCaja` + trigger `recalcBoveda`; roles `rolDeCaja`/`rolDeOwner`; alertas→`saludEventos` vía `notificarCaja`; `emitirAlerta` durable; smoke-test de carga ✓). **L-72 ya resuelto en `d244d77`** (verificado `pos.js:430`). **B5b-0 transporte cliente ✅** (`js/pedidos-service.js`: 7 callables + 8 listeners de lectura + default export; build Vite verde). **B5b-1 UI ✅** (build verde + `test:caja-format` 9/9):
  - `js/admin/caja-format.js` (helper PURO: ecuación del cajón espejo §8.1.7 + labels + `trasladoSugerido`/`superaLimite`) + `tests/caja-format.test.mjs` (9/9).
  - `js/pedidos-service.js`: 8 listeners de lectura por rol (`onCajaEstadoChange`/`onConfigCajaChange`/`onTurnoChange`/`onMovsCajaChange`/`onVentasTurnoChange`/`onTrasladosTurnoChange`/`onBovedaChange`/`onBovedaMovsChange`).
  - **POS caja** (`js/admin/pos.js` + `admin-pos.html`): panel de turno (abrir/estado/efectivo estimado del cajón/barra de límite), movimientos ingreso/egreso, modal de traslado obligatorio al superar el límite (venta bloqueada hasta trasladar), cierre enlazado (cerrarTurno vs Z legacy según haya turno abierto) + desglose por medio digital. Gate `isCaja` (owner/admin/caja); catálogo/editor conservan el Z legacy.
  - **Bóveda owner** (`js/admin/boveda.js` + `admin-boveda.html`, `requireAuth('owner')`): saldo + consignar al banco/reponer cambio/conteo físico (ajuste) + reverso desde el ledger + **panel de aprobaciones** (Dual-Approval §9.1). DOM seguro (sin innerHTML).
  - Nav: ítem "Bóveda" owner-only (`sidebar-data.js`) + `APP_VERSION v39`.
  - ⚠️ **Efectivo del cajón = ESTIMACIÓN operativa** (autoridad = cierre server): el owner usa el ledger real (`onTrasladosTurnoChange`, exacto/reload-proof); un rol sin lectura de bóveda cae a un contador de traslados **en memoria por sesión** (no localStorage: invariante "nada de dinero en storage") → **L-77**.
  - 🚩 **BRECHA DEL ROL `caja` (reportada a Daniel, gate de dinero)**: el rol `caja` distingue turno/bóveda (reglas + `rolDeCaja`), PERO `crearPedido` usa `rolDeVentas` (sin `caja`), `pedidos` read = `isVentas` (sin `caja`), y el cliente (`ROLE_LEVELS`/`roleLanding` en `auth.js`) no conoce `caja`. → la cajera con su propio usuario aún NO registra ventas ni lee pedidos. En prod Kary opera como `owner` (todo funciona). Habilitar la cajera end-to-end = paso aparte con gate (toca `crearPedido` = dinero → TDD + autorización) → **L-78 / candidato B5d**.
  - **Pend: B5c** = SW bump v76 + **deploy manual** (functions+rules+indexes, aún sin desplegar) + sembrar `config/caja` (owner) + gate Chrome (usuario owner + futuro `caja`) + ADR.
