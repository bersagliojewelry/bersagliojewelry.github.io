...
- [~] B5 UI + gate prod (Chrome) + SW bump + ADR — **B5a wrappers onCall ✅** (`pedidos.js`+`index.js`: `abrirTurno`/`cerrarTurno`/`movimientoCaja`/`registrarTraslado`/`reversoTraslado`/`ajusteBoveda`/`aprobarEventoCaja` + trigger `recalcBoveda`; roles `rolDeCaja`/`rolDeOwner`; alertas→`saludEventos` vía `notificarCaja`; `emitirAlerta` durable; smoke-test de carga ✓). **L-72 ya resuelto en `d244d77`** (verificado `pos.js:430`). **B5b-0 transporte cliente ✅** (`js/pedidos-service.js`: 7 callables `abrirTurno`…`aprobarEventoCaja` + default export; build Vite verde 3.9s). **Pend: B5b-1 UI** (POS caja apertura/cierre/movimientos/modal traslado + vista Bóveda owner + panel aprobaciones; requiere sesión admin autenticada en Chrome) → **B5c** SW bump + deploy manual + gate Chrome + ADR.
