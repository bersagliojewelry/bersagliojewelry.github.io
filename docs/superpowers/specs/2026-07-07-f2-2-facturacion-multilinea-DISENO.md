...
- **D · FISCAL = ✅ RESUELTO** (Claude-contador sobre el RUT real, 2026-07-07 · detalle → `42-LEGAL §7`): la titular **Diana M. Niño Mendoza (NIT 32.908.305-6)** es **NO responsable de IVA (RUT cód. 49)** + renta **régimen ordinario (cód. 05)**, actividad 3211 (joyas). **⇒ NO cobra IVA — ni en joyas ni en servicios.** El precio de cada línea es el precio FINAL (coherente con Términos §02, LEGAL-09). **Por tanto F2.2 NO necesita lógica fiscal por línea** (joya y servicio se cobran idéntico, sin impuesto). **Future-proofing (comité, barato)**: el esquema igual nace con `naturaleza:'bien'|'servicio'` + `precioSnapshot` por línea y slot `impuesto` (default 0) → si algún día cruza el umbral y pasa a **Responsable de IVA**, agregar el 19% es aditivo, NO migración. El export contador gana una **columna `naturaleza`** (útil para su renta por actividad: joyas 3211 vs otras), pero **sin línea de IVA**. **[a verificar / monitorear]**: (a) factura electrónica DIAN para persona natural No-responsable — obligación separada de F2.2 (F2.2 registra la venta interna; la e-factura es otra integración); el §2 del lóbulo decía "es sociedad" → **corregir: es persona natural/establecimiento**; (b) umbral de No-responsable de IVA — si las ventas/consignaciones anuales crecen, debe volverse Responsable (avisar en cada cierre de año, como SAGRILAFT). Disclaimer §0 aplica (orientación sólida; visto bueno de abogado = recomendado, no freno — riesgo ya aceptado por Daniel, fila LAWYER).
...
- [x] **GATE D (fiscal) RESUELTO** (Claude-contador sobre el RUT real): No responsable de IVA → SIN lógica fiscal por línea; `naturaleza` como future-proof; export gana columna `naturaleza`. Detalle → `42-LEGAL §7`.
- [ ] Build: `servicios` (colección+reglas+CMS owner) · `crearPedido` `lineasExtra` (TDD estricto) · POS multi-línea (bloque colapsado, chips, línea libre blindada) · export columna `naturaleza`.
