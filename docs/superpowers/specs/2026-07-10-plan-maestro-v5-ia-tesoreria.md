...
Cuando Daniel dé la orden final: **BORRAR** pedidos de prueba (todos EXCEPTO `d87ab568…` código BJ-HPFS-84R8, la compra real de $5.000) · turnos · movsCaja · bovedaMovimientos · arqueo · webhookEvents de prueba · saludEventos resueltos · clientes/vendedoras DE PRUEBA. **CONSERVAR**: piezas/colecciones reales, config, usuarios, el pedido real. **✅ DECISIÓN RESUELTA (Daniel, 2026-07-27 — confirma y AMPLÍA el RESET-A-CERO del 2026-06-20)**: *"toda la información cargada en el panel admin responde a puras pruebas a excepción de las colecciones y las piezas; todo lo demás, incluyendo cartera, alertas, movimientos y demás, son puras pruebas y debemos depurarlas del panel y de Firebase para no dejar basura"*. ⇒ **la cartera migrada TAMBIÉN se purga** (Kary re-registra desde cero); se CONSERVAN solo **piezas y colecciones** (+ config/usuarios técnicos). Se ejecuta AL FINAL, cuando todo esté terminado. → **TODO-80** · `[[project_purga_datos_prueba]]`. Corolario vinculante: **NO reparar, migrar ni backfillear datos del panel** — es trabajo tirado sobre basura. El borrado se hará por script auditado (Admin SDK, dry-run primero, respaldo previo vía backup diario §64) — nunca a mano.
