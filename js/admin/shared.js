...
// El traductor vive en `error-format.js` (helper PURO con test: `npm run test:errores`);
// aquí solo se RE-EXPORTA para que los ~18 callsites del panel lo sigan importando de
// `shared.js` sin cambio alguno (§3.2 API estable). TODO-79: normaliza el prefijo
// `functions/` que añade el SDK de callables y PREFIERE el motivo real del servidor en
// los rechazos de negocio (antes, TODO rechazo de una CF se veía como el genérico).
export { errorMessage, errorCode, esErrorDeCallable } from './error-format.js';
