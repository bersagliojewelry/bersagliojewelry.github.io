...
function itemTemplateHTML(sectionKey, f, item, index) {
    ...
    const body = itemFields
        .map(sf => fieldHTML(`${sectionKey}.${f.name}.${index}`, sf, it[sf.name] ?? ''))
        .join('');
...
function collectList(raw, sectionKey, f) {
    ...
    for (const key of Object.keys(raw)) {
        ...
        if (!declared.has(sub)) continue;          // solo sub-campos declarados
    ...
    let arr = indices.map(i => {
        const item = {};
        for (const sf of itemFields) item[sf.name] = byIndex.get(i)[sf.name] ?? '';
        return item;
    });
