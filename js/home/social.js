/**
 * Home · Sección NUEVA — "Lo último en nuestras redes" (feed social).
 * Tabs por red + grid de tarjetas con logos de marca reales (IG/FB/TikTok).
 * Datos de ejemplo en js/data/home-media.js (TODO: feed real vía APIs).
 */
import { html, escape } from '../core/html.js';
import { SOCIAL, SOCIAL_PLATFORMS } from '../data/home-media.js';
...
function statIcon(kind) {
    if (kind === 'views') {
        return html`<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M1 12s4-7 11-7 11 7 11 7-4 7-11 7-11-7-11-7z"/><circle cx="12" cy="12" r="3"/></svg>`;
    }
    return html`<svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 21s-8-4.5-8-10a4.5 4.5 0 0 1 8-2.9A4.5 4.5 0 0 1 20 11c0 5.5-8 10-8 10z"/></svg>`;
}

function renderCards() {
    const list = _tab === 'Todas' ? SOCIAL : SOCIAL.filter(p => p.platform === _tab);
    return list.slice(0, 8).map(p => html`
        <a class="social-card" href="${escape(HREFS[p.platform] || '#')}" target="_blank" rel="noopener noreferrer">
            ...
            <span class="mono social-type">${escape(p.type)}</span>
            <div class="social-card-body">
                <div class="social-card-caption">${escape(p.caption)}</div>
                <div class="social-card-meta">
                    <span class="social-card-stat">${statIcon(p.kind)}${escape(p.stat)}</span>
...
export function renderSocial() {
    return html`
        <section class="home-social reveal">
            <div class="container">
                ...
                <div class="social-tabs">
                    ${SOCIAL_PLATFORMS.map(p => html`<button class="social-tab ${p === _tab ? 'on' : ''}" type="button" data-social-tab="${escape(p)}">${p === 'Todas' ? '' : platformIcon(p, 14)}${escape(p)}</button>`)}
                </div>
                ...
                <div class="social-follow">
                    <a class="btn-aqua" href="${HREFS.Instagram}" target="_blank" rel="noopener noreferrer">${platformIcon('Instagram', 15)} @bersagliojewelry</a>
                    <a class="btn-aqua" href="${HREFS.TikTok}" target="_blank" rel="noopener noreferrer">${platformIcon('TikTok', 14)} TikTok</a>
                    <a class="btn-aqua" href="${HREFS.Facebook}" target="_blank" rel="noopener noreferrer">${platformIcon('Facebook', 15)} Facebook</a>
                </div>
            </div>
        </section>`;
}
...
export default { renderSocial, initSocial };
